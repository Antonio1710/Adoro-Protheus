...
/*/{Protheus.doc} User Function MT103EXC
	...
	@history ticket 18465   - Everson - 13/01/2023 - Removida valida็ใo de exclusใo.
	@history ticket 18465   - Everson - 16/01/2023 - Atualiza registro na rotina de entrada de mat้ria-prima..
    @history ticket TI - Antonio Domingos - 26/05/2023 - Revisใo Ajuste Nova Empresa
	@history ticket TI - Antonio Domingos - 02/06/2023 - Valida็ใo Ajuste Nova Empresa
/*/
User Function MT103EXC()
	...
	Private _cEmpFLA  := SuperGetMv("MV_#EMPFLA",.F.,"0102/0103/010B/1301") //Codigo de Empresas Ativas Grupo A //ticket TI - William Costa - 20/05/2023

	If Alltrim(cEmpAnt) $ _cEmpAt1 //Codigo de Empresas Ativas Grupo 1 //ticket TI - William Costa - 20/05/2023
		If !(SF1->F1_CODIGEN<1) .AND. !IsInCallStack("U_ADSAG001P") .and. !(SF1->F1_FORMUL=='S' .AND. "PRODUTOR RURAL" $ SF1->F1_MENNOTA) .And. ! U_ADFAT639(SF1->F1_DOC, SF1->F1_SERIE, SF1->F1_FORNECE, SF1->F1_LOJA, 1) //Everson - 16/01/2023.
			lRet := .F.      
	...
	If Alltrim(cEmpAnt)+Alltrim(cFilAnt) $ _cEmpFLA //Codigo de Empresas Ativas Grupo 1 //ticket TI - William Costa - 20/05/2023
	...
	//Deixar a chamada desta fun็ใo sempre por ๚ltimo, pois atualiza o status do registro na rotina de entrada de mat้ria-prima.
	//Everson - 16/01/2023 - ticket 18465.
	If lRet .And. (cFilAnt $cFiliais) .And. Alltrim(SF1->F1_ESPECIE) = "SPED"
		U_ADFAT63O(SF1->F1_DOC, SF1->F1_SERIE, SF1->F1_FORNECE, SF1->F1_LOJA, "NF", "4") 
...
Static Function ChkPrjNeg()
	...
	Do While Work->( !EOF() )
		...
		// Consumo Projeto sem a NF que estแ sendo estornada/excluํda
	...
	Do While TRB->( !EOF() )
		...
		If SC7->( dbSeek(FWxFilial("SC7")+TRB->D1_PEDIDO+TRB->D1_ITEMPC) )
			...
			Else
				...
				// @history ticket 18137   - Fernando Macieira - 09/08/2021 - Estorno NF de projeto (A็ใo: Corrigido cแlculo)

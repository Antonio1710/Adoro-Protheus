...
/*/{Protheus.doc} User Function FA050ALT
	...
	@history ticket 82889  -  Antonio Domingos - 03/11/2022 - erro na ambiente de produção
	@history ticket 84559  -  Antonio Domingos - 08/12/2022 - AVISO DE CODIGO DE BARRAS DUPLICADO E DIVERGENCIA DE VALOR ENTRE BOLETO E TITULO
				...
				if lPerAlt
					if M->E2_VENCTO < dE2VENCTO  // Antecipa Vencimento
						if M->E2_VENCTO < ( dE2VENCTO - nDiaMenos ) .OR. nVlrCust > TOLPERJUR
							...
							M->E2_VENCREA := DataValida(dE2VENCREA,.T.) //Chamado 82889 03/11/2022 - Antonio Domingos
					...
					Elseif M->E2_VENCTO > dE2VENCTO  // Posterga Vencimento
						if M->E2_VENCTO  > ( dE2VENCTO + nDiaMais ) .OR. nVlrCust > TOLPERDES
							...
							// Volta o vencimento original, apos aprovacao sera alterado
							M->E2_VENCTO  := dE2VENCTO
							M->E2_VENCREA := DataValida(dE2VENCREA,.T.) //ticket 81308 01/11/2022 - Antonio Domingos
...
User Function ChkCodBar(cCodBar)
	...
	//AEval( FWLoadSM0(), {|u| iif( AScan(aEmpresas, {|v| u[1] == v[1] }) <= 0, AAdd( aEmpresas, { u[1]} ) , Nil) })

	for i:=1 to len(aEmpresas)

		If !(cX2ARQUIVO $ cX2CodEmp)

			cX2ARQUIVO := fGetSX2Emp( aEmpresas[i,1], "SE2", "X2_ARQUIVO")
			...
			MEMOWRITE("C:\TEMP\FA050ALT.SQL", cQuery )
...
@param cTab, character, Tabela
@param cFieldRet, character, Campo da SX2 para consulta
@return cRet, Conteudo do campo consultado
/*/
Static Function fGetSX2Emp( cEmp, cTab, cFieldRet ) //fGetSX2Emp( aEmpresas[i,1], "SE2", "X2_ARQUIVO")
	local cX2Alias 	 := GetNextAlias()
	local cRet 		 := ""
	local cX2Table   := ""
	local cFieldQry  := ""
	local lMVATDICB  := SUPERGETMV("MV_#ATDICB",.F.,.F.) //Ativa Dicionario no Banco ticket 84559  -  Antonio Domingos - 08/12/2022
	local _cPath     := "\SYSTEM\" //ticket 84559  -  Antonio Domingos - 08/12/2022
	Local cIndex   	 :=	 "X2_CHAVE" //ticket 84559  -  Antonio Domingos - 08/12/2022
	default cEmp		:= cEmpAnt
	default cTab 		:= ""
	default cFieldRet 	:= "X2_CHAVE" //ticket 84559  -  Antonio Domingos - 08/12/2022

	// @history Ticket 85390 - Rodrigo Mello | Flek - 22/02/2023 - Ajustes para dic. banco de dados.
	//If lMVATDICB

		cX2Table := "%SX2"+cEmp+"0%"
		cFieldQry := '%'+alltrim(cFieldRet)+'%'

		beginSQL Alias cX2Alias
			select %exp:cFieldQry%
			from %exp:cX2Table%
			where %notdel%
				and X2_CHAVE = %exp:cTab%
		endSQL

		if (cX2Alias)->(!eof())
			cRet := alltrim( (cX2Alias)->( FieldGet( FieldPos( cFieldRet ) ) ) )
		endif

		(cX2Alias)->(dbCloseArea())
	/*
	Else
	  	//inicion ticket 84559  -  Antonio Domingos - 08/12/2022
		_cArquivo := _cPath+"SX2"+cEmp+"0"
		_cArqInd := CriaTrab( Nil, .F. )
		If File(_cArquivo+".dtc")
		   dbUseArea(.T., __LocalDriver, _cArquivo+".dtc", "SX2EMP", .T., .F.)
		   dbSetIndex( _cArquivo+".cdx" )  
		   dbSelectArea("SX2EMP")
		   dbSeek(cTab)
		   cRet := 	alltrim(SX2EMP->X2_ARQUIVO)
		   SX2EMP->(dbCloseArea())
		   FErase( _cArqInd+OrdBagExt())
		EndIf
		//fim ticket 84559  -  Antonio Domingos - 08/12/2022
	EndIf
	*/

...
/*/{Protheus.doc} User Function M460FIM
	...
	@history Everson, 07/10/2022, ticket 80379 - Troca de placa na NF para operações de transbordo e carga seccionada.
	@history Everson, 12/12/2022, ticket 83060 - Envio de mensagem ao ChatGuru.
...
User Function M460FIM()
	...
	Local cNotaO	  := SF2->F2_DOC
	Local cSeriO	  := SF2->F2_SERIE
	Local cPedO		  := SC5->C5_NUM
   ...
   // ticket 84085 - 01/12/2022 - Fernando Macieira - TRANSFERENCIA DE ATIVO IMOBILIZADO nota fiscal não aparece no Faturamento
	...
	u_GrLogZBE(Date(), Time(), cUserName, "1-Checagem da geração de frete", "FATURAMENTO", "M460FIM",;
			   "Empresa/Filial/Parâmetro/Pedido: " + cValToChar(cEmpAnt) + "/" + cValToChar(cFilAnt) + "/" + cValToChar(cFilGFrt) + "/" + cValToChar(SC5->C5_NUM), ComputerName(), LogUserName())
	...
	If Alltrim(cEmpAnt)+Alltrim(cFilAnt) $ _cEmpFLA //.And. ! ISINCALLSTACK("U_CCSP_002") //Everson-CH:044314-03/06/19. //Everson-CH:044314-12/06/19. //ticket TI - William Costa - 20/05/2023
		...
		u_GrLogZBE(Date(), Time(), cUserName, "2-Checagem da geração de frete", "FATURAMENTO", "M460FIM",;
				   "Empresa/Filial/Parâmetro/Pedido/U_CCSP_002: " + cValToChar(cEmpAnt) + "/" + cValToChar(cFilAnt) + "/" + cValToChar(cFilGFrt) + "/" + cValToChar(SC5->C5_NUM) + "/" + cValToChar(ISINCALLSTACK("U_CCSP_002")), ComputerName(), LogUserName())
	...
	If Alltrim(cEmpAnt) $ _cEmpAt1 //ticket TI - William Costa - 20/05/2023
		...
		//Everson - 07/10/2022 - Ticket 80379.
		If !Empty(cCrgAgr)
			altPlaca(cCrgAgr, cPlcAgr, cNotaO, cSeriO, cPedO)
	...
	If FIE->( dbSeek(SC5->C5_FILIAL+"R"+SC5->C5_NUM) )
    	...
    	If FIE->FIE_SALDO > 0 // ticket 745 - FWNM - 21/09/2020 - Implementação título PR
			...
			If SE1->( dbSeek(FWxFilial("SE1")+FIE->(FIE_PREFIX+FIE_NUM+FIE_PARCEL+FIE_TIPO)) )
				...
				PERGUNTE("AFI340",.F.) // Commpensação Contas Pagar
				MV_PAR11 := 2 // Contabiliza On Line ? = NÃO

				// @history ticket 69652 - Fer Macieira - 15/03/2022 - COMPENSAÇÃO DE RA - MADRUGADA
				PERGUNTE("FIN330",.F.) // Commpensação Contas Receber
...
Static Function grvBarr(cOperacao, cNumero, cNF, cSerie)
	...
	If cOperacao == "I" .And. ! Empty(SD2->D2_PEDIDO)
		...
		//Everson - 12/12/2022 - 83060 - Salva registro na tabela ZIQ para envio ao ChatGuru.
		U_ADFAT640(SF2->F2_DOC, SF2->F2_SERIE, SF2->F2_CLIENTE, SF2->F2_LOJA) 
...
Static Function chkOrd(cNF, cSerie)
	...
	cQuery += " SELECT " 
	cQuery += " C5_XORDPES, C5_XCOTRMP "  //Everson - 14/03/2023. Chamado 18465.
	cQuery += " FROM " 
	...
	cQuery += " AND (C5_XORDPES <> '' OR  C5_XCOTRMP <> '') " //Everson - 14/03/2023. Chamado 18465.
...
/*/{Protheus.doc} altPlaca
	Altera placa e transportadora para operação de transbordo e carga seccionada.
	@type  Static Function
	@author Everson
	@since 07/10/2022
	@version 01
/*/
Static Function altPlaca(cCrgAgr, cPlcAgr, cNotaO, cSeriO, cPedO)
	...
	DbSelectArea("ZV4")
	ZV4->(DbGoTop())
	If ! ZV4->( DbSeek( FWxFilial("ZV4") + cPlcAgr ) )

		u_GrLogZBE(Date(), Time(), cUserName, "Placa transbordo não localizada " + cPlcAgr, "FATURAMENTO", "M460FIM",;
			       "Empresa/Filial/NF: " + cValToChar(cEmpAnt) + "/" + cValToChar(cFilAnt) + "/" + cValToChar(cNotaO) + cValToChar(cSeriO), ComputerName(), LogUserName())
	...
	If Empty(cTransp)
		...
		u_GrLogZBE(Date(), Time(), cUserName, "Placa transbordo " + cPlcAgr + " não possui transportadora vinculada", "FATURAMENTO", "M460FIM",;
			       "Empresa/Filial/NF: " + cValToChar(cEmpAnt) + "/" + cValToChar(cFilAnt) + "/" + cValToChar(cNotaO) + cValToChar(cSeriO), ComputerName(), LogUserName())
	...
	SF2->(DbSetOrder(1))
	SF2->(DbGoTop())
	SF2->(DbSeek( FWxFilial("SF2") + cNotaO + cSeriO ))

	cPlcAnt := SF2->F2_PLACA 
	cTraAnt := SF2->F2_TRANSP

	RecLock("SF2", .F.)
		SF2->F2_PLACA  := cPlcAgr 	
		SF2->F2_TRANSP := cTransp 	
	SF2->(MsUnlock())

	u_GrLogZBE(Date(), Time(), cUserName, "Placa/transportadora alterada de/para " + cPlcAnt + " " + cTraAnt + " / " + cPlcAgr + " " + cTraAnt, "FATURAMENTO", "M460FIM",;
			   "Empresa/Filial/NF: " + cValToChar(cEmpAnt) + "/" + cValToChar(cFilAnt) + "/" + cValToChar(cNotaO) + cValToChar(cSeriO), ComputerName(), LogUserName())

...
/*/{Protheus.doc} User Function nomeFunction
	...
	@history ticket 89839 - Fernando Macieira - 08/03/2023 - Problema no empenho ordens processadas Galo/galinha importadas do e-data sem empenho
/*/
...
Static Function ADEDA007A
	...
	For x:=1 To Len(aOutros)
		...
		AADD( aHeader, { 	AllTrim( GetSX3Cache( aOutros[x], 'X3_TITULO') )	,; // 01 - Titulo
							GetSX3Cache( aOutros[x], 'X3_CAMPO'  	)	 		,; // 02 - Campo
							GetSX3Cache( aOutros[x], 'X3_PICTURE'	)	 		,; // 03 - Picture
							GetSX3Cache( aOutros[x], 'X3_TAMANHO'	)	 		,; // 04 - Tamanho
							GetSX3Cache( aOutros[x], 'X3_DECIMAL'	)	 		,; // 05 - Decimal
							GetSX3Cache( aOutros[x], 'X3_VALID'  	)	 		,; // 06 - Valid
							GetSX3Cache( aOutros[x], 'X3_USADO'  	)	 		,; // 07 - Usado
							GetSX3Cache( aOutros[x], 'X3_TIPO'  	)	 		,; // 08 - Tipo
							GetSX3Cache( aOutros[x], 'X3_F3'		)	 		,; // 09 - F3
							GetSX3Cache( aOutros[x], 'X3_CONTEXT' 	)	 		,; // 10 - Contexto
							GetSX3Cache( aOutros[x], 'X3_CBOX'		) 			,; // 11 - ComboBox
							GetSX3Cache( aOutros[x], 'X3_RELACAO' 	)	 		,; // 12 - Relacao
							GetSX3Cache( aOutros[x], 'X3_INIBRW'  	)	 		,; // 13 - Inicializador Browse
							GetSX3Cache( aOutros[x], 'X3_BROWSE'  	)	 		,; // 14 - Mostra no Browse
							GetSX3Cache( aOutros[x], 'X3_VISUAL'  	)	 		})
		...
		If aCols[x,2] // Marcado
			...
			If AllTrim(aCols[x,11]) == "OPR" .and. AllTrim(aCols[x,13]) == "T"
				...
				If Empty(aCols[x,12]) .or. !(AllTrim(aCols[x,12]) $ cLocal3)
					...
					msgAlert("Produ็ใo de terceiro precisa possuir um LOCAL vแlido! " + Chr(13) + Chr(10) + "Verifique/Corrija a linha " + AllTrim(Str(x)) + " clicando com o botใo direito do mouse..." )
					Return .f.

				Else
					
					// Gravo novo conte๚do na tabela OPR010 pois a fun็ใo ADEDA003P que farแ os execautos nใo olha o acols e sim faz uma nova query na tabela OPR010)
					...
					If AllTrim(aCols[x,12]) <> cLocOPR
						...
						If tcSqlExec(cSql) < 0
							Alert("Local alterado na linha " + Str(x) + " nใo foi gravado na tabela OPR010! Informe ao TI o erro que serแ mostrado na pr๓xima tela...")
		...
		For x:=1 To Len(aArrays) 
			For y:=1 To Len(aArrays[x])
				...
				If aArrays[x,y,1,9] == "Processado" .AND. !EMPTY( ALLTRIM( aArrays[x,y,1,8] ) )
					...
					Else
						//TcSqlExec("UPDATE " + cTabela + " SET D_E_L_E_T_='*' WHERE REC = " + ALLTRIM( StrZero(aArrays[x,y,2,1], 10) ) )	// @history ticket 89839 - Fernando Macieira - 08/03/2023 - Problema no empenho ordens processadas Galo/galinha importadas do e-data sem empenho
					EndIf
		...
		If aCols[i,2] // Marcado
			...
			If AllTrim(aCols[i,11]) == "OPR" .and. AllTrim(aCols[i,13]) == "T"
				...
				If Empty(aCols[i,12]) .or. !(AllTrim(aCols[i,12]) $ cLocal3)
					...
					lRet := .f.
					msgAlert("Produ็ใo de terceiro precisa possuir um LOCAL vแlido! " + Chr(13) + Chr(10) + "Verifique/Corrija a linha " + AllTrim(Str(i)) + " clicando com o botใo direito do mouse..." )

...
/*/{Protheus.doc} User function ADEST018P()
	...
	@history ticket   80451 - FWNM - 27/09/2022 - Configuração rotina Massa de frango Itapira ADEDA009
	@history ticket   80451 - FWNM - 29/09/2022 - Inibir produtos que não tiverem produção
	@history ticket   80460 - FWNM - 17/10/2022 - Adequação querys para funcionamento tanto na PRODUCAO quanto no SIG, devido diferenças de compartilhamento das tabelas
	@history ticket   80460 - FWNM - 19/10/2022 - Correção join na query campos filiais, diferentes na produção, devido diferenças de compartilhamento das tabelas
/*/
...
Static function ADEST018G()
	...
	Local nSomaQuant := 0
	Local nCustoVivo := 0
	Local nProdSD3   := 0 // @history ticket   80451 - FWNM - 29/09/2022 - Configuração rotina Massa de frango Itapira ADEDA009
	...
	/* 
	@history ticket   80451 - FWNM - 27/09/2022 - Configuração rotina Massa de frango Itapira ADEDA009
	Parametro MV_XCODMAS será configurado por filial
	produtos Itapira: 354821 - MASSA DE FRANGO ITAPIRA E  354822 - QUEBRA MASSA DE FRANGO ITAPIRA 
	*/
		...
		For y:= 1 To Len(aCampos[x]) //só estou usando a variavel x porque também possui o valor 3
			//If SX3->( dbSeek( aCampos[x, y] ) )
				AADD( aHeader, { 	AllTrim( GetSX3Cache(aCampos[x, y], 'X3_TITULO') )	,; // 01 - Titulo
									...
									iif( aCampos[x, y] == "D3_CUSTO1", "@E 999,999,999.99", GetSX3Cache(aCampos[x, y], 'X3_PICTURE'	) )	 		,; // 03 - Picture
									GetSX3Cache(aCampos[x, y], 'X3_TAMANHO'	) + iif( aCampos[x, y] ==  "B1_DESC", 10, 0) ,; // 04 - Tamanho
...
Static Function ADEST018H()
	...
	Local cGranjaQry  := FormatIn(cFilGranjas, "|")// @history ticket   80451 - FWNM - 27/09/2022 - Configuração rotina Massa de frango Itapira ADEDA009

...
/*{Protheus.doc} User Function ADPMS005P
	...
	@history Ticket 84925 - 25/01/2022 - Fernando Macieira - Divergência nos valores(Consumo e saldo) entre relatórios: POSIÇÃO PROJETOS INVEST  e  PROJETO DETALHADO
	@history ticket 85390 - Antonio - 30/01/2023 -Validacao fontes v33 - dic. banco de dados.       
	@history ticket 94000 - Antonio Domingos - 17/05/2023 - sem acesso aos projetos da empresa vista - Incluido aviso não existe projetos aprovados.
	@history ticket 94000 - Antonio Domingos - 17/05/2023 - Incluido Parambox
/*/
User Function ADPMS005P(cRotina, nPosArotina) // PRODUCAO
	...
	Private aParamBox := {} //ticket 94000 - Antonio Domingos - 17/05/2023
	Private aRet      := {} //ticket 94000 - Antonio Domingos - 17/05/2023
	Private _cAeaCod  := CriaVar("AEA_COD",.F.)
    ...
    Aadd( aParamBox ,{1,"Da  Fase ",_cAeaCod,"" ,'.T.',"AEA",'.T.',30,.T.,}) //ticket 94000 - Antonio Domingos - 17/05/2023
    Aadd( aParamBox ,{1,"Ate Fase ",_cAeaCod,"" ,'.T.',"AEA",'.T.',30,.T.,}) //ticket 94000 - Antonio Domingos - 17/05/2023

	//
	If ParamBox(aParamBox, "Parametros", @aRet,,,,,,,,.T.,.T.) //ticket 94000 - Antonio Domingos - 17/05/2023
		dbSelectArea("AF8")
		dbSetOrder(1) 
		...
		Else
			//Inicio ticket 94000 - Antonio Domingos - 17/05/2023
			If Select("TMP") > 0
				mBrowse( 6, 1,22,75,"TMP",aCposBrw,,,,,,,,,,,,,cFilter)
			else
				Aviso("Atenção - ADPMS005P-01", " Não existe projeto(s) com fase 03-Aprovado(s)! Verifique aprovação dos projetos...", {"OK"}, 3)
			EndIf
			//Fim ticket 94000 - Antonio Domingos - 17/05/2023
		Endif
			
		If Select("TMP") > 0
			TMP->(dbCloseArea())
		EndIf
	EndIf //ticket 94000 - Antonio Domingos - 17/05/2023
...
//	cQuery := " SELECT AF8_PROJET, AF8_REVISA, AF8_DESCRI, AF8_DATA, AF8_XVALOR, AF8_XCONSU, AF8_XDTCON " // PRODUCAO
	...
	cQuery += " AND AF8_FASE>='"+AllTrim(mv_par01)+"' " //ticket 94000 - Antonio Domingos - 17/05/2023
	cQuery += " AND AF8_FASE<='"+AllTrim(mv_par02)+"' " //ticket 94000 - Antonio Domingos - 17/05/2023
	...
	If !TRB->(Eof())
		...
		oTemp := FWTemporaryTable():New("TMP", aEstrut)
		oTemp:AddIndex("IDX001", {"AF8_PROJET"} )
		oTemp:Create()		
...
User Function ADFC030CON()
	...
	If lC7Fiscori
		...
		aCampos3 :=	{{"FILORIG"		, "C", FWSizeFilial(), 0, GetSX3Cache("C7_FISCORI", "X3_TITULO")}, ;
	...
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	//@history Ticket 70142  - Rodrigo / Flek Solution - 18/07/2022 - Ajustes Dic. de dados no Banco	
	//aCampos4:=		{{"FILORIG"		, "C", FWSizeFilial(), 0, GetSX3Cache("C7_FISCORI", "X3_TITULO")SX3->(RetTitle("F1_FILORIG"))}, ;
	aCampos4:=		{{"FILORIG"		, "C", FWSizeFilial(), 0, GetSX3Cache("F1_FILORIG", "X3_TITULO")}, ;
	...
	If lC7Fiscori
		...
		aCampos5 :=	{{"FILORIG"	, "C", FWSizeFilial(), 0, GetSX3Cache("C7_FISCORI", "X3_TITULO")}, ;
		{"NUMERO"   , "C", 06, 0,OemToAnsi(STR0007) }, ;							//"Numero"
		{"EMISSAO"	, "D", 08, 0,OemToAnsi(STR0010) }, ;							//"Emissao"
	...
	If cPaisLoc	!=	"BRA"
		...
		aCampos6 :=	{{"FILORIG"		, "C", FWSizeFilial(), 0, GetSX3Cache("E2_FILORIG", "X3_TITULO")}, ;
	...
	aCampos7 :=	{{"FILORIG"		, "C", FWSizeFilial(), 0, GetSX3Cache("E2_FILORI", "X3_TITULO")}, ;
	...
	aCampos8 :=	{{"FILORIG"		, "C", FWSizeFilial(), 0, GetSX3Cache("E2_FILORI", "X3_TITULO")}, ;
	...
	aCampos9 :=	{{"FILIAL"		, "C", FWSizeFilial(), 0, GetSX3Cache("CP_FILIAL", "X3_TITULO") }, ;
...
Static Function FC030Mostr(lProc030a,lProc030b,lProc030c,lProc030d,lProc030e,lProc030f,lProc030g)
	...
	*/
	...
	If FWModeAccess("SF1",1) == "E"		/* GESTAO */
		...
		HEADER GetSX3Cache("E2_FILORIG", "X3_TITULO"),STR0007,STR0010,STR0030,STR0031,"Pedido","Item PC" SIZE aPosObj[2,4]-9,aPosObj[2,3]-82 OF oFolder030:aDialogs[2] PIXEL
	Else
...
//	oFolder030:aDialogs[3]:Refresh()
	...
	If lC7Fiscori .And. FWModeAccess("SC7",1) == "E"		/* GESTAO */
		...
		HEADER GetSX3Cache("C7_FISCORI", "X3_TITULO"),STR0007,STR0010,STR0135,STR0029 SIZE aPosObj[2,4]-9,aPosObj[2,3]-82 OF oFolder030:aDialogs[3] PIXEL
	Else
...
//	oFolder030:aDialogs[7]:Refresh()
	...
	If FWModeAccess("SF2",1) == "E"		/* GESTAO */
		...
		HEADER GetSX3Cache("E2_FILORIG", "X3_TITULO"),STR0007,STR0010,STR0030 SIZE aPosObj[2,4]-9,aPosObj[2,3]-82 OF oFolder030:aDialogs[5] PIXEL
	Else
	...
	If lC7Fiscori .And. FWModeAccess("SC7",1) == "E"		/* GESTAO */
		...
		HEADER  GetSX3Cache("C7_FISCORI", "X3_TITULO"),STR0007,STR0010,STR0032,STR0132,STR0033,STR0034,STR0035,STR0036,STR0037,STR0038 SIZE aPosObj[2,4]-9,aPosObj[2,3]-82 OF oFolder030:aDialogs[6] PIXEL
	...
	If FWModeAccess("SF1",1) == "E"		/* GESTAO */
		...
		HEADER "",GetSX3Cache("E2_FILORIG", "X3_TITULO"),STR0066,STR0010,"Pedido",STR0010,STR0032,STR0132,"Dt Prevista","Dt Realizada","Diferença Dias","" ;
	...
	If nProcess == 3
		...
		#IFDEF TOP
			If TcSrvType() != "AS/400"
				...
				aStru  := {}
				aX3Aux := {}
				AAdd( aX3Aux, "C7_FILIAL")
				If lC7Fiscori
					AAdd( aX3Aux, "C7_FISCORI")	
				Endif
				AAdd( aX3Aux, "C7_FORNECE")
				AAdd( aX3Aux, "C7_LOJA")
				AAdd( aX3Aux, "C7_NUM")
				AAdd( aX3Aux, "C7_TOTAL")
				AAdd( aX3Aux, "C7_PRODUTO")
				AAdd( aX3Aux, "C7_DESCRI")
				AAdd( aX3Aux, "C7_QUJE")
				AAdd( aX3Aux, "C7_EMISSAO")
				AAdd( aX3Aux, "C7_DATPRF")
				AAdd( aX3Aux, "C7_UM")
				AAdd( aX3Aux, "C7_SEGUM")
				AAdd( aX3Aux, "C7_QUANT")
				AAdd( aX3Aux, "C7_QTSEGUM")
				AAdd( aX3Aux, "C7_PRECO")
				AAdd( aX3Aux, "C7_COND")
				AAdd( aX3Aux, "C7_VLDESC")
				AAdd( aX3Aux, "C7_RESIDUO")
				AAdd( aX3Aux, "C7_VALFRE")
				AAdd( aX3Aux, "C7_DESPESA")
				AAdd( aX3Aux, "C7_SEGURO")
				AAdd( aX3Aux, "C7_VALIPI")
				AAdd( aX3Aux, "C7_ICMSRET")

				AEVal( aX3Aux, {|u| aadd( aStru,{ u, GetSX3Cache(u, "X3_TIPO"), GetSX3Cache(u, "X3_TAMANHO"), GetSX3Cache(u, "X3_DECIMAL")}) } )
	...
	If nProcess == 4
		...
		#IFDEF TOP
			If TcSrvType() != "AS/400"
				...
				aStru  := {}
				aX3Aux := {}
				AAdd( aX3Aux, "F1_FILIAL")
				AAdd( aX3Aux, "F1_FORNECE")	
				AAdd( aX3Aux, "F1_LOJA")
				AAdd( aX3Aux, "F1_EMISSAO")
				AAdd( aX3Aux, "F1_TIPO")
				AAdd( aX3Aux, "F1_VALMERC")
				AAdd( aX3Aux, "F1_FRETE")
				AAdd( aX3Aux, "F1_VALIPI")
				AAdd( aX3Aux, "F1_ICMSRET")
				AAdd( aX3Aux, "F1_DESPESA")
				AAdd( aX3Aux, "F1_DESCONT")
				AAdd( aX3Aux, "F1_DOC")
				AAdd( aX3Aux, "F1_DUPL")
				AAdd( aX3Aux, "D1_CF")
				AAdd( aX3Aux, "D1_VALICM")
				AAdd( aX3Aux, "D1_VALIMP5")
				AAdd( aX3Aux, "D1_VALIMP6")
				AAdd( aX3Aux, "D1_TOTAL")
				AAdd( aX3Aux, "D1_VALIPI")
				AAdd( aX3Aux, "D1_VALFRE")
				AAdd( aX3Aux, "D1_DESPESA")
				AAdd( aX3Aux, "D1_SEGURO")
				AAdd( aX3Aux, "D1_ICMSRET")
				AAdd( aX3Aux, "D1_VALDESC")
				If cPaisLoc <> "BRA"
					AAdd( aX3Aux, "F1_MOEDA")
					AAdd( aX3Aux, "F1_TXMOEDA")
				EndIf
				AEVal( aX3Aux, {|u| aadd(aStru ,{ u, GetSX3Cache(u, "X3_TIPO"), GetSX3Cache(u, "X3_TAMANHO"), GetSX3Cache(u, "X3_DECIMAL")} ) } )
	...
	If nProcess == 5
		...
		#IFDEF TOP
			If TcSrvType() != "AS/400"
				...
				aStru  := {}
				aX3Aux := {}
				AAdd( aX3Aux, "F2_FILIAL")
				AAdd( aX3Aux, "F2_CLIENTE")
				AAdd( aX3Aux, "F2_LOJA")
				AAdd( aX3Aux, "F2_EMISSAO")
				AAdd( aX3Aux, "F2_TIPO")
				AAdd( aX3Aux, "F2_VALMERC")
				AAdd( aX3Aux, "F2_FRETE")
				AAdd( aX3Aux, "F2_VALIPI")
				AAdd( aX3Aux, "F2_DESPESA")
				AAdd( aX3Aux, "F2_DESCONT")
				AAdd( aX3Aux, "F2_DOC")
				AAdd( aX3Aux, "F2_DUPL")
				AAdd( aX3Aux, "D2_TOTAL")
				AAdd( aX3Aux, "D2_DESCON")
				AEVal( aX3Aux, {|u| aadd(aStru ,{ u, GetSX3Cache(u, "X3_TIPO"), GetSX3Cache(u, "X3_TAMANHO"), GetSX3Cache(u, "X3_DECIMAL")}) } )
				/*
				aStru := {}
				dbSelectArea("SX3")
	...
	If nProcess == 6
		...
		#IFDEF TOP
			If TcSrvType() != "AS/400"
				...
				AAdd( aX3Aux, "F1_DOC")
				AAdd( aX3Aux, "F1_DUPL")
				AAdd( aX3Aux, "D1_PEDIDO")
				AAdd( aX3Aux, "D1_ITEMPC")
				AAdd( aX3Aux, "D1_COD")
				AAdd( aX3Aux, "C7_DATPRF")
				AAdd( aX3Aux, "C7_EMISSAO")
				AAdd( aX3Aux, "D1_DTDIGIT")
				AEVal( aX3Aux, {|u| aadd(aStru ,{ u, GetSX3Cache(u, "X3_TIPO"), GetSX3Cache(u, "X3_TAMANHO"), GetSX3Cache(u, "X3_DECIMAL")}) } )
				/*
				aStru := {}
				dbSelectArea("SX3")
	...
	If nProcess == 7 // ABA SA - CARQ9
		...
		#IFDEF TOP
			If TcSrvType() != "AS/400"
				...
				aStru  := {}
				aX3Aux := {}
				AAdd( aX3Aux, "CP_FILIAL")
				AAdd( aX3Aux, "CP_NUM")
				AAdd( aX3Aux, "CP_ITEM")
				AAdd( aX3Aux, "CP_PRODUTO")
				AAdd( aX3Aux, "CP_DESCRI")
				AAdd( aX3Aux, "CP_QUANT")
				AAdd( aX3Aux, "CP_UM")
				AAdd( aX3Aux, "CP_EMISSAO")
				AAdd( aX3Aux, "CP_XPRJVLR")
				AEVal( aX3Aux, {|u| aadd(aStru ,{ u, GetSX3Cache(u, "X3_TIPO"), GetSX3Cache(u, "X3_TAMANHO"), GetSX3Cache(u, "X3_DECIMAL")}) } )
				/*
				aStru := {}
				dbSelectArea("SX3")
...
Static Function F030PCVis(cFilOrig,cNumPC)
	...
	*/
	...
	If !empty(cNumPC)
		...
		cFilAnt := AllTrim(SM0->M0_CODFIL) // @history Ticket TI - 28/02/2023 - Fernando Macieira - Ajustes estabilização pos golive migração dicionário dados
		dbSelectArea("SC7")
		dbSetOrder(1)

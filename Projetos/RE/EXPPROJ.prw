...
/*/{Protheus.doc} User Function EXPPROJ
	...
	@history Everson - 07/02/2023 - Chamado 84724. Incluído item de estudo do pedido de compra.
	@history Ticket 70142 	- Rodrigo Mello | Flek - 14/02/2023 - Substituicao de funcao CriaTrab por FWTemporaryTable MP 12.1.33
/*/
...
Static Function RunCont
	...
	AADD(_aStr,{'FILIAL'     ,"C",02,00})
	AADD(_aStr,{'PEDIDO'     ,"C",06,00})
	//AADD(_aStr,{'PEDATE'   ,"C",01}) &&Chamado 008500 - Inclusao de status para pedidos parcialmente atendidos.
	AADD(_aStr,{'SOLICARMAZ' ,"C",06,00}) // FWNM - CHAMADO 039677-B - 07/05/2019
	AADD(_aStr,{'REQUISICAO' ,"C",06,00})
	AADD(_aStr,{'NFISCAL'    ,"C",09,00})
	AADD(_aStr,{'SERIE'      ,"C",03,00})
	AADD(_aStr,{'DATAEMINF'  ,"C",10,00}) // ADRIANO SAVOINE 26/07/2019 CHAMADO 050635
	AADD(_aStr,{'DATAEMIPC'  ,"C",10,00}) // ADRIANO SAVOINE 26/07/2019 CHAMADO 050635
	AADD(_aStr,{'ITEM'       ,"C",04,00})
	AADD(_aStr,{'CCUSTO'     ,"C",45,00})
	AADD(_aStr,{'CNPJFORNEC' ,"C",14,00}) //WILLIAM COSTA 29/06/2018 CHAMADO 042341
	AADD(_aStr,{'NOME'       ,"C",50,00})
	AADD(_aStr,{'PRODUTO'    ,"C",15,00})                                                                    
	AADD(_aStr,{'DESCRI'     ,"C",40,00})
	AADD(_aStr,{'VALOR'      ,"N",17,02})
	AADD(_aStr,{'CNTPED'     ,"C",20,00}) //WILLIAM COSTA 29/06/2018 CHAMADO 042341
	AADD(_aStr,{'DESCCONTA1' ,"C",60,00}) //WILLIAM COSTA 29/06/2018 CHAMADO 042341
	AADD(_aStr,{'CNTCONT'    ,"C",20,00}) //WILLIAM COSTA 29/06/2018 CHAMADO 042341
	AADD(_aStr,{'DESCCONTA2' ,"C",60,00}) //WILLIAM COSTA 29/06/2018 CHAMADO 042341
	AADD(_aStr,{'VALORCTB'   ,"N",17,02}) //WILLIAM COSTA 29/06/2018 CHAMADO 042341
	AADD(_aStr,{'ENCERRADO'  ,"C",24,00})
	AADD(_aStr,{'DTDIGNF'    ,"C",10,00})  &&Chamado 005696 - Mauricio.
	AADD(_aStr,{'VALORNF'    ,"N",17,02}) &&Chamado 007139 - Mauricio.
	AADD(_aStr,{'NFDEVOL'    ,"C",09,00})  &&Chamado 007452 - Mauricio - inicio
	//AADD(_aStr,{'SERDEV'   ,"C",03})
	AADD(_aStr,{'VLRDEV'     ,"N",17,02})
	AADD(_aStr,{'DTNFDEV'    ,"C",10,00})  &&Chamado 007452 - Mauricio - fim
	AADD(_aStr,{'OBSERVACA'  ,"C",50,00})  && Chamado
	AADD(_aStr,{'REVESTRUT'  ,"C",TamSX3("C1_REVISAO")[1],00})  //Everson - 24/08/2016. Chamado 030283.
	AADD(_aStr,{'DADOSINV'   ,"C",TamSX3("C1_XDDINVE")[1],00})  //Everson - 03/02/2017. Chamado 033095.
	AADD(_aStr,{'DTLANC'     ,"C",10,00})  //Adriana - 16/10/2017 Chamado 037637
	AADD(_aStr,{'GRUPOPRJ'   ,"C",10,00}) // ADRIANO SAVOINE - 02/01/2020 CHAMADO 054383
	AADD(_aStr,{'GRUPDESC'   ,"C",53,00}) // ADRIANO SAVOINE - 02/01/2020 CHAMADO 054383
	AADD(_aStr,{'PROJETOPED' ,"C",53,00})
	AADD(_aStr,{'PROJETONOT' ,"C",53,00})
	AADD(_aStr,{'MESEXTENSO' ,"C",11,00})

	AADD(_aStr,{'ITEMESTUDO' ,"C",10,00}) //Everson - 02/02/2023 - ticket 84724.

	// @history Ticket 70142 	- Rodrigo Mello | Flek - 14/02/2023 - Substituicao de funcao CriaTrab por FWTemporaryTable MP 12.1.33
	oTrb := FWTemporaryTable():New("PROJ", _aStr)
	oTrb:AddIndex("IDX001", {"PEDIDO","ITEM"} )
	oTrb:Create()
	...
	*/
	...
	Do While PSD1->( !EOF() )
		...
		cItemEstudo		:= "" //Everson - 07/02/2023 - ticket 84724.
		...
		If !Empty(_cPEDIDO)
			...
			If SC7->( dbSeek(xFilial("SC7")+_cPEDIDO+_cITEM) )
				...
				cItemEstudo		:= SC7->C7_XITEMST //Everson - 07/02/2023 - ticket 84724.
	...
	//	PROJ->SERDEV       := _cSEDEV
		...
		PROJ->ITEMESTUDO   := cItemEstudo //Everson - 07/02/2023 - ticket 84724.
...
Static Function GeraXLS()
	...
	oExcel:AddColumn(cArqXLS,cTitulo,"ITEMESTUDO"     ,1,1) // 36 //Everson - 07/02/2023 - ticket 84724.
    ...
    Do While PROJ->( !EOF() )
    	...
    	nLinha++
	   	...
	   	aAdd(aLinhas,{ "", ; // 01 A  
	   	               ...
	   	               "", ; // 08 H   
	   	               "", ; // 08 H   
	   	               "" ; //Everson - 07/02/2023 - ticket 84724.
	   	                   })
		...
		aLinhas[nLinha][36] := PROJ->ITEMESTUDO      //Everson - 07/02/2023 - ticket 84724.
	...
	For nExcel := 1 to nLinha
       	oExcel:AddRow(cArqXLS,cTitulo,{aLinhas[nExcel][01],; // 01 A  
	                                     ...
	                                     aLinhas[nExcel][36] ; //Everson - 07/02/2023 - ticket 84724.

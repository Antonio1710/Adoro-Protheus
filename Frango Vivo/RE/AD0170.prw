...
Static Function RunReport(Cabec1,Cabec2,Titulo,nLin)
	...
	AADD(DZV,{'TERCEIR'	 ,"C",01,00})
	AADD(DZV,{'DATAAB'	 ,"C",08,00})
	AADD(DZV,{'PLACA'	 ,"C",07,00})
	AADD(DZV,{'ORDEM'	 ,"C",06,00})
	AADD(DZV,{'FORNECE'	 ,"C",40,00})
	AADD(DZV,{'CIDADE'	 ,"C",40,00})
	AADD(DZV,{'TRANSPOR' ,"C",30,00})
	AADD(DZV,{'INTEGRA'	 ,"C",06,00})
	AADD(DZV,{'RETORNO'	 ,"C",01,00})
	AADD(DZV,{'APANHA'	 ,"C",01,00})
	AADD(DZV,{'PESOL'	 ,"N",13,00})
	AADD(DZV,{'KMSAI'	 ,"N",08,00})			//alterado por Adriana em 18/08/16 - chamado 030187
	AADD(DZV,{'KMENT'	 ,"N",08,00})		 	//alterado por Adriana em 18/08/16 - chamado 030187
	AADD(DZV,{'RANGE'	 ,"N",08,00})			//alterado por Sigoli em      22/12/16 - chamado 032044	
	AADD(DZV,{'DISTABT'	 ,"N",06,00})
	AADD(DZV,{'VFRETE'	 ,"N",08,02})
	AADD(DZV,{'AVCARR'	 ,"N",13,00})
	AADD(DZV,{'MORTAL'	 ,"N",13,00})
	AADD(DZV,{'PERDA'	 ,"N",10,02})
	AADD(DZV,{'HORAAB'	 ,"C",05,00})
	AADD(DZV,{'CLIMAV'	 ,"C",01,00})
	...
	oTrb := FWTemporaryTable():New(cAlias, DZV)
	...
	CASE _nOrdem=4	//Granja          
			aIndex:= {"DATAAB","INTEGRA"}
	OTHERWISE
			aIndex:= {"DATAAB","ORDEM","INTEGRA","PLACA"}
	ENDCASE	

	oTrb:AddIndex("IDX001", aIndex )
	oTrb:Create()
	// SETREGUA -> Indica quantos registros serao processados para a regua 

	//indRegua(cAlias,cArqTmp,cIndex,,,"Criando Indices...")
	...
	While FRT->(!EOF())
		...
		RecLock(cAlias,.T.)
			...
			(cAlias)->TERCEIR  := _cMark
			(cAlias)->DATAAB   := _dDtaBat
			(cAlias)->PLACA    := _cPlaca
			(cAlias)->ORDEM    := _cOrdem
			(cAlias)->FORNECE  := _cNOMFOR
			(cAlias)->CIDADE   := _cCidade
			(cAlias)->TRANSPOR := _cNoTrans
			(cAlias)->INTEGRA  := _cRGRANJ
			(cAlias)->RETORNO  := _cRetorno
			(cAlias)->APANHA   := _cApanh
			(cAlias)->PESOL    := _nPesoL
			(cAlias)->KMSAI    := _nKmSai
			(cAlias)->KMENT    := _nKMENT
			(cAlias)->RANGE    := _nRange
			(cAlias)->DISTABT  := _nDistABT
			(cAlias)->VFRETE   := _nValFrt
			(cAlias)->AVCARR   := _nAves
			(cAlias)->MORTAL   := _nMortal
			(cAlias)->PERDA    := _nPercent
			(cAlias)->CLIMAV   := _cMarkS_M
	...
	(cAlias)->(dbgotop())  //descomentei esta linha em 16/06, e o relatorio voltou a funcionar com os logs habilitados - HC
	...
	while (cAlias)->(!Eof())
		...
		_dDatEntr:=  SUBSTR ((cAlias)->DATAAB,7,2)+ "/" + SUBSTR ((cAlias)->DATAAB,5,2) +"/"+ SUBSTR ((cAlias)->DATAAB,1,4)
		
		if _lsint==1

			@ nlin,000 PSAY (cAlias)->TERCEIR
			@ nlin,002 PSAY _dDatEntr
			@ nlin,014 PSAY (cAlias)->PLACA
			@ nlin,023 PSAY (cAlias)->ORDEM
			IF (_lAnKM=1)
				@ nlin,031 PSAY substr((cAlias)->FORNECE,1,30)
			ELSE
				@ nlin,031 PSAY substr((cAlias)->CIDADE,1,30)
			ENDIF
			@ nlin,064 PSAY (cAlias)->TRANSPOR
			@ nlin,097 PSAY (cAlias)->INTEGRA
			@ nlin,107 PSAY (cAlias)->RETORNO
			@ nlin,112 PSAY (cAlias)->APANHA
			@ nlin,115 PSAY (cAlias)->CLIMAV
			@ nlin,118 PSAY (cAlias)->PESOL PICTURE '@E 999999'
			@ nlin,128 PSAY (cAlias)->KMSAI
			@ nlin,139 PSAY (cAlias)->KMENT
			@ nlin,149 PSAY (cAlias)->RANGE
			@ nlin,158 PSAY (cAlias)->DISTABT
			@ nlin,171 PSAY (cAlias)->VFRETE		
			@ nlin,182 PSAY (cAlias)->AVCARR
			@ nlin,196 PSAY (cAlias)->MORTAL
			@ nlin,203 PSAY (cAlias)->PERDA PICTURE '@E 999.99'
			nlin++
		...
		TPESOL += (cAlias)->PESOL                  //TOTAL PESO L
		TKMS   += (cAlias)->KMSAI					//TOTAL KM SAI
		TKMR   += (cAlias)->KMENT					//TOTAL KM ENT
		TKMT   += (cAlias)->RANGE					//TOTAL KM TOTAL
		TKMTB  += (cAlias)->DISTABT				//TOTAL KM TABELA
		TVFRT  += (cAlias)->VFRETE					//TOTAL VALOR FRETE
		TAVR   += (cAlias)->AVCARR					//TOTAL DE aVES RECEBIDAS
		TAVM   += (cAlias)->MORTAL                 //Total de Aves Mortas
		TPORC  += (cAlias)->PERDA                  //TOTAL PORCENTAGEM DAS PERDAS

		(cAlias)->(dbskip())
		
		//Quebrando por data o total
		if (_DtaF<>(cAlias)->DATAAB)
...
Static Function ProcTrApFt()//Processa Terceiros ; Apanha ; Frete
	...
	IF SA2->(DBSEEK(xFilial("SA2")+_cCodFor+_LojFor,.T.))

		// Se flag de Odometro estiver sim considerar Km do Odometro
		IF _cKmOdm = 'S'
			_nDistAbt:= _nKMENT - _nKMSAI        //DISTANCIA DO ABATEDOURO
		Else
			_nDistAbt	:= SA2->A2_KMABT	  //DISTANCIA DO ABATEDOURO
		Endif
		_cNOMFOR := SA2->A2_NOME	  //NOME DO FORNECEDOR
...
Static Function ProcCalc () //Processa Calculos
	...
	dbselectarea("SZK")
	SZK->(DbsetOrder(2))
	If SZK->(DbSeek(xFilial("SZK") + _nGuia + _dDtaBat + _cPLACA,.T.))
...
Static Function GetData() //Adquire dados
	...
	DbSelectArea("ZV4")
	ZV4->(DbSetOrder(1))
	IF ZV4->(DBSEEK(xFilial("ZV4")+_cPLACA,.T.))

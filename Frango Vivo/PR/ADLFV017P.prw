...
/*/{Protheus.doc} User Function U_ADLFV017P()
  ...
  @history Ticket 78331 - ADRIANO SAVOINE      - 18/08/2022 - Correção para o campo ZV1_PPESOT gravar valor.
  @history Ticket 78737 - ADRIANO SAVOINE      - 24/08/2022 - AJUSTE CAMPO PESO ZV1_PPESOT.
  @history Ticket 82009 - ADRIANO SAVOINE      - 20/10/2022 - AJUSTADO O FONTE PARA FECHAR ORDENS DE ITAPIRA.
  @history Ticket 88981 - Adriano Savoine	   - 20/03/2023 - Ajustado o Campo para alterarem o numero do Galpao na tela.
/*/
...
Static Function BrowseDef()
	...
	oBrowse:AddLegend("(EMPTY(ZV1_FECHA) .OR. ZV1_FECHA = '1')"	, "BR_BRANCO"	 , "ABERTO", "2")
...
/*/{Protheus.doc} User Function ADLFV171C
    ...
    Fecha ordem de carregamento apos o fechamento da ordem 1 - Aberto, 2 - Fechado Automático, 3 - Fechado Manual.
...
User Function ADLFV171()
	...
	IF Alltrim(cEmpAnt)+Alltrim(cFilAnt) $ cEmpFL1  //ticket TI - William Costa - 23/05/2023 
		...
		BEGINSQL ALIAS "cZV1C"
			...
			(CAST(((QN_PESOBRUTENTRAVEVIVA - ZV4.ZV4_PESO) / QN_CABEENTRAVEVIVA) AS DECIMAL(10,2)) * ZV1_PAVES) AS ZV1_PPESOT1
			...
			AND Z.ZV1_FILIAL = %xfilial:ZV1%



		ENDSQL

	ELSEIF cEmpAnt == '01' .AND. cFilAnt == '0B'
		...
		BEGINSQL ALIAS "cZV1C"
			...
			FROM %table:ZV1% Z
				...
				INNER JOIN [LKD_IAAS].[SMART].[dbo].[ENTRADA_AVE_VIVA] EAV ON
    ...
    IF SELECT("cZV1C")>0
        ...
        RecLock("ZV1", .F.)
			...
			ZV1->ZV1_PPESOT := cZV1C->ZV1_PPESOT1
...
Static Function checkOp(cOrdem, cNF, cSerie, cCliNF, cCliSr)
	...
	//Ticket 88981 - 20/03/2023 - Adriano Savoine
	DbSelectArea('ZFC')

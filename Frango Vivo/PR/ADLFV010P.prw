...
/*/{Protheus.doc} User Function ADLFV010P
	...
	@history ticket 80205 - Fernando Macieira - 20/09/2022 - Divergência Pedido Complemento Frango Vivo 931890
	@history ticket 80435 - Fernando Macieira - 22/09/2022 - ERRO AO GERAR COMPLEMENTO ITAPIRA
	@history ticket 80574 - Fernando Macieira - 26/09/2022 - FWCodFil e cFilAnt com problemas de build/lib
	@history ticket 81310 - Fernando Macieira - 06/10/2022 - Erro ao gerar PV complemento
/*/
User Function ADLFV010P()
	...
	Else
		...
		If lOk
			...
			cQuery := " SELECT DISTINCT ZV1_FILIAL
			cQuery += " FROM " + RetSqlName("ZV1") + " (NOLOCK)
			cQuery += " WHERE ZV1_DTABAT LIKE '"+Left(DtoS(msDate()),6)+"%'
			...
			Do While Work->( !EOF() )
				...
				aAdd( aDadAbat, {Work->ZV1_FILIAL} )
				cAbat += Work->ZV1_FILIAL + ","

				Work->( dbSkip() )
			
			EndDo

			If msgYesNo("Serão processados os abatedouros " + cAbat + " confirma?")
			
				For ii:=1 to Len(aDadAbat)

					If !Empty(aDadAbat[ii,1])
						cFilAnt := aDadAbat[ii,1]
					EndIf

					If msgYesNo("Deseja processar abatedouro " + cFilAnt + " ?")
...
Static Function GeraPV(lAuto, cFilGranja)
	...
	//ConfirmSX8(.T.) //@history ticket 80435 - Fernando Macieira - 22/09/2022 - ERRO AO GERAR COMPLEMENTO ITAPIRA
	...
	// @history ticket 81310 - Fernando Macieira - 06/10/2022 - Erro ao gerar PV complemento - Tabela de preço não existe na filial que está sendo gerado o PV
	lDA0 := .T.
	...
	If SE4->( !dbSeek(FWxFilial("SE4")+SA1->A1_COND) )
		lSE4 := .F.
	EndIf

	If !lDA0 	
		Alert("Tabela de preço codigo " + SA1->A1_TABELA + " contida no cliente/loja " + SA1->A1_COD + "/" + SA1->A1_LOJA + " não existe cadastrada na filial " + cFilPV + " ! Verifique com o comercial pois PV não será gerado...")
	ElseIf !lSE4
		Alert("Condição de pagamento codigo " + SA1->A1_COND + " contida no cliente/loja " + SA1->A1_COD + "/" + SA1->A1_LOJA + " não existe cadastrada na filial " + cFilPV + " ! Verifique com o comercial pois PV não será gerado...")
	Else
		...
		BEGIN TRANSACTION
			...
			If lMsErroAuto
				...
				// @history ticket 80205 - Fernando Macieira - 20/09/2022 - Divergência Pedido Complemento Frango Vivo 931890
				u_GrLogZBE(msDate(), TIME(), cUserName, "PV COMPLEMENTO FRANGO VIVO","CONTROLADORIA","ADLFV010P",;
				"ERROR LOG - NAO GEROU PV N " + cNumPV + " NESTA DATA PARA ESTA GRANJA(FILIAL) " + DtoC(msDate()) + " - " + cFilPV, ComputerName(), LogUserName())
				//
				...
				DisarmTransaction()
				//RollBackSxe() // @history ticket 80205 - Fernando Macieira - 20/09/2022 - Divergência Pedido Complemento Frango Vivo 931890
			...
			Else
				...
				If tcSqlExec(cSql) < 0
					...
					Alert("Flag tabela ZV1 não foi gravado! Informe ao TI o erro que será mostrado na próxima tela...")

					MessageBox(tcSqlError(),"",16)

					u_GrLogZBE(msDate(), TIME(), cUserName, "PV COMPLEMENTO FRANGO VIVO","CONTROLADORIA","ADLFV010P",;
					"UPDATE MARCA FLAG ZV1 NAO REALIZADO " + tcSqlError(), ComputerName(), LogUserName())

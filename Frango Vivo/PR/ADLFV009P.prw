...
/*/{Protheus.doc} User Function ADLFV009P
	...
	@history ticket  87651  - Adriano Savoine - 31/01/2023 - Não deixar alterar a ordem se a mesma estiver fechada. 
	@history Ticket  86586  - Adriano Savoine - 09/02/2023 - Retirado o campo ZFC_GALPAO do Recklock de Alteração, pois a alteração do galpão sera feito pela rotina AD0130.
/*/
...
Static Function PROGRAMACAO(COPC)
	...
	Private aFieldsZFC := FWSX3Util():GetAllFields( "ZFC" , .T. )   
	Private nPosCpo := 0

	aHeader:={}

	//@history Ticket 70142  - Rodrigo / Flek Solution - 18/07/2022 - Ajustes Dic. de dados no Banco
	AAdd(aCpos, { "ZFC_VEICUL", "U_VALVEICA()"} )
	AAdd(aCpos, { "ZFC_GAIOLA", "U_009VL1()"} )
	AAdd(aCpos, { "ZFC_AVEGA" , "U_VALAVEGA()"} )
	AAdd(aCpos, { "ZFC_SEQUEN", "U_009VL1()"} )
	AAdd(aCpos, { "ZFC_TOTAL" , "U_VALTOTAL()"} )
	AAdd(aCpos, { "ZFC_PESPRE", "U_009VL1()"} )
	AAdd(aCpos, { "ZFC_DTAPRE", "U_009VL1()"} )
	AAdd(aCpos, { "ZFC_DTACAR", "U_009VL1()"} )
	AAdd(aCpos, { "ZFC_HRAPRE", "U_HORACAR()"} )
	AAdd(aCpos, { "ZFC_TIPVIA", "U_VALVEICB()"} )
	AAdd(aCpos, { "ZFC_EQUIPE", "U_VALEQUIPE()"} )
	AAdd(aCpos, { "ZFC_LACRES", "U_VALLACRE()"} )
	AAdd(aCpos, { "ZFC_HRCHEG", "U_009VL1()"} )
	AAdd(aCpos, { "ZFC_OBSERV", "U_009VL1()"} )
	AAdd(aCpos, { "ZFC_FRTVLR", "U_009VL1()"} )
	AAdd(aCpos, { "ZFC_FRTPED", "U_009VL1()"} )
	AAdd(aCpos, { "ZFC_FRTTOT", "U_009VL1()"} )
	AAdd(aCpos, { "ZFC_NUMERO", "U_009VL1()"} )
	AAdd(aCpos, { "ZFC_NF"    , "U_009VL2(M->ZFC_NF)"} )
	AAdd(aCpos, { "ZFC_SERIE" , ""} )

	For nI := 1 To Len(aFieldsZFC)
		if (nPosCpo := AScan(aCpos,{|u| alltrim(u[1]) == alltrim(aFieldsZFC[nI]) } )) > 0
			AADD(aHeader,{	GetSX3Cache(aCpos[nPosCpo,1], "X3_TITULO") 	,;
							GetSX3Cache(aCpos[nPosCpo,1], "X3_CAMPO")  	,;
							GetSX3Cache(aCpos[nPosCpo,1], "X3_PICTURE")	,;
							GetSX3Cache(aCpos[nPosCpo,1], "X3_TAMANHO")	,; 
							GetSX3Cache(aCpos[nPosCpo,1], "X3_DECIMAL")	,; 
							aCpos[nPosCpo,2]							,;
							GetSX3Cache(aCpos[nPosCpo,1], "X3_USADO")	,; 
							GetSX3Cache(aCpos[nPosCpo,1], "X3_TIPO")	,; 
							GetSX3Cache(aCpos[nPosCpo,1], "X3_ARQUIVO")	,;
							GetSX3Cache(aCpos[nPosCpo,1], "X3_CONTEXT")	})
		endif
	Next
	...
	*/
	...
	aHeader := FWVetByDic( aHeader	, "ZFC", .F.)

	nPosTota      	:= aScan(aHeader,{|x| alltrim(x[2])=="ZFC_TOTAL" })
	nPosVeic		:= aScan(aHeader,{|x| alltrim(x[2])=="ZFC_VEICUL"})
	nPosGaio		:= aScan(aHeader,{|x| alltrim(x[2])=="ZFC_GAIOLA"})
	nPosAves		:= aScan(aHeader,{|x| alltrim(x[2])=="ZFC_AVEGA" })
	nPosSequ		:= aScan(aHeader,{|x| alltrim(x[2])=="ZFC_SEQUEN"})
	nPosPsPr		:= aScan(aHeader,{|x| alltrim(x[2])=="ZFC_PESPRE"})
	nPosDtPr		:= aScan(aHeader,{|x| alltrim(x[2])=="ZFC_DTAPRE"}) 
	nPosDtca		:= aScan(aHeader,{|x| alltrim(x[2])=="ZFC_DTACAR"}) 
	nPosHrPr		:= aScan(aHeader,{|x| alltrim(x[2])=="ZFC_HRAPRE"}) 
	nPosTpVa		:= aScan(aHeader,{|x| alltrim(x[2])=="ZFC_TIPVIA"}) 
	nPosEqui		:= aScan(aHeader,{|x| alltrim(x[2])=="ZFC_EQUIPE"}) 
	nPoslacr		:= aScan(aHeader,{|x| alltrim(x[2])=="ZFC_LACRES"}) 
	nPosHrCh		:= aScan(aHeader,{|x| alltrim(x[2])=="ZFC_HRCHEG"})
	nPosObse		:= aScan(aHeader,{|x| alltrim(x[2])=="ZFC_OBSERV"})
	nPosFrtVlr      := aScan(aHeader,{|x| alltrim(x[2])=="ZFC_FRTVLR"})
	nPosFrtPed      := aScan(aHeader,{|x| alltrim(x[2])=="ZFC_FRTPED"})
	nPosFrtTot      := aScan(aHeader,{|x| alltrim(x[2])=="ZFC_FRTTOT"})
	nPosNume      	:= aScan(aHeader,{|x| alltrim(x[2])=="ZFC_NUMERO"})
	nPosNF      	:= aScan(aHeader,{|x| alltrim(x[2])=="ZFC_NF"	 })     //Everson - 10/12/2019 - Chamado 029058.
	nPosSer      	:= aScan(aHeader,{|x| alltrim(x[2])=="ZFC_SERIE" })  //Everson - 10/12/2019 - Chamado 029058.
	...
	Else
		...
		aOrig:=Array(1,len(aHeader)+1)

		for nI:=1 to len(aHeader)
			If X3USO(GetSX3Cache(aHeader[nI,2], "X3_USADO")) .AND. cNivel >= GetSX3Cache(aHeader[nI,2], "X3_NIVEL")
				nUsado:=nUsado+1
				
				If nOpcx == 3
					
					If GetSX3Cache(aHeader[nI,2], "X3_TIPO") == "C"
						Conout(aHeader[nI,2])
						aCOLS[1][nUsado] := Space(TamSX3(aHeader[nI,2])[1])
					ElseIf GetSX3Cache(aHeader[nI,2], "X3_TIPO") == "N"
						aCOLS[1][nUsado] := 0
					ElseIf GetSX3Cache(aHeader[nI,2], "X3_TIPO") == "D"
						aCOLS[1][nUsado] := Iif(Alltrim(aHeader[nI,2])== "ZFC_DTACAR",dDtCarr,dDtPrev)
					ElseIf GetSX3Cache(aHeader[nI,2], "X3_TIPO") == "M"
						aCOLS[1][nUsado] := ""
					Else
						aCOLS[1][nUsado] := .F.
					EndIf
	...
	If lRetMod2 .and. COPC <> "V"
		...
		For _I := 1 To Len(aCols)
			...
			If	! aCols[_I][len(aCols[_I])] //Verifica se o registro nao esta deletado                                         
				...
				If	aCols[_I,nPosTpVa] == "1" .or. aCols[_I,nPosTpVa] == "3"//Primeira viagem ou Retorno       
					...
					If Empty(Alltrim(cValToChar(aCols[_I,nPosNume]))) 
						...
						MsUnlock()
						/*cNum := FWGetSX5( "Z6" )[2][3]
						FWPutSX5(,  "Z6", padl(alltrim(str(val(cNum)+1,6)),6,"0") )	
						lApanha:=.F.*/
					EndIf  
				...
				If	!lErro
					DbSelectArea("ZFC")
					DbSetOrder(4)
					If Empty(Alltrim(aCols[_I,nPosNume])) .And. aCols[_I,Len(aCols[_I])] == .F.
			...
			Else 
				...
				If Empty(cTicket)
					...
					DbSelectArea("ZFC")
					DbSetOrder(4)
					If dbseek(xFilial("ZFC")+aCols[_I,nPosNume]).And. aCols[_I,Len(aCols[_I])] == .T.
		...
		If DbSeek(xFilial("DA3")+cPlaca,.F.)
			...
			If !Eof()
				...
				For _po	:=	1 TO  Len(aCols)
					If	aCols[_po][nPosPlaca] == cPlaca  .and. !aCols[_po][Len(aCols[_po])]
...
User Function 009LINOK
	...
	If lVldNF .And. lRet .And. ! aCols[n,len(aCols[n])]

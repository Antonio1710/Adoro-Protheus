...
/*/{Protheus.doc} User Function ExprdNet
	...
	@history ticket 83237 - 10/11/2022 - Antonio Domingos - Ajuste da função copy to - erro enviar clientes roadnet
	@history ticket 89893 - 15/03/2022 - Antonio Domingos - Exportação Roadnet
	@history ticket 90211 - 15/03/2022 - ADRIANO SAVOINE  - Error Log.
	@history ticket 89893 - 15/03/2022 - Antonio Domingos - retorno da linha c:\roadnet Exportação Roadnet
/*/
User Function EXPRDNET()
	...
	//ticket 89893 - 15/03/2022 - Antonio Domingos
...
Static Function TxtRdNet( lChk1, lChk2, lChk3, lChk4, lChk5 )
	...
	If lChk1  
		...
		cMensag := cMensag + Chr(13) + ' - Exportou Cadastro de Clientes para: '+cPath+'cli_rdnet.txt' + Chr(13) + Chr(13) //ticket 89893 - 15/03/2022 - Antonio Domingos
	Endif
	If lChk2	
		pergunte("GERABO",.T.)
		MsgRun("Aguarde, Exportando Pedidos para  "+cPath+"...","ROADNET",{|| CursorWait(), ExpPedid(), CursorArrow()}) //ticket 89893 - 15/03/2022 - Antonio Domingos
	Endif
	If lChk3
		MsgRun("Aguarde, Exportando Cadastro de Veículos ...","ROADNET",{|| CursorWait(), ExpVeic(), CursorArrow()})
		cMensag := cMensag + Chr(13) + ' - Exportou Cadastro de Veículos para: '+cPath+'vei_rdnet.txt' + Chr(13) + Chr(13) //ticket 89893 - 15/03/2022 - Antonio Domingos
	Endif
	If lChk4
		MsgRun("Aguarde, Exportando Cadastro de Produtos ...","ROADNET",{|| CursorWait(), ExpProdu(), CursorArrow()})
		cMensag := cMensag + Chr(13) + ' - Exportou Cadastro de Produtos para: '+cPath+'prd_rdnet.txt' + Chr(13) + Chr(13) //ticket 89893 - 15/03/2022 - Antonio Domingos
	Endif
	If lChk5
		pergunte("GERABP",.T.)
		MsgRun("Aguarde, Exportando Pedidos por TES para "+cPath+"...","ROADNET",{|| CursorWait(), ExpPedi1(), CursorArrow()}) //ticket 89893 - 15/03/2022 - Antonio Domingos
	Endif
...
Local cQuery  
Local _cArquivo := cPath+"cli_rdnet.txt" //ticket 83237 - 10/11/2022 - Antonio Domingos
	  _cRoteiI  := MV_PAR01
	  _cRoteiF  := MV_PAR02
	  _dDtaIni  := MV_PAR03
	  _dDtaFim  := MV_PAR04
	...
	Delete File  cPath+'cli_rdnet.txt' //ticket 89893 - 15/03/2022 - Antonio Domingos
	...
	DbSelectArea("FRT")
	Dbgotop()
    COPY To &_cArquivo SDF //ticket 83237 - 10/11/2022 - Antonio Domingos
    FRT->(DbCloseArea()) //ticket 83237 - 10/11/2022 - Antonio Domingos
    _cRoteiI  := ''
	_cRoteiF  := ''
	_dDtaIni  := ''
	_dDtaFim  := ''
...
Local cQuer1, _cRoteiI, _cRoteiF, _dDtaIni, _dDtaFim, _cCaminho
	...
	_cCaminho := cpath+"ped" + DTOS(DDATABASE) + LEFT(TIME(),2)  + SUBSTR(TIME(),4,2) + RIGHT(TIME(),2) + ".txt" //ticket 89893 - 15/03/2022 - Antonio Domingos
    ...
    FRU->(DbCloseArea()) //ticket 83237 - 10/11/2022 - Antonio Domingos
...
Local cQuer2
Local _cArquivo := cPath+"vei_rdnet.txt" //ticket 83237 - 10/11/2022 - Antonio Domingos	
	...
	Delete File  cPath+'vei_rdnet.txt' //ticket 89893 - 15/03/2022 - Antonio Domingos
	...
	Dbgotop()
    COPY To &_cArquivo SDF //ticket 83237 - 10/11/2022 - Antonio Domingos
    FRV->(DbCloseArea()) //ticket 83237 - 10/11/2022 - Antonio Domingos
...
Static Function ExpProdu()
	...
	Local cQuer3
	Local _cArquivo := cPath+"prd_rdnet.txt" //ticket 89893 - 15/03/2022 - Antonio Domingos
	...
	Delete File cPath+_cArq //ticket 83237 - 10/11/2022 - Antonio Domingos
	...
	Dbgotop()
    COPY To &_cArquivo SDF //ticket 83237 - 10/11/2022 - Antonio Domingos
    FRX->(DbCloseArea()) //ticket 83237 - 10/11/2022 - Antonio Domingos
...
Local cQuer4, _cRoteiI, _cRoteiF, _dDtaIni, _dDtaFim, _cCaminh1
	...
	_cTes_In  := MV_PAR05
	_cTes_Fn  := MV_PAR06
	_cCaminh1 :=  cPath+"ped" + DTOS(DDATABASE) + LEFT(TIME(),2)  + SUBSTR(TIME(),4,2) + RIGHT(TIME(),2) + ".txt" //ticket 89893 - 15/03/2022 - Antonio Domingos
    ...
    FRY->(DbCloseArea()) //ticket 83237 - 10/11/2022 - Antonio Domingos

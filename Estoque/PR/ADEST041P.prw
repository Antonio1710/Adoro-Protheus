...
/*/{Protheus.doc} User Function ADEST041P
	...
	@history ticket 85390 - Antonio - 06/02/2023 -Valid fontes v33-dic.bco.       
/*/
...
Static Function CriaSaldo()
	...
	aHeader:={}  
	aCpos := {'DA_PRODUTO','DA_LOCAL','DB_QUANT','DB_LOCALIZ','DB_QTSEGUM','DA_LOTECTL','DA_NUMLOTE','DB_NUMSERI','B8_DTVALID'}
	For x:=1 To Len(aCpos)
		Aadd(aHeader,{AllTrim(GetSX3Cache(aCpos[x], 'X3_TITULO')),;
					GetSX3Cache(aCpos[x], 'X3_CAMPO'),;
					GetSX3Cache(aCpos[x], 'X3_PICTURE'),;
					GetSX3Cache(aCpos[x], 'X3_TAMANHO'),;
					GetSX3Cache(aCpos[x], 'X3_DECIMAL'),;
					GetSX3Cache(aCpos[x], 'X3_VALID'),;
					GetSX3Cache(aCpos[x], 'X3_USADO'),;
					GetSX3Cache(aCpos[x], 'X3_TIPO'),;
					GetSX3Cache(aCpos[x], 'X3_ARQUIVO'),; //@history ticket 85390 - Antonio - 07/02/2023
					GetSX3Cache(aCpos[x], 'X3_CONTEXT') })
	Next

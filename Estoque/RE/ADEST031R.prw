...
Static cTitulo      := "XLS de Projetos de Investimentos"

/*/{Protheus.doc} User Function nomeFunction
	Exporta para txt massa de dados relativos ao controle de Centro de custo(Requisicoes e Pedidos de Compras)
	@type  Function
...
/*/
User Function ADEST031R() 

	Local _cRet := ""
    Private oProj //ticket 85390 - Antonio - 25/01/2023
	
	U_ADINF009P(SUBSTRING(ALLTRIM(PROCNAME()),3,LEN(ALLTRIM(PROCNAME()))) + '.PRW',SUBSTRING(ALLTRIM(PROCNAME()),3,LEN(ALLTRIM(PROCNAME()))),'Exporta para txt massa de dados relativos ao controle de Centro de custo(Requisicoes e Pedidos de Compras)')
	...
	Private oGeraTxt
	Private _cPerg := "ADEST031R"
	Private _cArqTmp := "EXPPROJ" //ticket 85390 - Antonio - 25/01/2023

	Private _cDtLanc := ""
	
	//@history Ticket 70142  - Edvar / Flek Solution - 06/04/2022 - Retirada de função PUTSX1
	//u_xPutSx1(_cPerg,"01","Filial de           ?" , "Filial de           ?" , "Filial de           ?" , "mv_ch1","C",2 ,0,0,"G","","SM0","","","mv_par01","","","","","","","","","","","","","","","","")
	//u_xPutSx1(_cPerg,"02","Filial ate          ?" , "Filial ate          ?" , "Filial ate          ?" , "mv_ch2","C",2 ,0,0,"G","","SM0","","","mv_par02","","","","","","","","","","","","","","","","")
	//u_xPutSx1(_cPerg,"03","Da digitação        ?" , "Da digitação          ?" , "Da digitação          ?" , "mv_ch3","D",8 ,0,0,"G","","   ","","","mv_par03","","","","","","","","","","","","","","","","")
	//u_xPutSx1(_cPerg,"04","Ate digitação       ?" , "Ate digitação         ?" , "Ate digitação         ?" , "mv_ch4","D",8 ,0,0,"G","","   ","","","mv_par04","","","","","","","","","","","","","","","","")
	//u_xPutSx1(_cPerg,"05","Diretório de saida  ?" , "Diretório de saida    ?" , "Diretório de saida    ?" , "mv_ch5","C",50,0,0,"G","","   ","","","mv_par05","","","","","","","","","","","","","","","","")
	//u_xPutSx1(_cPerg,"06","Centro de Custo de  ?" , "Centro de Custo de  ?" , "Centro de Custo de  ?" , "mv_ch6","C",4 ,0,0,"G","","   ","","","mv_par06","","","","","","","","","","","","","","","","")
	//u_xPutSx1(_cPerg,"07","Centro de Custo ate ?" , "Centro de Custo ate ?" , "Centro de Custo ate ?" , "mv_ch7","C",4 ,0,0,"G","","   ","","","mv_par07","","","","","","","","","","","","","","","","")
	pergunte(_cPerg,.T.)
	...
	If Left(AllTrim(mv_par06),1) == "9" .OR. EMPTY(MV_PAR06) .OR. ISALPHA(MV_PAR06) 
		MsgAlert("Não pode utilizar centro de custo iniciados por 9 ou em branco existe outro relatório para Centro de Custo de Projetos EXPPROJ! Verifique os parâmetros...","ADEST031R-01 - Centro de Custo Inválido")
		Return
	EndIf
	
	// FWNM - 23/03/2018 - CENTRO DE CUSTO ATE
	If Left(AllTrim(mv_par07),1) == "9" .OR. EMPTY(MV_PAR07) .OR. ISALPHA(MV_PAR07) 
		MsgAlert("Não pode utilizar centro de custo iniciados por 9 ou em branco existe outro relatório para Centro de Custo de Projetos EXPPROJ! Verifique os parâmetros...","ADEST031R-02 - Centro de Custo Inválido")
		Return
	EndIf
	
	IF VAL(mv_par07) - VAL(mv_par06) > 100
		MsgAlert("Só pode utilizar o de Ate para 100 Centro de Custos! Verifique os parâmetros...","ADEST031R-03 - Centro de Custo Inválido")
		Return
	EndIf
	...
	@ 66,128 BMPBUTTON TYPE 01 ACTION OkGeraTxt()
	@ 66,158 BMPBUTTON TYPE 02 ACTION Close(oGeraTxt)
...
Static Function RunCont
	...
	AADD(_aStr,{'FILIAL'     ,"C",02,0})
	AADD(_aStr,{'PEDIDO'     ,"C",06,0})
	//AADD(_aStr,{'PEDATE'   ,"C",01,0}) &&Chamado 008500 - Inclusao de status para pedidos parcialmente atendidos.
	AADD(_aStr,{'SOLICARMAZ' ,"C",06,0}) // FWNM - CHAMADO 039677-B - 07/05/2019
	AADD(_aStr,{'REQUISICAO' ,"C",06,0})
	AADD(_aStr,{'NFISCAL'    ,"C",09,0})
	AADD(_aStr,{'SERIE'      ,"C",03,0})
	AADD(_aStr,{'DATAEMISS'  ,"C",10,0})
	AADD(_aStr,{'ITEM'       ,"C",04,0})
	AADD(_aStr,{'CCUSTO'     ,"C",45,0})
	AADD(_aStr,{'CNPJFORNEC' ,"C",14,0}) //WILLIAM COSTA 29/06/2018 CHAMADO 042341
	AADD(_aStr,{'NOME'       ,"C",50,0})
	AADD(_aStr,{'PRODUTO'    ,"C",15,0})                                                                    
	AADD(_aStr,{'DESCRI'     ,"C",40,0})
	AADD(_aStr,{'VALOR'      ,"N",17,02})
	AADD(_aStr,{'CNTPED'     ,"C",20,0}) //WILLIAM COSTA 29/06/2018 CHAMADO 042341
	AADD(_aStr,{'DESCCONTA1' ,"C",60,0}) //WILLIAM COSTA 29/06/2018 CHAMADO 042341
	AADD(_aStr,{'CNTCONT'    ,"C",20,0}) //WILLIAM COSTA 29/06/2018 CHAMADO 042341
	AADD(_aStr,{'DESCCONTA2' ,"C",60,0}) //WILLIAM COSTA 29/06/2018 CHAMADO 042341
	AADD(_aStr,{'VALORCTB'   ,"N",17,02}) //WILLIAM COSTA 29/06/2018 CHAMADO 042341
	AADD(_aStr,{'ENCERRADO'  ,"C",24,0})
	AADD(_aStr,{'DTDIGNF'    ,"C",10,0})  &&Chamado 005696 - Mauricio.
	AADD(_aStr,{'VALORNF'    ,"N",17,02}) &&Chamado 007139 - Mauricio.
	AADD(_aStr,{'NFDEVOL'    ,"C",09,0})  &&Chamado 007452 - Mauricio - inicio
	//AADD(_aStr,{'SERDEV'   ,"C",03,0})
	AADD(_aStr,{'VLRDEV'     ,"N",17,02})
	AADD(_aStr,{'DTNFDEV'    ,"C",10,0})  &&Chamado 007452 - Mauricio - fim
	AADD(_aStr,{'OBSERVACA'  ,"C",50,0})  && Chamado
	AADD(_aStr,{'REVESTRUT'  ,"C",TamSX3("C1_REVISAO")[1],0})  //Everson - 24/08/2016. Chamado 030283.
	AADD(_aStr,{'DADOSINV'   ,"C",TamSX3("C1_XDDINVE")[1],0})  //Everson - 03/02/2017. Chamado 033095.
	AADD(_aStr,{'DTLANC'     ,"C",10,0})  //Adriana - 16/10/2017 Chamado 037637
	AADD(_aStr,{'PROJETOPED' ,"C",53,0})
	AADD(_aStr,{'PROJETONOT' ,"C",53,0})
	AADD(_aStr,{'MESEXTENSO' ,"C",11,0})
	...
	_cQuery += "FROM "
	_cQuery += RetSqlName( "SD3" ) + " SD3 (NOLOCK), "
	...
	*/
	...
	_cQuery1 += "FROM "
	_cQuery1 += RetSqlName( "SF1" ) + " SF1 (NOLOCK), "
	_cQuery1 += RetSqlName( "SD1" ) + " SD1 (NOLOCK), "
	_cQuery1 += RetSqlName( "SB1" ) + " SB1 (NOLOCK), "
	_cQuery1 += RetSqlName( "SA2" ) + " SA2 (NOLOCK) "
	...
	Do While PSD1->( !EOF() )
		...
		_cQuery3 += "FROM "
		_cQuery3 += RetSqlName( "SD2" ) + " SD2 (NOLOCK) "
		...
		RecLock("PROJ",.T.)
			...
			PROJ->NFISCAL    := _cNOTA
			PROJ->SERIE      := _cSER
			PROJ->DATAEMISS  := substr(PSD1->D1_EMISSAO,7,2)+"/"+substr(PSD1->D1_EMISSAO,5,2)+"/"+substr(PSD1->D1_EMISSAO,1,4)
	...
	*/
	...
	//	nTamLin := 430
		...
		*/
		...
		cCpo := "ITEM"
		cLin := Stuff(cLin,43,04,cCpo)
		
		cCpo := PadR("C.CUSTO",45)
		cLin := Stuff(cLin,48,45,cCpo)
		
		cCpo := PadR("COD. FORNEC.",09)
		cLin := Stuff(cLin,94,09,cCpo)
		
		cCpo := PadR("RAZAO SOCIAL",40)
		cLin := Stuff(cLin,104,40,cCpo)
		
		cCpo := "PRODUTO        "
		cLin := Stuff(cLin,145,15,cCpo)
		
		cCpo := PadR("DESCRICAO",40)
		cLin := Stuff(cLin,161,40,cCpo)
		
		cCpo := PadR("VALOR PC",17)
		cLin := Stuff(cLin,201,17,cCpo)
		
		cCpo := PadR("CONTA",20)
		cLin := Stuff(cLin,219,20,cCpo)
		
		cCpo := PadR("STATUS PC",24)
		cLin := Stuff(cLin,240,24,cCpo)
		
		cCpo := "DT.DIG.NF "
		cLin := Stuff(cLin,264,10,cCpo)
		
		cCpo := PadR("VALOR NF",17)
		cLin := Stuff(cLin,275,17,cCpo)
		
		cCpo := "NF DEVOL "
		cLin := Stuff(cLin,293,09,cCpo)
		
		cCpo := "SER"
		cLin := Stuff(cLin,303,03,cCpo)
		
		cCpo := "VALOR DEVOLUCAO  "
		cLin := Stuff(cLin,309,17,cCpo)
		
		cCpo := "DT.NF.DEV."
		cLin := Stuff(cLin,326,10,cCpo)
		
		cCpo := PadR("OBSERVACAO",50)
		cLin := Stuff(cLin,337,50,cCpo)
		
		cCpo := "REV"
		cLin := Stuff(cLin,388,3,cCpo)
		
		cCpo := PadR("DADOS INVENTARIO",60)
		cLin := Stuff(cLin,392,60,cCpo)
		
		cCpo := "DT.LANC.  "
		cLin := Stuff(cLin,453,10,cCpo)
		
		// FWNM - 09/04/2018 - Chamado 039677
		cCpo := PadR("PROJETOPED",100)
		cLin := Stuff(cLin,464,100,cCpo)
		
		// FWNM - 09/04/2018 - Chamado 039677
		cCpo := "MES EXTENSO"
		cLin := Stuff(cLin,565,11,cCpo)
...
Static Function obterRevisao(cFil,cNum,cItem)
	...
	cQuery += " " + RetSqlName("SC7") + " AS SC7 (NOLOCK)"
	cQuery += " INNER JOIN "
	cQuery += " " + RetSqlName("SC1") + " AS SC1 (NOLOCK) ON "
...
Static Function GeraXLS()
	...
	oExcel:AddColumn(cArqXLS,cTitulo,"DATAEMISS"      ,1,1) // 07 G
	...
	oExcel:AddColumn(cArqXLS,cTitulo,"PROJETOPED"     ,1,1) // 30 I
	oExcel:AddColumn(cArqXLS,cTitulo,"PROJETONOT"     ,1,1) // 31 I
	oExcel:AddColumn(cArqXLS,cTitulo,"MESEXTENSO"     ,1,1) // 32 I
    ...
    Do While PROJ->( !EOF() )
		...
		aLinhas[nLinha][07] := PROJ->DATAEMISS          //F
		aLinhas[nLinha][08] := PROJ->ITEM          //F
		aLinhas[nLinha][09] := PROJ->CCUSTO          //F
		aLinhas[nLinha][10] := PROJ->CNPJFORNEC          //F
		aLinhas[nLinha][11] := PROJ->NOME          //F
		aLinhas[nLinha][12] := PROJ->PRODUTO          //F
		aLinhas[nLinha][13] := PROJ->DESCRI          //F
		aLinhas[nLinha][14] := PROJ->VALOR          //F
		aLinhas[nLinha][15] := PROJ->CNTPED          //F
		aLinhas[nLinha][16] := PROJ->DESCCONTA1          //F
		aLinhas[nLinha][17] := PROJ->CNTCONT          //F
		aLinhas[nLinha][18] := PROJ->DESCCONTA2          //F
		aLinhas[nLinha][19] := PROJ->VALORCTB          //F
		aLinhas[nLinha][20] := PROJ->ENCERRADO          //F
		aLinhas[nLinha][21] := PROJ->DTDIGNF          //F
		aLinhas[nLinha][22] := PROJ->VALORNF          //F
		aLinhas[nLinha][23] := PROJ->NFDEVOL          //F
		aLinhas[nLinha][24] := PROJ->VLRDEV          //F
		aLinhas[nLinha][25] := PROJ->DTNFDEV          //F
		aLinhas[nLinha][26] := PROJ->OBSERVACA          //F
		aLinhas[nLinha][27] := PROJ->REVESTRUT          //F
		aLinhas[nLinha][28] := PROJ->DADOSINV          //F
		aLinhas[nLinha][29] := PROJ->DTLANC          //F
		aLinhas[nLinha][30] := PROJ->PROJETOPED          //F
		aLinhas[nLinha][31] := PROJ->PROJETONOT          //F
		aLinhas[nLinha][32] := PROJ->MESEXTENSO          //F

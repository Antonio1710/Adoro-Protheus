...
/*/{Protheus.doc} User Function RCONCCLI
	...
	@history ticket 90133 - 14/03/2023 - Fernando Macieira - CONCILIAÇÃO CLIENTES NACIONAIS X EXPORTAÇÃO
	@history ticket 90133 - 24/03/2023 - Fernando Macieira - Inclusão de parâmetro na pergunta do filtro implantado cliente EX
/*/
...
Static Function RunReport(Cabec1,Cabec2,Titulo,nLin)
	...
	AADD(_aStr1,{'E1_CLIENTE'  ,"C",06,0}) //ticket 85390 - Antonio - 25/01/2023
	AADD(_aStr1,{'E1_LOJA'    ,"C",02,0})  //ticket 85390 - Antonio - 25/01/2023
	AADD(_aStr1,{'E1_VALOR'   ,"N",17,02})  // RICARDO LIMA - 27/11/17 | AJUSTE DE TAMANHO DE CAMPO
	If mv_par08 == 1 //Analitico
		AADD(_aStr1,{'E1_EMIS1'   ,"D",08,0}) //ticket 85390 - Antonio - 25/01/2023
	Endif
	...
	oTmpSe1 := FWTemporaryTable():New("TMPSE1", _aStr1)
	oTmpSe1:AddIndex("01", {"E1_CLIENTE","E1_LOJA"} )
	oTmpSe1:Create()
	...
	If mv_par08 == 1 //Analitico
		AADD(_aStr2,{'CT2_DATA'   ,"D",08,0}) //ticket 85390 - Antonio - 25/01/2023
	Endif
...
Static Function fDadosAnl(Cabec1,Cabec2,Titulo,nLin)
	...
	AADD(_aStr4,{'CLIENTE'  ,"C",06,0}) //ticket 85390 - Antonio - 25/01/2023
	...
	oTmpAnl:AddIndex("01", {"CLIENTE","LOJA","FILIAL","NUMERO","PREFIXO","PARCELA","TIPO"} )
	oTmpAnl:Create()	
	...
	While SE1TMP->(!EOF())
		...
		// @history ticket 90133 - 24/03/2023 - Fernando Macieira - Inclusão de parâmetro na pergunta do filtro implantado cliente EX
		If MV_PAR11 == 1 // Considera Cliente Exportação = Não
			// @history ticket 90133 - 14/03/2023 - Fernando Macieira - CONCILIAÇÃO CLIENTES NACIONAIS X EXPORTAÇÃO
			SA1->( dbSetOrder(1) )
			If SA1->( dbSeek(FWxFilial("SA1")+SE1TMP->E1_CLIENTE+SE1TMP->E1_LOJA) )
				If AllTrim(SA1->A1_TIPO) == "X" .or. AllTrim(SA1->A1_EST) == "EX"
					SE1TMP->( dbSkip() )
					Loop
				EndIf
			EndIf
	...
	While SE5TMP->(!EOF())
		...
		If MV_PAR11 == 1 // Considera Cliente Exportação = Não
			// @history ticket 90133 - 14/03/2023 - Fernando Macieira - CONCILIAÇÃO CLIENTES NACIONAIS X EXPORTAÇÃO
			SA1->( dbSetOrder(1) )
			If SA1->( dbSeek(FWxFilial("SA1")+SE5TMP->E5_CLIFOR+SE5TMP->E5_LOJA) )
				If AllTrim(SA1->A1_TIPO) == "X" .or. AllTrim(SA1->A1_EST) == "EX"
					SE5TMP->( dbSkip() )
					Loop
				EndIf
	...
	While CT2TMP->(!EOF())
		...
		If MV_PAR11 == 1 // Considera Cliente Exportação = Não
			// @history ticket 90133 - 14/03/2023 - Fernando Macieira - CONCILIAÇÃO CLIENTES NACIONAIS X EXPORTAÇÃO
			SA1->( dbSetOrder(1) )
			If SA1->( dbSeek(FWxFilial("SA1")+CT2TMP->CT2_CLIFOR+CT2TMP->CT2_LOJACF) )
				If AllTrim(SA1->A1_TIPO) == "X" .or. AllTrim(SA1->A1_EST) == "EX"
					CT2TMP->( dbSkip() )
					Loop
				EndIf
	...
	While SA1TMP->(!EOF())
		...
		If MV_PAR11 == 1 // Considera Cliente Exportação = Não
			// @history ticket 90133 - 14/03/2023 - Fernando Macieira - CONCILIAÇÃO CLIENTES NACIONAIS X EXPORTAÇÃO
			SA1->( dbSetOrder(1) )
			If SA1->( dbSeek(FWxFilial("SA1")+SA1TMP->A1_COD+SA1TMP->A1_LOJA) )
				If AllTrim(SA1->A1_TIPO) == "X" .or. AllTrim(SA1->A1_EST) == "EX"
					SA1TMP->( dbSkip() )
					Loop
				EndIf

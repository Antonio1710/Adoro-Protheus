...
/*/{Protheus.doc} User Function ADFIN121P
    ...
    @ticket 78488 - Everson - 18/08/2022 - Tratamento para errorlog no job.
    @ticket 79561 - Fernando Macieira - 08/09/2022 - Tratamento para errorlog no job
    @ticket 88109 - 07/02/2023 - Fernando Macieira - Geração de acordo
    @history Ticket TI - 28/02/2023 - Fernando Macieira - Ajustes estabilização pos golive migração dicionário dados
    @history Ticket 91727 - 12/04/2023 - Fernando Macieira - Erro titulos acordos trabalhistas
    @history Ticket 92566 - 25/04/2023 - Fernando Macieira - Erro processos trabalhistas
...
User Function ADFIN121P(lAuto)
    ...
    Local aDadSE2      := {}
    // Local cStatusRM    := ""
    Local dDtBkp       := Nil //Everson - 18/08/2022. Chamado 78488. //dDataBase // @ticket 72340 - Fernando Macieira - 16/05/2022 - RM - Acordos - Inclusao de filial - Baixas futuras
    ...
    Private _cEmpFLE := SuperGetMv("MV_#EMPFLC",.F.,"0102/0103/0701/0901/1301") //Codigos de Empresas+Filiais Ativas Grupo E //ticket TI - Antonio Domingos - 24/05/2023
    Private lSigaOn := .t.
    Private cLinked := "RM"
    ...
    If lAuto
        ...
        rpcClearEnv()
        //rpcSetType(3) // @ticket 79561 - Fernando Macieira - 08/09/2022 - Tratamento para errorlog no job
    ...
    dDtBkp := dDataBase //Everson - 18/08/2022. Chamado 78488.
	...
	SM0->(dbGoTop())
	Do While SM0->(!EOF())
		//If (AllTrim(SM0->M0_CODIGO) $ cEmpRun) .and. (AllTrim(SM0->M0_CODFIL) $ cFilRun) // @history Ticket TI - 28/02/2023 - Fernando Macieira - Ajustes estabilização pos golive migração dicionário dados
		If (AllTrim(SM0->M0_CODIGO)+AllTrim(SM0->M0_CODFIL) $ _cEmpFLE) //ticket TI - Antonio Domingos - 24/05/2023 
    ...
    For i:=1 to Len(aEmpresas)
        ...
        // @history Ticket 92566 - 25/04/2023 - Fernando Macieira - Erro processos trabalhistas - inibida mensagem
        // @history Ticket 91727 - 12/04/2023 - Fernando Macieira - Erro titulos acordos trabalhistas
        /*
        If !lAuto
            Work->( dbGoTop() )
            If Work->( EOF() )
                Alert("Nenhum acordo encontrado com os filtros utilizados! Verifique..." )
                Alert(cQuery)
                Alert(cQuery)
            EndIf
        EndIf
                    ...
                    Do While SE2->( !EOF() ) .and. SE2->E2_FILIAL==FWxFilial("SE2") .and. AllTrim(SE2->E2_PREFIXO)==cPrefixo .and. SE2->E2_NUM==ZHB->ZHB_NUM
                        ...
                        If AllTrim(SE2->E2_TIPO) == cTipoNDI
                            ...
                            If Month(ZHB->ZHB_VENCTO) == 2 .and. ZHB->ZHB_PARCEL > 1 .and. SE2->E2_PARCELA == "001"
                                nSumDias := 28
                                If Val(Right(DtoS(LastDay(ZHB->ZHB_VENCTO)),2)) == 29 // @ticket 88109 - 07/02/2023 - Fernando Macieira - Geração de acordo
                                    nSumDias := 29
                                EndIf
                            EndIf
                            //

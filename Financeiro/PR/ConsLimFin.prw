...
/*/{Protheus.doc} User Function ConsLimFin
	...
	@hisotry Ticket 81594   - Everson  - 18/10/2022 - Log para bloqueio por prazo médio da condição de pagamento
	@hisotry Ticket 85037   - Antonio Domingos  - 20/12/2022 - Ajuste de Filial - BAIXA AUTOMATICA PEDIDOS ANTECIPADO - BOLETOS.
/*/ 
User Function ConsLimFin(cCodCli,_cTpCons,cRotina,_dDTE1,_dDTE2)
	...
	TMPF0->(DbCloseArea()) //ticket 85037 - Antonio Domingos - 20/12/2022
	...
	Else
		...
		Do While !EOF()
			...
			Enddo
			If SELECT("TMPFSA1") > 0
				TMPFSA1->(DbCloseArea()) //ticket 85037 - Antonio Domingos - 20/12/2022
			EndIf
			dbSelectArea("TMPFZ")
			dbSkip()
		Enddo
	...
	Endif
    If SELECT("TMPFZ") > 0
		TMPFZ->(DbCloseArea()) //ticket 85037 - Antonio Domingos - 20/12/2022
	EndIf
	RestArea(_aArea)
...
User Function AtuLCPed(_cPedido)
	...
	DbSetOrder(1)
	If DbSeek(_cPedido)
		While !Eof() .And. SC6->C6_NUM == Substr(_cPedido,3,6); //ticket 85037 - Antonio Domingos - 20/12/2022
					 .And. SC6->C6_FILIAL == Substr(_cPedido,1,2) //ticket 85037 - Antonio Domingos - 20/12/2022
			...
			If DbSeek(_cPedido)
				...
				If dbseek(xFilial("SF4")+SC6->C6_TES)
					If !((ALLTRIM(SF4->F4_DUPLIC) == "S") .and. (ALLTRIM(_Tipo) $ "N/C") .and. (ALLTRIM(_estado)<> "EX"))         
						...
						dbSelectArea("ZBE")
						RecLock("ZBE",.T.)
						Replace ZBE_FILIAL WITH SC5->C5_FILIAL //ticket 85037 - Antonio Domingos - 20/12/2022
						Replace ZBE_DATA   WITH dDataBase
						Replace ZBE_HORA   WITH TIME()
						Replace ZBE_USUARI WITH UPPER(Alltrim(cUserName))
						Replace ZBE_LOG    WITH "FILIAL "+SC5->C5_FILIAL+"PEDIDO " + SC5->C5_NUM +" STATUS: "+SC5->C5_FLAGFIN //ticket 85037 - Antonio Domingos - 20/12/2022
	...
	If DbSeek(_cPedido)
		...
		If _nMedPGPd > _nMedPGA1  &&media da condição de pagto do pedido é maior do que media do cliente, bloqueia, só passa se for igual ou menor.
			...
			u_GrLogZBE(Date(), Time(), cUserName, "Prz med cond pag _nMedPGPd > _nMedPGA1 " + cValToChar(_nMedPGPd) + " " + cValToChar(_nMedPGA1),"FINANCEIRO","CONSLIMFIN",;
	...
	If DbSeek(_cPedido)
		...
		If !Empty(Alltrim(cValToChar(SC5->C5_XWSPAGO))) // @history ticket 71027 - Fernando Macieira - 07/04/2022 - Liberação Pedido Antecipado sem Aprovação Financeiro - PV 9BEGCC foi incluído depois que o job do boleto parou, não gerou FIE e SE1 (PR) e foi liberado manualmente pelo financeiro, sendo faturado como pv normal... por isso da dupla checagem
			...
			//@history Ticket 70142  - Edvar   / Flek Solution - 23/03/2022 - Substituicao de funcao Static Call por User Function MP 12.1.33
			u_200AVLA0(SC5->C5_FILIAL,SC5->C5_NUM) //ticket 85037 - Antonio Domingos - 20/12/2022
		EndIf
		//
	...
	If lBCEntrou
		...
		If AlLtrim(_cStAntPed) <> "B" 
			U_SendMlLC(_cPedido,"BLQ",_cRotina,_eMailVend,_eMailSup) //ticket 85037 - Antonio Domingos - 20/12/2022
		Endif
			
	Else
		U_SendMlLC(_cPedido,"LIB",_cRotina,_eMailVend,_eMailSup) //ticket 85037 - Antonio Domingos - 20/12/2022
	Endif
	If SELECT("TMPFZ") > 0
		TMPFZ->(DbCloseArea()) //ticket 85037 - Antonio Domingos - 20/12/2022
	EndIf

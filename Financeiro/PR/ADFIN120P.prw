...
/*/{Protheus.doc} User Function ADFIN120P
    ...
    @ticket 81943 - Fernando Macieira - 18/10/2022 - RM - Acordos - Geração NDI - Duplicidade de número no execauto
/*/
...
Static Function GeraZC7RM(aDadRM)
    ...
    Local lExistSE2 := .f.
    Local lExistZHB := .f.
    Local cNumero   := NextSE2() // GetSXENUM("RC1","RC1_NUMTIT")   //NextRC1() // @ticket 81943 - Fernando Macieira - 18/10/2022 - RM - Acordos - Geração NDI - Duplicidade de número no execauto
    ...
    If !lExisteP12
        ...
        Do While .t.
            ...
            Else
                ...
                If SE2->( dbSeek(FWxFilial("SE2")+cFornece+cLoja+cPrefixo+cNumero))
                    lExitSE2 := .T.
                    cNumero := Soma1(AllTrim(cNumero))
                    dbSelectArea("RC1")
                    ConfirmSX8() 
            ...
            */
            ...
            If SE2->( dbSeek(FWxFilial("SE2")+cPrefixo+cNumero) )
                cNumero := Soma1(AllTrim(cNumero))
                dbSelectArea("RC1")
                ConfirmSX8()
            Else
        ...
        Begin Transaction 
            ...
            // @ticket 74690 - Fernando Macieira - 14/06/2022 - RM - Acordos - Contabilização título PR não deve ocorrer
            PERGUNTE( "FIN050",.F.) // Inclusão Contas Pagar
...
Static Function GetZCF()
    ...
    If IsInCallStack("u_ADFI118") .or. FunName() == "RPC" // @ticket 76965 - Fernando Macieira - 15/08/2022 - RM - Acordos - Gerar o PR na saída da rotina
...
/*/{Protheus.doc} Static Function NextSE2
    Função para substituir Rc1TitIni() 
    @type  Function
    @author FWNM
    @since 18/10/2022
...
Static Function NextSE2()
    ...
    cQuery := " SELECT MAX(E2_NUM) AS NEXT_COD
    cQuery += " FROM " + RetSqlName("SE2") + " (NOLOCK)
    cQuery += " WHERE E2_PREFIXO='GPE'
    cQuery += " AND D_E_L_E_T_='' 

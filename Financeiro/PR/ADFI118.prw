...
/*/{Protheus.doc} ADFI118
    ...
    @ticket 76965 - Fernando Macieira - 15/08/2022 - RM - Acordos - Gerar o PR na saída da rotina
...
User Function ADFI118()
	...
	//FWMsgRun(, {|| u_ADFIN120P() }, "Aguarde", "Gerando acordos trabalhistas para aprovação ["+Time()+"] ...") // @ticket 76965 - Fernando Macieira - 15/08/2022 - RM - Acordos - Gerar o PR na saída da rotina
    u_ADFIN120P()
    u_Run121P(.f.)
...
Static Function MenuDef()
    ...
    ADD OPTION aRot TITLE '*Alterar dados NDI'       ACTION 'u_ADFI134()'      OPERATION MODEL_OPERATION_UPDATE ACCESS 0 // @ticket 77598 - Fernando Macieira - 24/08/2022 - RM - Acordos - Alteração NDI (CNPJ ou CPF e dados bancários)
...
/*/{Protheus.doc} User Function nomeFunction
    Job - Função para executar no schedule a geração do PR e NDI
    @type  Function
    @author FWNM
    @since 15/08/2022
    @version version
    ...
    @ticket 76965 - Fernando Macieira - 15/08/2022 - RM - Acordos - Gerar o PR na saída da rotina
/*/
User Function JobAcordo()

    Local cEmpRun   := "01"
    Local cFilRun   := "02"

    Private cRotina := "JOBACORDO"
    ...
    u_ADFIN120P() // Função para gerar alçada de aprovação das despesas dos acordos trabalhistas (tipo PR)
    u_Run121P(.t.) // Função para gerar parcelas dos acordos trabalhistas dos acordos trabalhistas (tipo NDI)
...
Static Function logZBN(cStatus)
    ...
    RecLock("ZBN", lLockZBN)
        ...
        ZBN_ROTINA	:= cRotina
        ZBN_DESCRI  := 'JOB ACORDOS - GERA TITULOS PR E NDI'
        ZBN_DATA    := msDate()
        ZBN_HORA    := TIME()
        ZBN_PERIOD  := '1'
        ZBN_PERDES  := 'DIA'
        ZBN_QTDVEZ  := ZBN_QTDVEZ + 1
        ZBN_HORAIN  := Time()
        ZBN_DATAPR  := msDate() + 1
        ZBN_HORAPR  := Time()
        ZBN_STATUS	:= cStatus

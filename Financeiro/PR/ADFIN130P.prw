...
function u_ADFIN130P( aParms, nE1Id, lRpc )
    ...
    Local aEmpresas  := {}
    //
    Local _cEmpFL3 := " " //Codigos de Empresas+Filiais Ativas Grupo 3 //ticket TI - Antonio Domingos - 24/05/2023
    private nIdE1 := 0
    ...
    if lRpc
        ...
        Do While SM0->(!EOF())
            ...
            If (Alltrim(SM0->M0_CODIGO)+Alltrim(SM0->M0_CODFIL) $ _cEmpFL3)
...
static function Execute()
    ...
    while (cAlias)->(!eof())
        ...
        if SE1->E1_XLOGPIX != 'GER'
            ...
            oPayLoad['nomePersonalizacaoQr'] := "AD ORO"
...
static function fLogRes(cCodRes, cLogRes)
    ...
    //T.I -  Fernando Sigoli -  Se o titulo nao conseguir registrar, o job vai fazer tratativa de reenviar
    If cCodRes = 'ERR'

        recLock("SE1", .F.)
        SE1->E1_XMEMPIX := cCodRes+'-'+cLogRes
        SE1->(MsUnLock())

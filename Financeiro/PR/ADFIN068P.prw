...
STATIC FUNCTION GPSITAU()
	...
	While SM0->(!Eof())
		...
		IF AllTrim(SM0->M0_CODIGO) == AllTrim(CEMPANT) .AND. AllTrim(SM0->M0_CODFIL) == AllTrim(SE2->E2_FILIAL) // @history Ticket TI - 28/02/2023 - Fernando Macieira - Ajustes estabilização pos golive migração dicionário dados
			cCgc := SM0->M0_CGC
		ENDIF	

...
/*/{Protheus.doc} User Function ADFIN087P
	...
	https://centraldeatendimento.totvs.com/hc/pt-br/articles/360035250533-Cross-Segmento-TOTVS-Backoffice-Linha-Protheus-ADVPL-Criptografia-PKCS-7-para-enviar-%C3%A0-API-de-cobran%C3%A7a-online-do-Bradesco
	@type  Function
	@author FWNM
	@since 24/04/2020
	@version version
    ...
    For i:=1 to Len(aEmpresas)
		...
		*/
        ...
        Do While Work->( !EOF() )
					...
					If ( Val(AllTrim(SE1->E1_XWSBRAC)) == Val(AllTrim(cCodRetOk)) ) .or. ( Val(AllTrim(SE1->E1_XWSBRAC)) == Val(AllTrim(cCodRet69)) ) 
						...
						//If SC5->( dbSeek(Work->(C5_FILIAL+C5_NUM)) ) .and. !SC5->C5_CONDPAG $ cCondPixLnk // @history ticket 68450   - Fernando Macieira     - 18/02/2022 - Email em duplicidade para o cliente/vendedor
							...
							If !(SC5->C5_CONDPAG $ cCondPixLnk) // @history ticket 68450   - Fernando Macieira     - 18/02/2022 - Email em duplicidade para o cliente/vendedor
								logZBE(SE1->E1_FILIAL,SE1->E1_NUM + " esta acessando funcao GERABLPV para gerar o boleto em PDF e enviar o email") //ticket 85037 - Antonio Domingos - 21/12/2022
								u_GeraBlPV(cEmpAnt, SC5->C5_FILIAL, SC5->C5_NUM) //ticket 85037 - Antonio Domingos - 20/12/2022
								logZBE(SE1->E1_FILIAL,SE1->E1_NUM + " saiu da funcao GERABLPV e se o email foi enviado com sucesso o campo C5_XWSBOLG foi flegado") //ticket 85037 - Antonio Domingos - 21/12/2022
							EndIf
            ...
            logZBE(Work->C5_FILIAL,Work->C5_NUM + " entrou na query C5_BLQ='' C5_XWSPAGO='' C5_XWSBOLG='' E4_CTRADT='1'") //ticket 85037 - Antonio Domingos - 21/12/2022

            If lGeraBol

	            logZBE(Work->C5_FILIAL,Work->C5_NUM + " autorizado pelo status integracao do sales force") //ticket 85037 - Antonio Domingos - 21/12/2022
            ...
            Else
				...
				logZBE(Work->C5_FILIAL,Work->C5_NUM + " nao foi autorizado pelo status integracao sales force") //ticket 85037 - Antonio Domingos - 21/12/2022
...
User Function GeraRAPV()
	...
	If lCondRA .and. !Empty(cBcoSA1)
		...
		*/
		...
		If !lAllTESFin
            logZBE(SC5->C5_FILIAL,SC5->C5_NUM + " possui item com TES F4_DUPLIC='N'" ) //ticket 85037 - Antonio Domingos - 21/12/2022
		...
		If FIE->( dbSeek(SC5->C5_FILIAL+"R"+SC5->C5_NUM) ) .AND. alltrim(SC5->C5_CONDPAG) $ cCondBlt  // @history 07/12/2021 - verifica se meio de pagamento é Boleto | Rodrigo Mello - Flek Solutions
			...
			If SE1->( dbSeek(FIE->(FIE_FILIAL+FIE_PREFIX+FIE_NUM+FIE_PARCEL+FIE_TIPO)) ) 
				...
				If Empty(SE1->E1_NUMBCO)
		            ...
		            logZBE(SE1->E1_FILIAL,SE1->E1_NUM + " gerou E1_NUMBCO pela rotina SF2460I via ROTINA") //ticket 85037 - Antonio Domingos - 21/12/2022
				...
				If Empty(SE1->E1_XWSBRAC) .or. ( Val(AllTrim(SE1->E1_XWSBRAC)) <> Val(AllTrim(cCodRetOk)) .or. Val(AllTrim(SE1->E1_XWSBRAC)) <> Val(AllTrim(cCodRet69)) )
					...
					logZBE(SE1->E1_FILIAL,SE1->E1_NUM + " esta acessando funcao T288BPCK7 para fazer registro do E1_NUMBCO") //ticket 85037 - Antonio Domingos - 21/12/2022
					u_T288BPCK7(SE1->E1_FILIAL, SE1->E1_PREFIXO, SE1->E1_NUM, SE1->E1_PARCELA, SE1->E1_TIPO)
					logZBE(SE1->E1_FILIAL,SE1->E1_NUM + " saiu da funcao T288BPCK7 que faz o registro do E1_NUMBCO") //ticket 85037 - Antonio Domingos - 21/12/2022				
				...
				If Empty(SC5->C5_XWSBOLG)
					...
					// Gera Boleto PDF
					logZBE(SE1->E1_FILIAL,SE1->E1_NUM + " esta acessando funcao GERABLPV para gerar o boleto em PDF e enviar o email") //ticket 85037 - Antonio Domingos - 21/12/2022
					u_GeraBlPV(cEmpAnt, SC5->C5_FILIAL, SC5->C5_NUM)
					logZBE(SE1->E1_FILIAL,SE1->E1_NUM + " saiu da funcao GERABLPV e se o email foi enviado com sucesso o campo C5_XWSBOLG foi flegado") //ticket 85037 - Antonio Domingos - 21/12/2022
		...
		*/
		...
		If SE1->( dbSeek(FWxFilial("SE1")+PadR(cTipoE1,TamSX3("E1_PREFIXO")[1])+PadR(AllTrim(SC5->C5_NUM),TamSX3("E1_NUM")[1])+PadR("",TamSX3("E1_PARCELA")[1])+PadR(cTipoE1,TamSX3("E1_TIPO")[1])) )
			nOpcAuto := 4
			logZBE(SC5->C5_FILIAL,SC5->C5_NUM + " nOpcAuto = 4 para executar o EXECAUTO FINA040 (inclusao/alteracao) titulo tipo PR") //ticket 85037 - Antonio Domingos - 21/12/2022
		EndIf
		//
		...
		logZBE(SC5->C5_FILIAL,SC5->C5_NUM + " EXECAUTO FINA040 foi executado") //ticket 85037 - Antonio Domingos - 21/12/2022
		...
		If lMsErroAuto
			logZBE(SC5->C5_FILIAL,SC5->C5_NUM + " EXECAUTO FINA040 foi executado com erro e o boleto (tipo " + cTipoE1 + ") nao foi gerado") //ticket 85037 - Antonio Domingos - 21/12/2022
		    MostraErro()
			DisarmTransaction()
		Else
		    logZBE(SE1->E1_FILIAL,SE1->E1_NUM + " EXECAUTO FINA040 foi executado com sucesso e o boleto (tipo " + cTipoE1 + ") foi gerado") //ticket 85037 - Antonio Domingos - 21/12/2022
			...
			logZBE(FIE->FIE_FILIAL,FIE->FIE_PEDIDO + " tabela FIE (Amarracao PV x " + cTipoE1 + ") foi gerado com sucesso") //ticket 85037 - Antonio Domingos - 21/12/2022
...
User Function T288BPCK7(_cFilial, _cPrefixo, _cNum, _cParcela, _cTipo)
	...
	*/
	...
	logZBE(SC5->C5_FILIAL,SC5->C5_NUM + " entrou na funcao T288BPCK7 para registrar nosso numero no bradesco via JSON" ) //ticket 85037 - Antonio Domingos - 21/12/2022
	...
	*/
	...
	logZBE(SE1->E1_FILIAL,SE1->E1_NUM + " utilizara endpoint para registro " + cURL ) //ticket 85037 - Antonio Domingos - 21/12/2022

    aCert    := GetCertificate(BAR + cPathCerts, cNomeCerts, cPasswd)
    cSign    := SignJson(aCert, cPasswd)

	logZBE(SE1->E1_FILIAL,SE1->E1_NUM + " passou pela funcao GETCERTIFICATE utilizando senha " + cPasswd + " e pela funcao SIGNJSON retornando " + cSign ) //ticket 85037 - Antonio Domingos - 21/12/2022
	...
	logZBE(SE1->E1_FILIAL,SE1->E1_NUM + " enviou requisicao " + cResp ) //ticket 85037 - Antonio Domingos - 21/12/2022
	...
	*/
	...
	logZBE(SE1->E1_FILIAL,SE1->E1_NUM + " retornou requisicao com motivos " + SE1->E1_XWSBRAC + " - " + AllTrim(SE1->E1_XWSBRAD) + " - " + cResp ) //ticket 85037 - Antonio Domingos - 21/12/2022
...
Static Function GetCertificate(cCertPath As Character, cFileName As Character, cPassword As Character)
	...
	logZBE(SE1->E1_FILIAL,SE1->E1_NUM + " esta na rotina GETCERTIFICATE e o certificado _ca.pem eh " + cFullPath ) //ticket 85037 - Antonio Domingos - 21/12/2022

    // VERIFICA SE O ARQUIVO JÁ EXISTE,
    // CASO NÃO EFETUA A CRIAÇÃO
    If (!lFind)
        If (!PFXCA2PEM(cCertPath + cFileName + ".pfx", cFullPath, @cError, cPassword))
			logZBE(SE1->E1_FILIAL,SE1->E1_NUM + " erro na extração do certificado *_CA" ) //ticket 85037 - Antonio Domingos - 21/12/2022
	...
	logZBE(SE1->E1_FILIAL,SE1->E1_NUM + " esta na rotina GETCERTIFICATE e o certificado .KEY eh " + cFullPath ) //ticket 85037 - Antonio Domingos - 21/12/2022

    // VERIFICA SE O ARQUIVO JÁ EXISTE,
    // CASO NÃO EFETUA A CRIAÇÃO
    If (!lFind)
        If (!PFXKey2PEM(cCertPath + cFileName + ".pfx", cFullPath, @cError, cPassword))
			logZBE(SE1->E1_FILIAL,SE1->E1_NUM + " erro na extração do certificado *_KEY" ) //ticket 85037 - Antonio Domingos - 21/12/2022
	...
	logZBE(SE1->E1_FILIAL,SE1->E1_NUM + " esta na rotina GETCERTIFICATE e o certificado _cert.pem eh " + cFullPath ) //ticket 85037 - Antonio Domingos - 21/12/2022

    // VERIFICA SE O ARQUIVO *.CERT JÁ EXISTE,
    // CASO NÃO EFETUA A CRIAÇÃO
    If (!lFind)
        If (!PFXCert2PEM(cCertPath + cFileName + ".pfx", cFullPath, @cError, cPassword))
			logZBE(SE1->E1_FILIAL,SE1->E1_NUM + " erro na extração do certificado *_CERT" ) //ticket 85037 - Antonio Domingos - 21/12/2022
...
Static Function UpSoap(cXML)
	...
	logZBE(SE1->E1_FILIAL,SE1->E1_NUM + " entrou na funcao UPSOAP para gravar retorno bradesco ") //ticket 85037 - Antonio Domingos - 21/12/2022
	...
	If Empty(cXML)
		...
		aAdd(aRet, {.F.,oXml})
		logZBE(SE1->E1_FILIAL,"FUNCAO UpSoap: NAO RECEBEU ARGUMENTO cXML" ) //ticket 85037 - Antonio Domingos - 21/12/2022
		Return lRet
	EndIf
	...
	If (oXML == NIL )
		...
		logZBE(SE1->E1_FILIAL,"FUNCAO UpSoap: FALHA NA GERACAO DO XML (ERRO) " +cError+" / "+cWarning ) //ticket 85037 - Antonio Domingos - 21/12/2022
		Return lRet
	Endif
	...
	If cError # ""
		...
		aAdd(aRet, {.F.,oXml})
		logZBE(SE1->E1_FILIAL,"FUNCAO UpSoap: FALHA NA GERACAO DO XML (ERRO) " + cError ) //ticket 85037 - Antonio Domingos - 21/12/2022
		Return lRet
	EndIf
	...
	If cWarning # ""
		...
		aAdd(aRet, {.F.,oXml})
		logZBE(SE1->E1_FILIAL,"FUNCAO UpSoap: FALHA NA GERACAO DO XML (ALERTA) " + cWarning) //ticket 85037 - Antonio Domingos - 21/12/2022
		Return lRet
	EndIf
	...
	logZBE(SE1->E1_FILIAL,SE1->E1_NUM + "Retorno SOAP " + SE1->E1_XWSBRAC + " : " + SE1->E1_XWSBRAD ) //ticket 85037 - Antonio Domingos - 21/12/2022
...
/*/{Protheus.doc} Static Function LOGZBE
	...
	@version 01
/*/
Static Function logZBE(_cFilial,cMensagem) //ticket 85037 - Antonio Domingos - 21/12/2022

	RecLock("ZBE", .T.)
		Replace ZBE_FILIAL 	   	With _cFilial
...
Static Function UpE1IDCNAB()
	...
	If Empty(SE1->E1_IDCNAB)
		...
		Do While .t. // @history ticket TI  - FWNM - 10/09/2021 - Melhoria IDCNAB após golive CLOUD
			...
			cQuery += " AND E1_FILIAL='"+_cFilial+"' " //ticket 85037 - Antonio Domingos - 20/12/2022
			...
			If WorkIDCNAB->( !EOF() )
				...
				logZBE(_cFilial,"E1_IDCNAB n. " + cIdCnab + " já existe na base! Será alterado por este PE antes de gravar no E1_NUM = " + SE1->E1_NUM) //ticket 85037 - Antonio Domingos - 21/12/2022
				...
				logZBE(_cFilial,"Novo E1_IDCNAB n. " + cNextCod + " foi gravado no título n. " + SE1->E1_NUM + " para evitar duplicidade") //ticket 85037 - Antonio Domingos - 21/12/2022
...
Static Function ChkFIE()
	...
	Do While WorkFIE->( !EOF() )
		...
		RecLock("FIE", .t.)
			FIE->FIE_FILIAL := WorkFIE->E1_FILIAL //ticket 85037 - Antonio Domingos - 20/12/2022
			FIE->FIE_CART   := "R"
		...
		logZBE(WorkFIE->C5_FILIAL,WorkFIE->C5_NUM + " FIE GERADO POIS IDENTIFICOU QUE EXISTE BOLETO E PV - CHKFIE - ADFIN087P") //ticket 85037 - Antonio Domingos - 21/12/2022
...
User Function BxWSPR(lTela, cE1_FILIAL, cE1_PREFIXO, cE1_NUM, cE1_PARCELA, cE1_TIPO, cE1_CLIENTE, cE1_LOJA)
	...
	If SE1->( dbSeek(cE1_FILIAL+cE1_PREFIXO+cE1_NUM+cE1_PARCELA+cE1_TIPO+cE1_CLIENTE+cE1_LOJA) )
		...
		Begin Transaction
			...
			///////////////////////////////////////////
			cFIEPEDIDO := PadR(AllTrim(SE1->E1_NUM),TamSX3("C5_NUM")[1]) //ticket 85037 - Antonio Domingos - 20/12/2022
			...
			If SC5->( dbSeek(SE1->E1_FILIAL+cFIEPEDIDO) )
            	...
            	If AllTrim(Posicione("SE4",1,FWxFilial("SE4")+SC5->C5_CONDPAG,"E4_CTRADT")) == "1" // Cond Adiantamento = SIM 
					...
					If FIE->( dbSeek(cE1_FILIAL+"R"+cE1_CLIENTE+cE1_LOJA+cE1_PREFIXO+cE1_NUM+cE1_PARCELA+cE1_TIPO) ) //ticket 85037 - Antonio Domingos - 20/12/2022
						...
						logZBE(cE1_FILIAL,cE1_FILIAL+cE1_NUM + " FIE DELETADO - TITULO TIPO " + cTipoE1 + Iif(lTela," MANUALMENTE", " RETORNO CNAB") ) //ticket 85037 - Antonio Domingos - 21/12/2022
					...
					cFilOld := cFilAnt        //ticket 85037 - Antonio Domingos - 20/12/2022
					cFilAnt := SE1->E1_FILIAL //ticket 85037 - Antonio Domingos - 20/12/2022
					...
					logZBE( cE1_FILIAL,cE1_FILIAL+cE1_NUM + " ALTERADO TIPO "+cE1_TIPO+" PARA "+SE1->E1_TIPO ) //ticket 85037 - Antonio Domingos - 21/12/2022

					SE1->(DbGoto(nRecnoE1PR)) //ticket 35037 - Antonio Domingos - 20/12/2022
					...
					If lMsErroAuto
						...
						logZBE(SE1->E1_FILIAL,SE1->E1_FILIAL+"-"+SE1->E1_FILIAL+"-"+SE1->E1_PREFIXO+"-"+SE1->E1_PARCELA+SE1->E1_TIPO + " ERRO - BAIXA NÃO REALIZADA ")  //TICKET 85037 - Antonio Domingos - 21/12/2022
					
					Else

						logZBE(SE1->E1_FILIAL,SE1->E1_FILIAL+"-"+SE1->E1_FILIAL+"-"+SE1->E1_PREFIXO+"-"+SE1->E1_PARCELA+SE1->E1_TIPO + " BAIXADO POR SUBSTITUICAO - TITULO TIPO " + cTipoE1 + Iif(lTela," MANUALMENTE", " RETORNO CNAB") ) //TICKET 85037 - Antonio Domingos - 21/12/2022
						...
						cFIHSeq	  := SE5->E5_SEQ
						cFilOri   := cE1_FILIAL //ticket 85037 Antonio - 20/12/2022
						cPrefOri  := cTipoE1
						cNumOri   := SE1->E1_NUM
						...
						dbselectarea("FIH")
						//ticket 85037 Antonio - 20/12/2022 (cFilOri)
						FCriaFIH(cFilOri, cPrefOri, cNumOri, cParcOri, cTipoOri, cCfOri, cLojaOri,;
						"SE1", cPrefDest, cNumDest, cParcDest, cTipoDest, cCfDest, cLojaDest,;
						cFilDest, cFIHSeq )

						logZBE( cE1_FILIAL,cE1_FILIAL+cE1_NUM + " GEROU FIH (AMARRACAO TITULOS SUBSTITUIDOS) - ORIGEM X DESTINO " ) //TICKET 85037 - Antonio Domingos - 21/12/2022
						...
						Else
							...
							RecLock("FIE", .t.)
								FIE->FIE_FILIAL := cE1_FILIAL //ticket 85037 - Antonio Domingos - 20/12/2022
								FIE->FIE_CART   := "R"
							...
							logZBE( cE1_FILIAL,cE1_FILIAL+cE1_NUM + " GEROU RA E FIE (AMARRACAO PV E ADIANTAMENTO) COM SUCESSO " ) //ticket 85037 - Antonio Domingos - 21/12/2022

						EndIf				
					
					EndIf
				
					cFilAnt := cFilOld //ticket 85037 - Antonio Domingos - 20/12/2022
...
Static Function SendMailRA()
	...
	Local cMensagem	:= ""
	Local cAssunto	:= "[ RA ] - Adiantamento gerado para o PV n. " + SE1->E1_FILIAL+"-"+SE1->E1_NUM //ticket 85037 - Antonio Domingos - 20/12/2022
	...
	// Detalhes
	cMensagem += '<tr>'
	cMensagem += '<td>' + cValToChar(SE1->E1_FILIAL)     + '</td>' //ticket 85037 - Antonio Domingos - 20/12/2022

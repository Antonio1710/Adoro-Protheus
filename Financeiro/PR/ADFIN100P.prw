...
User Function ADFIN100P(cRotina)
	...
	*/
	AEval( FWLoadSM0(), {|u| iif( AScan(aEmpresas, {|v| u[1] == v[1] }), AAdd( aEmpresas, { u[1]} ) , Nil) })
	...
	*/
	...
	IF ! (cTitTip $ MVPAGANT)
		...
		For i:=1 to Len(aEmpresas)
			...
			cQryCR02 += " UNION ALL "
			cQryCR02 += " SELECT E2_FILIAL, E2_PREFIXO, E2_NUM, E2_PARCELA, E2_TIPO, E2_FORNECE, E2_LOJA, E2_NOMFOR, E2_VENCREA, E2_VALOR, E2_SALDO, E2_MOEDA, E2_CODBAR "
			cQryCR02 += " FROM " + fGetSX2Emp( AllTrim(aEmpresas[i,1]), "SE2", "X2_ARQUIVO" ) + " SE2 (NOLOCK) "
			cQryCR02 += " INNER JOIN " + fGetSX2Emp( AllTrim(aEmpresas[i,1]), "SA2", "X2_ARQUIVO" ) +" SA2 (NOLOCK) ON A2_COD=E2_FORNECE AND A2_LOJA=E2_LOJA AND SA2.D_E_L_E_T_='' "
			cQryCR02 += " WHERE A2_CGC LIKE '"+cCGCRaiz+"%' "
			cQryCR02 += " AND ( E2_TIPO IN "+FormatIn(MVPAGANT,";")+" OR E2_TIPO IN "+FormatIn(MV_CPNEG,"|")+" ) "
			cQryCR02 += " AND E2_SALDO > 0 "
			cQryCR02 += " AND A2_CGC <> '' "
	...
	If !(cTitPre $ cPreGTA) // @history Ticket  14432 - Fer Macieira- 08/06/2021 - BLOQUEIO DE GTA - IMPOSTO (PAINEL DE PAGAMENTOS)
        ...
        For i:=1 to Len(aEmpresas)
            ...
            cQryCR03 += " UNION ALL "
            cQryCR03 += " SELECT E1_FILIAL, E1_PREFIXO, E1_NUM, E1_PARCELA, E1_TIPO, E1_CLIENTE, E1_LOJA, E1_NOMCLI, E1_VALOR, E1_SALDO, E1_MOEDA "
			cQryCR03 += " FROM " + fGetSX2Emp( AllTrim(aEmpresas[i,1]), "SE1", "X2_ARQUIVO" ) + " SE1 (NOLOCK) "
			cQryCR03 += " INNER JOIN " + fGetSX2Emp( AllTrim(aEmpresas[i,1]), "SA1", "X2_ARQUIVO" ) + " SA1 (NOLOCK) ON A1_COD=E1_CLIENTE AND A1_LOJA=E1_LOJA AND SA1.D_E_L_E_T_='' "
            cQryCR03 += " WHERE A1_CGC LIKE '"+cCGCRaiz+"%' "
            cQryCR03 += " AND E1_SALDO > 0 "
            cQryCR03 += " AND A1_CGC <> '' "
            cQryCR03 += " AND '"+cCGCRaiz+"' <> '' "
	...
	For i:=1 to Len(aEmpresas)
		...
		cQryCR04 += " UNION ALL "
		cQryCR04 := " SELECT E2_FILIAL, E2_PREFIXO, E2_NUM, E2_PARCELA, E2_TIPO, E2_FORNECE, E2_LOJA, E2_NOMFOR, E2_VENCREA, E2_VALOR, E2_SALDO, E2_MOEDA, E2_CODBAR "
		cQryCR04 += " FROM " + fGetSX2Emp( AllTrim(aEmpresas[i,1]), "SE2", "X2_ARQUIVO" ) + " SE2 (NOLOCK) "
		cQryCR04 += " INNER JOIN " + fGetSX2Emp( AllTrim(aEmpresas[i,1]), "SA2", "X2_ARQUIVO" ) + " SA2 (NOLOCK) ON A2_COD=E2_FORNECE AND A2_LOJA=E2_LOJA AND SA2.D_E_L_E_T_='' "
...
/*/
Static Function fGetSX2Emp( cEmp, cTab, cFieldRet )

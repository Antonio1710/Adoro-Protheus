...
Static Function ProcAtu()
	...
	cQuery:= " SELECT C5_FILIAL,C5_NUM,C5_XPREAPR,C5_TIPO"
		...
		If SC5->(dbseek(LSC5->C5_FILIAL+LSC5->C5_NUM))
			...
			If SC9->(dbseek(LSC5->C5_FILIAL+LSC5->C5_NUM))
				While SC9->(!Eof()) .And. SC9->(C9_FILIAL+C9_PEDIDO) == LSC5->C5_FILIAL+LSC5->C5_NUM
					...
					Elseif LSC5->C5_XPREAPR == "B"
						if Reclock("SC9",.F.)
							SC9->C9_BLCRED := IF(LSC5->C5_TIPO $ "BD"," ","01") //ticket 90119	- Antonio Domingos - 10/04/2023
							SC9->C9_BLEST  := IF(LSC5->C5_TIPO $ "BD"," ","01") //ticket 90119	- Antonio Domingos - 18/04/2023
							SC9->(Msunlock())
						endif
					Endif				
					SC9->(DbSkip())

...
/*/{Protheus.doc} User Function HCFU001
	...
	@history Ticket 85319  - Antonio Domingos - 26/01/2023 - Validação Fontes V33 - Dic. Banco de Dados.
	@history Ticket 86150 - Everson, 01/02/2023 - Tratamento para selecionar mais de um vendedor.
	@history Ticket 89457 - Antonio Domingos - 06/03/2023 - Bloqueio 180 dias - Criação e declaração da Variavel cDIVvend
	@history Ticket 93192 - Antonio Domingos - 05/05/2023 - Relatório 180 dias - Ajuste de parametro
/*/
User Function HCFU001()  // U_HCFU001()
	...
	Private oTr1
	Private _lUsaFxVend := SuperGetMv("MV_#UFXVEN",.F.,.F.) //Ativa a Função de Selecão de Vendedores.

	//
	If !_lUsaFxVend //Desativada seleção de vendedores via parametro. Se não utiliza usa função parambox e desativa pergunte.
		Aadd( aParamBox ,{1,"Bloq. c/ Ultimo Movimento ate",CtoD(space(8)),"" ,'.T.',,'.T.',50,.T.})
		Aadd( aParamBox ,{1,"Descons.Vendedores:",Space(99),"" ,'.T.',,'.T.',150,.F.})
	...
	//@history Ticket 70142  - Edvar / Flek Solution - 06/04/2022 - Retirada de função PUTSX1
	//validPerg()
	If _lUsaFxVend //Se Usa Seleção de Vendedores usa a função pergunte.
		if !pergunte(cPerg,.T.)
			_lContinua := .F.
		return
		EndIf
...
Static Function criaTemp()
	...
	oTr1 := FWTemporaryTable():New("TR1", aCpos)
	oTr1:Create()
...
Static Function BLOQCLI()
	...
	If AT("','",mv_par02) = 0 //Ticket 93192 - Antonio Domingos - 05/05/2023
		mv_par02 := "'"+STRTRAN(mv_par02,",","','")+"'" //Ticket 89457 - Antonio Domingos - 03/03/2023
	EndIf
	If AT("/",mv_par02) > 0 //Ticket 93192 - Antonio Domingos - 05/05/2023
		mv_par02 := STRTRAN(mv_par02,"/","','") //Ticket 89457 - Antonio Domingos - 03/03/2023
	EndIf
	...
	if !empty(mv_par02)
		cQry+= "   AND "+RetSqlName("SA1")+ ".A1_VEND NOT IN (" + mv_par02 + ") "
	endif
	...
	Memowrite("C:\TEMP\HCFU001.SQL",cQry)
			...
			If Alltrim(cEmpAnt) == "01" && apenas para a empresa 01, entrar na PB3  Chamado: 034630
				...
				If PB3->(dbSeek( xFilial( 'PB3' )+TR1->A1_COD+TR1->A1_LOJA))
					...
					If TR1->NUM_PED <= 0
						...
						//Grava LOG  
						U_Ado05Log( PB3_COD, PB3_LOJA, alltrim(cusername), dDataBase, GetSX3Cache( 'PB3_BLOQUE', 'X3_Titulo' ),'Desbloqueado','Bloqueado' )
						U_Ado05Log( PB3_COD, PB3_LOJA, alltrim(cusername), dDataBase, GetSX3Cache( 'PB3_MOTBLQ', 'X3_Titulo' ), PB3->PB3_MOTBLQ," BLOQUEADO PELA ROTINA AUTOMATICA, S/ UTILIZAÇÃO A 6 MESES")
...
(long_description)
	...
	@history Ticket 85319  - Antonio Domingos - 26/01/2023 - Validação Fontes V33 - Dic. Banco de Dados.
/*/
Static Function _fValidChar(_cVariavel)

	Local _n1
	Local _aChar := {"/",";",".","-","_","&"," "}
	
	For _n1 := 1 to Len(_aChar)
		If AT(_aChar[_n1],_cVariavel) > 0
			_cVariavel := STRTRAN(_cVariavel,_aChar[_n1],",")
			Exit
		EndIf
	Next

Return(_cVariavel)
/*/{Protheus.doc} User Function FXVEND
	Consulta padrão para selecionar vendedores.
	@type  Function
	@author Everson
	@since 31/01/2023
	@version 01
/*/
User Function FXVEND()
	...
	CursorWait()
		...
		While ! SA3->(Eof()) .And. SA3->A3_FILIAL == FWxFilial("SA3")

			Aadd(aGrupo, Alltrim(SA3->A3_COD) + " - " + Alltrim(SA3->A3_NOME))
	...
	If f_Opcoes(@MvPar, "Vendedores", aGrupo, MvParDef, 12, 49, .F., nTam, 11)

...
User Function F200AVL()
    ...
    lProcessa := ChkIDCNAB(SE1->E1_FILIAL,cIDCNAB) //ticket 85037 - Antonio Domingos - 20/12/2022

    If !lProcessa
        logZBE(SE1->E1_FILIAL,"E1_FILIAL "+SE1->E1_FILIAL+ "E1_IDCNAB n. " + cIdCnab + " duplicado na base! Título " + SE1->E1_NUM + " não será baixado por segurança...") //ticket 85037 - Antonio Domingos - 20/12/2022
        Alert("[F200AVL] - E1_FILIAL "+SE1->E1_FILIAL+" E1_IDCNAB n. " + cIDCNAB + " está duplicado! Este título não será baixado! Anote e verifique depois...") //ticket 85037 - Antonio Domingos - 20/12/2022
        Return lProcessa
    EndIf
    //
    ...
    If !Empty(AllTrim(cIDCNAB)) // @history ticket 71284   - Fernando Macieira - 12/04/2022 - Baixa Retorno CNAB - Tratar msg
        lProcessa := ChkBxSTP(SE1->E1_FILIAL,cIDCNAB) //ticket 85037 - Antonio Domingos - 20/12/2022

        If !lProcessa
            logZBE(SE1->E1_FILIAL,"E1_FILIAL "+SE1->E1_FILIAL+" E1_IDCNAB n. " + cIdCnab + " , tipo PR, já possui baixa por substituição E5_MOTBX = STP! Título " + SE1->E1_NUM + " não será baixado por segurança...") //ticket 85037 - Antonio Domingos - 20/12/2022
    ...
    If AllTrim(cBanco) == "237" .and. cOcorre == "06" // Bradesco WS
            ...
            If SE1->( dbSeek(cIDCnab) )
                ...
                If AllTrim(SE1->E1_TIPO) == AllTrim(cTipoE1) .and. !Empty(SE1->E1_XWSBRAC) .and. ( Val(AllTrim(SE1->E1_XWSBRAC)) == Val(AllTrim(cCodRetOk)) .or. Val(AllTrim(SE1->E1_XWSBRAC)) == Val(AllTrim(cCodRet69)) )  // ticket 745 - Fernando Macieira - 21/09/2020 - Implementação título PR
                    ...
                    */
                        ...
                        cFIEPEDIDO := ALLTRIM(PadR(AllTrim(SE1->E1_NUM),TamSX3("C5_NUM")[1])) //ticket 85037 - Antonio Domingos -  20/12/2022
                        ...
                        //If SC5->( dbSeek(SE1->E1_FILIAL+FIE->FIE_PEDIDO) ) // @history ticket 102 - Fernando Macieira - 28/08/2020 - WS Bradesco - FIE deletados sem explicação, mesmo tendo C5 e E1
                            ...
                            If AllTrim(Posicione("SE4",1,FWxFilial("SE4")+SC5->C5_CONDPAG,"E4_CTRADT")) == "1" // Cond Adiantamento = SIM // @history ticket 102 - Fernando Macieira - 28/08/2020 - WS Bradesco - FIE deletados sem explicação, mesmo tendo C5 e E1
                                ...
                                If lBxPROk
                                    ...
                                    SC5->( msUnLock() )
                                    //ticket 85037 - Antonio Domingos - 20/12/2022
                                    logZBE(SC5->C5_FILIAL,SC5->C5_FILIAL+SC5->C5_NUM + " GRAVADO CAMPO C5_XWSPAGO=S PELO RETORNO CNAB ARQ " + AllTrim(MV_PAR04)) // Chamado n. 056247 || OS 057671 || FINANCEIRO || LUIZ || 8451 || BOLETO BRADESCO WS - Fernando Macieira - 21/05/2020

                                    //Everson - 07/07/2020. Chamado T.I.
                                    //Everson - 03/07/2020. Chamado 059444.
                                    //limpZBH(Alltrim(cValToChar(SC5->C5_NUM)))
                                    //ticket 85037 - Antonio Domingos - 20/12/2022
                                    limpZBH(SC5->C5_FILIAL,SC5->C5_NUM)
                                    //
                                    ...
                                    If SC9->( dbSeek(SC5->(C5_FILIAL+C5_NUM)) )
                                        Do While SC9->( !EOF() ) .and. SC9->C9_FILIAL==SC5->C5_FILIAL .and. SC9->C9_PEDIDO==SC5->C5_NUM // Chamado n. 056247 || OS 057671 || FINANCEIRO || LUIZ || 8451 || BOLETO BRADESCO WS - Fernando Macieira - 27/05/2020
                                            //a450Grava(1,.T.,.T.)
                                            U_ADEST084P(SC5->C5_FILIAL,1,.T.,.T.) //ticket 85037 - Antonio Domingos - 20/12/2022
                                            SC9->( dbSkip() )
                                        EndDo
                                        //ticket 85037 - Antonio Domingos - 20/12/2022
                                        logZBE(SC5->C5_FILIAL,SC5->C5_FILIAL+SC5->C5_NUM + " FOI LIBERADO CREDITO/ESTOQUE PELO RETORNO CNAB ROTINA ADORO U_ADEST084P") // Chamado n. 056247 || OS 057671 || FINANCEIRO || LUIZ || 8451 || BOLETO BRADESCO WS - Fernando Macieira - 21/05/2020
                                    EndIf
                                    //
                                ...
                                Else
                                    ...
                                    // ticket 745 - Fernando Macieira - 21/09/2020 - Implementação título PR
                                    //ticket 85037 - Antonio Domingos - 20/12/2022
                                    logZBE(SC5->C5_FILIAL, SC5->C5_FILIAL+SC5->C5_NUM + " NAO GEROU RA! FATURAMENTO NAO LIBERADO PELO RETORNO CNAB ARQ " + AllTrim(MV_PAR04) )
                                    MessageBox( "Boleto n. " + SC5->C5_FILIAL+SC5->C5_NUM + " não gerou RA! Faturamento não liberado...","WS Bradesco - Substituição Retorno CNAB PR -> RA", MB_ICONHAND )
        ...
        If !lAchouTit
            ...
            cQuery += " AND E1_FILIAL = '"+SE1->E1_FILIAL+"' " //ticket 85037 - Antonio Domingos - 20/12/2022
                        ...
                        //If SC5->( dbSeek(SE1->E1_FILIAL+FIE->FIE_PEDIDO) ) // Cond Adiantamento = SIM // @history ticket 102 - Fernando Macieira - 28/08/2020 - WS Bradesco - FIE deletados sem explicação, mesmo tendo C5 e E1
                            ...
                            If AllTrim(Posicione("SE4",1,FWxFilial("SE4")+SC5->C5_CONDPAG,"E4_CTRADT")) == "1" // Cond Adiantamento = SIM // @history ticket 102 - Fernando Macieira - 28/08/2020 - WS Bradesco - FIE deletados sem explicação, mesmo tendo C5 e E1
                                ...
                                Else
                                    ...
                                    // ticket 745 - Fernando Macieira - 21/09/2020 - Implementação título PR
                                    logZBE(SC5->C5_FILIAL, SC5->C5_FILIAL+SC5->C5_NUM + " NAO GEROU RA! FATURAMENTO NAO LIBERADO PELO RETORNO CNAB ARQ " + AllTrim(MV_PAR04) ) //ticket 85037 - Antonio Domingos - 20/12/2022
                                    MessageBox( "Boleto n. " + SC5->C5_FILIAL+SC5->C5_NUM + " não gerou RA! Faturamento não liberado...","WS Bradesco - Substituição Retorno CNAB PR -> RA", MB_ICONHAND ) //ticket 85037 - Antonio Domingos - 20/12/2022
...
/*/
Static Function logZBE(_cFilial,cMensagem)
...
/*/{Protheus.doc} limpZBH
    ...
    @version 01
/*/
Static Function limpZBH(_cFilial,cPedido) //ticket 85037 - Antonio Domingos - 20/12/2022
    //Variáveis.
    Local aArea := GetArea()
    Local cUpt  := ""
    //
    Default _cFilial := " " //ticket 85037 - Antonio Domingos - 20/12/2022
    Default cPedido  := ""
    //Everson, 07/07/2020, Chamado T.I.
    logZBE(_cFilial,_cFilial + cPedido + " INÍCIO EXCLUSÃO DE MENSAGENS DE BLOQUEIO - ZBH") //ticket 85037 - Antonio Domingos - 20/12/2022
    //
    If Empty(_cFilial+cPedido)
        Conout( DToC(Date()) + " " + Time() + " F200AVL - limpZBH - variáveis _cFilial e cPedido vazias." )
        RestArea(aArea)
        Return Nil 
    EndIf
    //
    //ticket 85037 - Antonio Domingos - 20/12/2022
    cUpt  := " UPDATE " + RetSqlName("ZBH") + " SET D_E_L_E_T_ = '*' WHERE ZBH_FILIAL = '" + _cFilial + "' AND ZBH_PEDIDO = '" + cPedido + "' AND D_E_L_E_T_  = '' "
    //
    If TcSqlExec(cUpt) < 0
        Conout( DToC(Date()) + " " + Time() + " F200AVL - limpZBH - motivos de bloqueio NÃO excluídos (ZBH) " + _cFilial+cPedido + " " + TCSQLError() ) //ticket 85037 - Antonio Domingos - 20/12/2022
        logZBE(_cFilial,_cFilial+cPedido + " MENSAGENS DE BLOQUEIO NÃO FORAM EXCLUÍDAS - ZBH") //ticket 85037 - Antonio Domingos - 20/12/2022
    Else 
        Conout( DToC(Date()) + " " + Time() + " F200AVL - limpZBH - motivos de bloqueio excluídos (ZBH) " + _cFilial+cPedido ) //ticket 85037 - Antonio Domingos - 20/12/2022
        logZBE(_cFilial,_cFilial+cPedido + " MENSAGENS DE BLOQUEIO EXCLUÍDAS - ZBH") //ticket 85037 - Antonio Domingos - 20/12/2022
    EndIf
    //
    RestArea(aArea)
Return Nil
...
/*/{Protheus.doc} Static Function ChkIDCNAB
    ...
    @see (links_or_references)
/*/
Static Function ChkIDCNAB(_cFilial,cIDCNAB) //ticket 85037 - Antonio Domingos - 20/12/2022
...
/*/
Function u_200AVLA0( uPar1,uPar2 )
Return( limpZBH( uPar1,uPar2 ) )
...
/*/{Protheus.doc} Static Function ChkIDCNAB
    ...
    @see (links_or_references)
    @history ticket 70929 - Fernando Macieira - 08/04/2022 - Baixa retorno CNAB, TIPO PR, mesmo após baixado por STP
    @history ticket 85037 - Antonio Domingos  - 20/12/2022 - BAIXA AUTOMATICA PEDIDOS ANTECIPADO - BOLETOS
/*/
Static Function ChkBxSTP(_cFilial,cIDCNAB) //ticket 85037 - Antonio Domingos - 20/12/2022

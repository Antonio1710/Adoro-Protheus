...
User Function ExcRAPV()
	...
	// Logins autorizados
    // INICIO - ticket 89538   - Antonio Domingos - 17/03/2023
    If !(RetCodUsr() $ cUsrAut)
        MsgStop(;
        "Login " + RetCodUsr() + " - " + AllTrim(cUserName) + " sem acesso para usar esta rotina!"+CHR(13)+CHR(10)+;
        CHR(13)+CHR(10)+" Solução: Incluir o codigo do usuario no Parametro MV_#WSAUTF.",;
        "Função EXCRAPV - FA740BRW")
		Return
	EndIf
    // FIM - ticket 89538   - Antonio Domingos - 17/03/2023
    ...
    If FIE->( dbSeek(FWxFilial("FIE")+"R"+SE1->(E1_CLIENTE+E1_LOJA+E1_PREFIXO+E1_NUM+E1_PARCELA+E1_TIPO)+PadR(SE1->E1_NUM,Len(FIE->FIE_PEDIDO))) )
        ...
        If SC5->( dbSeek(FIE->(FIE_FILIAL+FIE_PEDIDO)) ) 
            //If SC5->( !msSeek(FIE->(FIE_FILIAL+FIE_PEDIDO)) )
            If !Empty(SC5->C5_NOTA) // ticket 1768 - FWNM - 23/09/2020 - PV com adiantamento superior a NF
                RecLock("FIE", .F.)
                    FIE->( dbDelete() )
                FIE->( msUnLock() )
                logZBE(SE1->E1_TIPO+" n. " +SE1->E1_FILIAL+"-"+SE1->E1_NUM + " " +SC5->C5_FILIAL+"-"+SC5->C5_NUM + " AMARRACAO  " +SE1->E1_TIPO +" X PV FOI DESFEITA PELA ROTINA MANUAL U_EXCRAPV")
                MessageBox(SE1->E1_TIPO+" n. " + SE1->E1_FILIAL+"-"+SE1->E1_NUM + " foi desvinculado do PV com sucesso!", "WS Bradesco - Desvinculação  " +SE1->E1_TIPO +" x PV manual", MB_OK)
            Else
                If MsgYesNO("Pedido "+FIE->(FIE_FILIAL+"-"+FIE_PEDIDO)+" Não foi faturado, Deseja realmente retirar o vinculo deste  " +SE1->E1_TIPO +" x PV?")        
                    RecLock("FIE", .F.)
                        FIE->( dbDelete() )
                    FIE->( msUnLock() )
                    logZBE(SE1->E1_TIPO+" n. " +SE1->E1_FILIAL+"-"+SE1->E1_NUM + " PV " + SC5->C5_NUM + " AMARRACAO  " +SE1->E1_TIPO +"  X PV FOI DESFEITA PELA ROTINA MANUAL U_EXCRAPV")
                    MessageBox(SE1->E1_TIPO+" n. " + SE1->E1_FILIAL+"-"+SE1->E1_NUM + " foi desvinculado do PV " + SC5->C5_FILIAL+"-"+SC5->C5_NUM + " com sucesso!", "WS Bradesco - Desvinculação  " +SE1->E1_TIPO +" x PV manual", MB_OK)
                    ...
                    MessageBox(SE1->E1_TIPO+" n. " + SE1->E1_FILIAL+"-"+SE1->E1_NUM + " não foi desvinculado do PV " + SC5->C5_FILIAL+"-"+SC5->C5_NUM + "!", "Vinculação  " +SE1->E1_TIPO +" x PV mantida!", MB_OK)
                EndIf
            EndIf
        Else
            If MsgYesNO("Pedido "+FIE->(FIE_FILIAL+FIE_PEDIDO)+" Não existe, Deseja retirar o vinculo deste  " +SE1->E1_TIPO +" x PV?")        
                RecLock("FIE", .F.)
                    FIE->( dbDelete() )
                FIE->( msUnLock() )
                logZBE(SE1->E1_TIPO+SE1->E1_FILIAL+"-"+SE1->E1_NUM+" PV "+SC5->C5_FILIAL+"-"+SC5->C5_NUM + " AMARRACAO " +SE1->E1_TIPO+" X PV FOI DESFEITA PELA ROTINA MANUAL U_EXCRAPV")
                MessageBox(SE1->E1_TIPO+" n. " + SE1->E1_FILIAL+"-"+SE1->E1_NUM + " foi desvinculado do PV com sucesso!", "WS Bradesco - Desvinculação " +SE1->E1_TIPO +" x PV manual", MB_OK)
            Else
                MessageBox(SE1->E1_TIPO+" n. " + SE1->E1_FILIAL+"-"+SE1->E1_NUM + " não foi desvinculado do PV!", "Vinculação  " +SE1->E1_TIPO +"  x PV mantida!", MB_OK)
            EndIf
        EndIf
        // FIM - ticket 89538   - Antonio Domingos - 17/03/2023
    ...
    Else
        ...
        MessageBox("RA n. " + SE1->E1_FILIAL+"-"+SE1->E1_NUM + " não possui PV vinculado!", "WS Bradesco - Desvinculação RA x PV manual", MB_OK)

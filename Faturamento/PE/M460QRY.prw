...
/*/{Protheus.doc} User Function M460QRY
	...
	@history Everson, 01/02/2023, ticket 87697 - Adicionado nolock na query. 
...
Static Function _Retorna(lCCSP)
	...
	IF ALLTRIM(CEMPANT) $ _cEmpAt1 .AND. cFilAnt <> '03' //ticket TI - Antonio Domingos - 25/05/2023 
	...
	IF AllTrim(SM0->M0_CODFIL) <> '03' .and. (Alltrim(cEmpAnt)+Alltrim(cFilAnt) $ _cEmpFL1) // sigoli 21/03/2017 // @history Ticket TI - 28/02/2023 - Fernando Macieira - Ajustes estabilização pos golive migração dicionário dados

		If lCCSP
			cQrySC9 += " AND C9_ROTEIRO BETWEEN '" + _cRotIni + "' AND '" + _cRotFin + "'"
			cQrySC9 += " AND C9_DTENTR = '" + DTOS(_dDtEntr) + "' "
			cQrySC9 += " AND EXISTS(SELECT C5_NUM FROM "+RetSQlname("SC5")+" (NOLOCK) " //Everson - 01/02/2023 - 87697.

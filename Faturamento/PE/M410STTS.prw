...
/*/{Protheus.doc} User Function M410STTS
	...
	@history Ticket 81594	- Everson  - 20/10/2022 - Tratamento para pedidos bloqueados por prazo médio da condição de pagamento.
	@history Ticket TI      - Everson  - 15/12/2022 - Tratamento para envio de pedido de Itapira ao Salesforce.
	@history Ticket 86504   - Abel Babini - 16/01/2023 - Não considerar títulos DFS para bloqueio por atraso na liberação de crédito
	@history Ticket 84697   - Everson - 24/01/2023 - Adicionada filial no while.
	@history Ticket 18465   - Everson - 25/01/2023 - Exclusão de pedido de envio para armazenagem, vinculado a contrato de compra de matéria-prima..
	@history Ticket 86504   - Abel Babini - 07/02/2023 - Não considerar títulos DFS para bloqueio por atraso na liberação de crédito
	@history Ticket 18465   - Everson - 14/03/2023 - Tratamento para envio de pedido de venda vinculado a contrato de compra de MP.
	@history Ticket 90219   - Everson - 16/03/2023 - Tratamento errorlog.
...
User Function M410STTS()
	...
	LOcal _lADPVTT	:= SuperGetmv("MV_#ADPVTT",.F.,.T.) //Ativa=.T.,Desativa=.F. Desvinculo de PV x TITULO
	...
	Private _cEmpAt1 := SuperGetMv("MV_#EMPAT1",.F.,"01/13") //Codigo de Empresas Ativas Grupo 1 //ticket TI - Antonio Domingos - 24/05/2023
	Private lSfInt	:= (IsInCallStack('U_RESTEXECUTE') .OR. IsInCallStack('RESTEXECUTE'))
	Private _cEmpAt2 := SuperGetMv("MV_#EMPAT2",.F.,"01/02/13") //Codigo de Empresas Ativas Grupo 2 //ticket TI - Antonio Domingos - 24/05/2023

	//If Alltrim(cEmpAnt) <> "01"  ;  //Incluido por Adriana devido ao error.log quando empresa <> 01 - chamado 032804
	If !(Alltrim(cEmpAnt) $ _cEmpAt1) ;  //ticket TI - Antonio Domingos - 24/05/2023 
	...
	If INCLUI
		...
		IF !lSfInt .And. !Empty(M->C5_XREFATD) .And. Alltrim(cEmpAnt) $ _cEmpAt2 //ticket TI - Antonio Domingos - 24/05/2023 
			DbSelectArea("SC5")
		...
		If SC6->(dbSeek(xFilial("SC6")+_cNumPed ))
			...
			//
			While SC6->(!Eof()) .And. FWxFilial("SC6") == SC6->C6_FILIAL .And. SC6->C6_NUM == _cNumPed //Everson - 24/01/2023 - Ticket 84697.
	...
	ElseIf ALTERA
		...
		If dbSeek(xFilial("SC6")+_cNumPed )
			...
			While !Eof() .And. FWxFilial("SC6") == SC6->C6_FILIAL .And. SC6->C6_NUM == _cNumPed //Everson - 24/01/2023 - Ticket 18465.
	...
	if _nOper == 5
		fFunDel()
		//Ticket 89538 - Antonio Domingos - 17/03/2023
		If _lADPVTT //Ativa e Desativa Função
			U_Desv_PVTIT(SC5->C5_FILIAL,SC5->C5_NUM,SC5->C5_TIPO) //Desvincula Pedidos de Amarração de Titulos de RA/PR/NCC
		EndIf
	endif

	//Everson - 16/03/2023. Chamado 90219.
	//Everson - 14/03/2023. Chamado 18465.
	//Everson - 18/03/2022. Chamado 18465. //Everson - 24/03/2022. Chamado 18465.
	If cEmpAnt $ _cEmpAt1 .And. (! Empty(Alltrim(cValToChar(SC5->C5_XORDPES))) .Or. chkOrdSC6(_nOper, SC5->C5_NUM) .Or. ! Empty(Alltrim(cValToChar(SC5->C5_XCOTRMP))))
...
static function fBscSld(_cCl,_cL)
	...
	_cQuery += " WHERE SE1.E1_CLIENTE = '"+_cCL+"' AND SE1.E1_LOJA = '"+_cL+"' AND SE1.E1_TIPO NOT IN ('NCC','RA','DSF') "  //Ticket 86504   - Abel Babini - 07/02/2023 - Não considerar títulos DFS para bloqueio por atraso na liberação de crédito
...
Static Function envSF(_lBon,_lDoa,cExpSql)
	...
	//Variáveis.
	Local aArea		:= GetArea()
	Local _cEmpFL3 := SuperGetMv("MV_#EMPFL3",.F.,"0102/010B/1301") //Codigos de Empresas+Filiais Ativas Grupo 3 //ticket TI - Antonio Domingos - 24/05/2023	
	Default _lBon	:= .F.
	Default _lDoa	:= .F.
	Default cExpSql	:= ""
	...
	//! (Alltrim(cFilAnt) $"02/0B") //Everson - 15/12/2022 - Ticket TI.
	If lSfInt .Or. !(Alltrim(cEmpAnt)+Alltrim(cFilAnt) $ _cEmpFL3)  //ticket TI - Antonio Domingos - 24/05/2023 
		RestArea(aArea)
		Return Nil
...
Static Function fLibCred(cCliente, cLojaCli, dDtEntr, lExcPedV, cNumPVEx, cNumPVIn)
	...
	//Ticket 86504   - Abel Babini - 16/01/2023 - Não considerar títulos DFS para bloqueio por atraso na liberação de crédito
	BeginSQL Alias cAls003
		SELECT 
		...
		WHERE 
			...
			AND SE1.E1_SALDO > 0
			AND SE1.E1_TIPO NOT IN ('NCC','RA','DSF')
			AND SE1.%notDel%
	...
	//@history Ticket  8      - Abel B.  - 12/03/2021 - Ajustes na rotina de liberação de crédito
	//Ticket 86504   - Abel Babini - 16/01/2023 - Não considerar títulos DFS para bloqueio por atraso na liberação de crédito
	IF _lDiasAtras
		BeginSQL alias cAls006
			SELECT 
...
Static Function fVldCrd(_cTipoCli, cCliente, cLojaCli, _cCdClIn, cFilPedV, cNumPedV, _dValidLC, _cRede, _cNmRede, _nVlMnPed, _nVlMnPSC, _nVlMnParc, _nDiasAtras, cPortadIn, cPortador, nPercen, nValPed, cSC5CPag, dSC5Emis, _lDiasAtras, _nValLim, cSC5Vend, lBlqAtr, aTpBlqAt)
	...
	//Everson - 20/10/2022 - ticket 81594.
	// _nMedPGPd := Posicione("SE4",1,FWxFilial("SE4") + cNumPedV,"E4_DMEDI")
	_nMedPGPd := Posicione("SE4",1,FWxFilial("SE4") + cSC5CPag,"E4_DMEDI") 
	//
	...
	If _nMedPGPd > _nMedPGA1
		...
		u_GrLogZBE(Date(), Time(), cUserName, "Prz med cond pag _nMedPGPd > _nMedPGA1 " + cValToChar(_nMedPGPd) + " " + cValToChar(_nMedPGA1),"FINANCEIRO","M410STTS",;
			       cNumPedV + "/" + cCliente+cLojaCli, ComputerName(), LogUserName())
...
Static Function fFunDel()
	...
	Local _cEmpAt1 := SuperGetMv("MV_#EMPAT1",.F.,"01/13") //Codigo de Empresas Ativas Grupo 1 //ticket TI - Antonio Domingos - 24/05/2023
	if !Supergetmv("MV_X410DEL",,.F.)
		...
		If !(cEmpAnt $ _cEmpAt1) .Or. IsInCallStack("U_PED001B") //ticket TI - Antonio Domingos - 24/05/2023 
		...
		If !lSfInt .And. SC5->C5_UFPLACA <> "99" .And. ! IsInCallStack("U_ADCOM042P")  //&&12/10/16 - Flag para pedido excluido por rotina ADFIN006P/ADFIN018P //Everson - 25/01/2023 - ticket 18465.
...
/*/{Protheus.doc} Desv_PVTIT()
	Função para Desvincular RA de Pedido de Venda Excluido
	...
	Ticket 89538 - Antonio Domingos - 17/03/2023
/*/
User Function Desv_PVTIT(_cFilial,_cPedido,_cTipo)
	Local _cAMARR_PVTIT := ""
	Local _aGetArea := GetArea()
	// Somente para Pedido Tipo Normal
    If _cTipo == "N"
		dbSelectArea("FIE")
		FIE->( dbSetOrder(1) ) // FIE_FILIAL+FIE_CART+FIE_PEDIDO
		If FIE->( dbSeek(_cFilial+"R"+_cPedido) )
			//FIE_FILIAL	FIE_CART	FIE_PEDIDO	FIE_PREFIX	FIE_NUM	FIE_PARCEL	FIE_TIPO	FIE_CLIENT	FIE_FORNEC	FIE_LOJA	FIE_VALOR	FIE_SALDO	FIE_FILORI	D_E_L_E_T_	R_E_C_N_O_	R_E_C_D_E_L_
			_cAMARR_PVTIT:=FIE->("FIL "+FIE_FILIAL+" CRT "+FIE_CART+" PV "+FIE_PEDIDO+" PRX "+FIE_PREFIX+ " TIT "+FIE_NUM+" PARC "+FIE_PARCEL+"TP "+FIE_TIPO)
			RecLock("FIE", .F.)
				FIE->( dbDelete() )
			FIE->( msUnLock() )
			RecLock("ZBE", .T.)
				...
				Replace ZBE_HORA   	With Time()
				Replace ZBE_USUARI  With Upper(Alltrim(cUserName))
				Replace ZBE_LOG	    With  _cAMARR_PVTIT+" AMARRACAO PV X RA DESFEITA PELA ROTINA Desv_PVTIT()-M410STTS"
				Replace ZBE_MODULO  With "SIGAFAT"
				Replace ZBE_ROTINA  With "M410STTS" 
			ZBE->( msUnlock() )
		EndIf
	EndIf

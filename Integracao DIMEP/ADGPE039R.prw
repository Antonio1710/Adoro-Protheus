...
/*{Protheus.doc} User Function ADGPE039R
	Relatorio para acompanhamento de tempo de permanencia do dimep de funcionarios no refeitorio. Relatorio em Excel. 
	@type  Function
...
User Function ADGPE039R()
	
	U_ADINF009P(SUBSTRING(ALLTRIM(PROCNAME()),3,LEN(ALLTRIM(PROCNAME()))) + '.PRW',SUBSTRING(ALLTRIM(PROCNAME()),3,LEN(ALLTRIM(PROCNAME()))),'Relatorio para acompanhamento de tempo de permanencia do dimep de funcionarios no refeitorio. Relatorio em Excel. ')
	...
	AADD(aButtons, { 1,.T.,{|o| nOpca:=1, o:oWnd:End(), Processa({||GPE039R()},"Gerando arquivo","Aguarde...")    }})
...
Static Function GPE039R()    

	PRIVATE oExcel     := FWMSEXCEL():New()
	PRIVATE cPath      := ''
	PRIVATE cArquivo   := 'REL_FUNC_REFEITORIO.XML'
	PRIVATE oMsExcel
	PRIVATE cPlanilha  := DTOC(MV_PAR03) + ' ate ' + DTOC(MV_PAR04)
    PRIVATE cTitulo    := "Relatorio Funcionários - " + DTOC(MV_PAR03) + ' ate ' + DTOC(MV_PAR04)
    PRIVATE aLinhas    := {}
...
Static Function SalvaXml()
	...
	oExcel:Activate()
	oExcel:GetXMLFile("C:\temp\REL_FUNC_REFEITORIO.XML")
...
Static Function CriaExcel()              
	...
	oMsExcel:WorkBooks:Open("C:\temp\REL_FUNC_REFEITORIO.XML")
...
Static Function SqlGeral() 
	...
	cQuery += "        AND CD_ESTRUTURA_RELACIONADA    = " + CVALTOCHAR(nFil) + " "

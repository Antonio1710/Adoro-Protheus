...
/*{Protheus.doc} User Function ADGPE041R
	Relatorio para acompanhamento de tempo de permanencia do dimep de Visitantes no refeitorio. Relatorio em Excel.
	@type  Function
...
User Function ADGPE041R()
	
	U_ADINF009P(SUBSTRING(ALLTRIM(PROCNAME()),3,LEN(ALLTRIM(PROCNAME()))) + '.PRW',SUBSTRING(ALLTRIM(PROCNAME()),3,LEN(ALLTRIM(PROCNAME()))),'Relatorio para acompanhamento de tempo de permanencia do dimep de Visitantes no refeitorio. Relatorio em Excel. ')
	...
	AADD(aButtons, { 1,.T.,{|o| nOpca:=1, o:oWnd:End(), Processa({||GPE041R()},"Gerando arquivo","Aguarde...")    }})
...
Static Function GPE041R()    

	PRIVATE oExcel     := FWMSEXCEL():New()
	PRIVATE cPath      := ''
	PRIVATE cArquivo   := 'REL_VISI_REFEITORIO.XML'
	PRIVATE oMsExcel
	PRIVATE cPlanilha  := DTOC(MV_PAR03) + ' ate ' + DTOC(MV_PAR04)
    PRIVATE cTitulo    := "Relatorio Visitante - " + DTOC(MV_PAR03) + ' ate ' + DTOC(MV_PAR04)
    PRIVATE aLinhas    := {}
...
Static Function GeraExcel()
	...
	WHILE TRC->(!EOF()) 
		...
		aLinhas[nLinha][03] := TRC->CD_VISITANTE                                                                              //C
...
Static Function SalvaXml()
	...
	oExcel:Activate()
	oExcel:GetXMLFile("C:\temp\REL_VISI_REFEITORIO.XML")
...
Static Function CriaExcel()              
	...
	oMsExcel:WorkBooks:Open("C:\temp\REL_VISI_REFEITORIO.XML")
...
//	Private cPyme	:= Nil
//	
//    U_xPutSx1(cPerg,'01','Visitante de  ?','','','mv_ch1','C',10,0,0,'G',bValid,"SRA",cSXG,cPyme,'MV_PAR01')
//	U_xPutSx1(cPerg,'02','Visitante Ate ?','','','mv_ch2','C',10,0,0,'G',bValid,"SRA",cSXG,cPyme,'MV_PAR02')
...
Static Function SqlGeral() 
	...
	cQuery += "   AND CD_VISITANTE        >= '" + MV_PAR01 + "' "
	cQuery += "   AND CD_VISITANTE        <= '" + MV_PAR02 + "' "
	cQuery += "   AND CD_VISITANTE        IS NOT NULL "   
	...
	cQuery += "    OR TP_EVENTO            = '23') " // Acesso Batch
	cQuery += "   ORDER BY CD_LOG_ACESSO   " // Acesso Batch

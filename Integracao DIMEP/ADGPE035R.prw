...
/*{Protheus.doc} User Function ADGPE035R
...
User Function ADGPE035R()
	...
	AADD(aButtons, { 1,.T.,{|o| nOpca:=1, o:oWnd:End(), Processa({||GPEADGPE035R()},"Gerando arquivo","Aguarde...")    }})
...
Static Function GPEADGPE035R()    

	PRIVATE oExcel     := FWMSEXCEL():New()
	PRIVATE cPath      := ''
	PRIVATE cArquivo   := 'REL_TEMPO_PERMANENCIA_FUNCIONARIOS_DIMEP.XML'
	PRIVATE oMsExcel
	PRIVATE cPlanilha  := DTOC(MV_PAR03) + ' ate ' + DTOC(MV_PAR04)
    PRIVATE cTitulo    := "Relatorio Tempo de Permanência de Funcionários - " + DTOC(MV_PAR03) + ' ate ' + DTOC(MV_PAR04)
    PRIVATE aLinhas    := {}
...
Static Function SalvaXml()
	...
	oExcel:Activate()
	oExcel:GetXMLFile("C:\temp\REL_TEMPO_PERMANENCIA_FUNCIONARIOS_DIMEP.XML")
...
Static Function CriaExcel()              
	...
	oMsExcel:WorkBooks:Open("C:\temp\REL_TEMPO_PERMANENCIA_FUNCIONARIOS_DIMEP.XML")
...
//	Private cPyme	:= Nil
//	
//    U_xPutSx1(cPerg,'01','Funcionario de  ?','','','mv_ch1','C',11,0,0,'G',bValid,cF3 ,cSXG,cPyme,'MV_PAR01')
//	U_xPutSx1(cPerg,'02','Funcionario Ate ?','','','mv_ch2','C',11,0,0,'G',bValid,cF3 ,cSXG,cPyme,'MV_PAR02')
...
Static Function SqlEntrada(cMat,cData)
    ...
    BeginSQL Alias "TRB"
			%NoPARSER%
			 SELECT TOP(1) LOG_ACESSO.CD_LOG_ACESSO AS CD_LOG_ACESSO_ENTRADA,
					        LOG_ACESSO.NU_CREDENCIAL,
					        LOG_ACESSO.TP_AUTENTICACAO,
					        LOG_ACESSO.NU_MATRICULA,
					        LOG_ACESSO.NM_PESSOA,
					        LOG_ACESSO.NM_ESTRUTURA,
					        LOG_ACESSO.DS_GRUPO,
					        LOG_ACESSO.DS_EQUIPAMENTO,
					        CASE WHEN LOG_ACESSO.TP_SENTIDO_CONSULTA = 1 THEN 'ENTRADA' ELSE 'SAIDA'END AS TP_SENTIDO_CONSULTA,
					        CONVERT(VARCHAR,LOG_ACESSO.DT_REQUISICAO, 120) AS DT_REQUISICAO,
					        LOG_ACESSO.NU_DATA_REQUISICAO,
					        LOG_ACESSO.CD_TIPO_CREDENCIAL,
					        DS_TIPO_CREDENCIAL,
					        LOG_ACESSO.CD_ESTRUTURA,
					        LOG_ACESSO.NM_ESTRUTURA
					   FROM [DMPACESSO].[DMPACESSOII].[DBO].[LOG_ACESSO] AS LOG_ACESSO 
					   INNER JOIN [DMPACESSO].[DMPACESSOII].[DBO].[ESTRUTURA_ORGANIZACIONAL]
						       ON CD_ESTRUTURA_ORGANIZACIONAL = CD_ESTRUTURA
						      AND CD_ESTRUTURA_RELACIONADA    = %EXP:nFil%
			 ...
			 SELECT TOP(1) CD_LOG_ACESSO AS CD_LOG_ACESSO_SAIDA,
					  ...
					  WHERE NU_DATA_REQUISICAO   = %EXP:cData%
						...
						AND (CD_EQUIPAMENTO      = 4 //Catraca Portaria
						 ...
						 OR CD_EQUIPAMENTO       = 21) // 21-Balança de Saida
						AND (TP_EVENTO           = '27' // Acesso Master
						 OR TP_EVENTO            = '10' // Acesso Concluido
						 OR TP_EVENTO            = '12' // Acesso Batch*/
						 OR TP_EVENTO            = '23')  // Acesso por autorização
			...
			%NoPARSER%
			 SELECT TOP(1) CD_LOG_ACESSO AS CD_LOG_ACESSO_SAIDA,
					  ...
					  WHERE NU_DATA_REQUISICAO   = %EXP:cData%
						...
						AND (CD_EQUIPAMENTO      = 4 //Catraca Portaria
						 OR CD_EQUIPAMENTO       = 6 //Cancela Saida Portaria
						 OR CD_EQUIPAMENTO       = 21) // 21-Balança de Saída
						AND (TP_EVENTO           = '27' // Acesso Master
						 OR TP_EVENTO            = '10' // Acesso Concluido
						 OR TP_EVENTO            = '12' // Acesso Batch*/
						 OR TP_EVENTO            = '23')  // Acesso por autorização

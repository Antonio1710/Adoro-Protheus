...
/*/{Protheus.doc} User Function XmlNfeSef
	Rdmake de- exemplo para geracao da Nota Fiscal Eletronica do
	...
	@since 13.02.2007
	@version 01
	@history chamado Interno- Adriana					- 15/02/2019 - Alteração do Tamanho do Vetor appdifal      
	@history chamado 047725 - Adriana					- 07/03/2019 - Alteracao local de entrega para Consignee   
	@history chamado 047943 - Fernando Sigoli - 15/03/2019 - Conforme solilcitação do SIF, necessario ajustar a mensagem da Nota. De LACRE SIF  para LACRE OFICIAL
	@history chamado 048333 - Adriana					- 25/04/2019 - Apresentar mensagem quando houver valor de IPR ou SENAR                                
	@history chamado 049014 - Abel						- 07/05/2019 - Acrescenta mensagem em todas as NF          
	@history chamado 049099 - Adriana 				- 17/05/2019 - Compatibiliza versao Totvs NFESEFAZ 10/04/19
	@history chamado 049388 - Adriana  				- 23/05/2019 - Correçao TAG ICMSUFDest, AliquotaInter para Erro XML: invalid property _ALIQUOTAINTERTAG
	@history chamado 044314 - Everson					- 03/06/2019 - Adicionado valor de frete na tag de observação adicionais.                             
	@history chamado 044314 - Everson					- 11/06/2019 - Removida tag de valor de frete  de observação adicionais.                             
	@history chamado 044314 - Everson					- 14/06/2019 - Adicionado valor de frete na tag de observação adicionais.                             
	@history chamado 044314 - Everson					- 26/06/2019 - Adicionado apólice de seguro na tag deobservação adicionais.                      
	@history chamado 044314 - Everson					- 06/08/2019 - Adicionado validação de empresa para gerartag de valor de frete.                      
	@history chamado 044314 - Adriana					- 06/08/2019 - Retirar o SN duplicado no endereco doemitente                                    
	@history chamado 051408 - Adriana  				- 30/08/2019 - se cliente utiliza DUN 14 no XML (MV_#DUN14) imprime B1_CODGTIN ao inves de B1_CODBAR    
	@history chamado 044314 - Adriana  				- 09/09/2019 - para alterar data saida na TAG dhSaiEnt conforme Parâmetros MV_#FILDTS e MV_#HRDTSMV
	@history chamado 054941 - Everson					- 16/01/2020 - Tratamento para adicionar apólice de segura para os demais tipos de cargas.
	@history chamado 050729 - FWNM 						- 15/07/2020 - || OS 052035 || TECNOLOGIA || LUIZ || 8451 || REDUCAO DE BASE
	@history chamado 060170 - Adriana 				- 04/08/2020 - incluida função Upper, para nao duplicar mensagem, apos habilitar MV_NFEMSF4=C
	...
	@history Ticket 78894  - Abel Babini             - 29/08/2022 - Valida se for exportação para não transmitir Notas sem a informação de lacre
	@history Ticket 69334  - Abel Babini             - 13/09/2022 - Ajusta criação de mensagens para NFE baseada nas formulas.
	@history Ticket 69334  - Abel Babini             - 21/09/2022 - Ajusta criação de mensagens para NFE para Filial Itapira
	@history Ticket 80379  - Everson				 - 30/09/2022 - Adicionada a placa do transbordo na mensagem da nota.
	@history Ticket 80379  - Everson				 - 25/10/2022 - Adicionada a placa do transbordo na mensagem da nota.
	@history Ticket TI     - Everson				 - 13/03/2023 - Correção errorlog.
...
User Function XmlNfeSef(cTipo,cSerie,cNota,cClieFor,cLoja,cNotaOri,cSerieOri)
	...
	//Local LRespTec  := SuperGetMV("MV_RESPTEC", ,.T.) // chamado 050729 - FWNM - 15/07/2020 - || OS 052035 || TECNOLOGIA || LUIZ || 8451 || REDUCAO DE BASE
	Local lNfCupZero	:= .F.
	Local lRural		:= .F.
	Local lSeekOk   	:= .F.
	Local lDifParc		:= .F.
	...
	Local cDesc 	:= ""
	Local cDesc1 	:= ""
	Local _cDUN14 	:= GETMV("MV_#DUN14",,"")  	// Incluido em 30/08/19 por Adriana para verificar se cliente utiliza DUN 14 no XML da NF - chamado 051408
	Local _cM57608 	:= GETMV("MV_#M57608",,"165")  	// Ticket 692 por Adriana em 04/09/2020
	Local _i				:= 0 //Ticket     TI	- Abel Babini			- 04/12/2020 - Validação de posicionamento nas empresas e filiais corretas para geração do XML.
					
	Private aPedCli   		:= {}
	Private _lVendaOrdem	:= .f.
	Private cRedeOP		    := GetMV("MV_#REDEOP") //Redes as quais é obrigatorio preencher Operador Logistico, ou Filial, e numero do pedido do cliente, em nosso Pedido de Venda (SAPORE / MASSIMA)- por Adriana chamado 031238 em novembro/2016
	Private cEmlKob         := GETMV("MV_#EMLKOB",,'documentos@kobold.com.br;reginaldo@adoro.com.br;alberto@adoro.com.br')
	// Incluido em 09/09/2019 por Adriana para alterar data saida na TAG dhSaiEnt chamado 044314
	Private _cSaiEnt		:= ""								
	Private _cFilDtS		:= GETMV("MV_#FILDTS",,"") 			
	Private _HrDtS			:= GETMV("MV_#HRDTS",,"20:00:00") 	
	//
	...
	// Especifico Adoro
	Private _cEnderEmit  	:= "" 	// incluido em 06/08/2019 por Adriana - chamado 044314
	Private _cA1_MENSAGE 	:= "" 	// Incluido em 01/06/17 por Adriana para considerar mensagem indicada no campo A1_MENSAGE - chamado 035300	
	Private _lDUN14				:= .F.  // Incluido em 30/08/19 por Adriana para verificar se cliente utiliza DUN 14 no XML da NF - chamado 051408       
	...
	Private _cEmpAt1 := SuperGetMv("MV_#EMPAT1",.F.,"01/13") //Codigo de Empresas Ativas Grupo 1 //ticket TI - Antonio Domingos - 13/05/2023
	lStsNFAd := .T.
	...
	//INICIO Ticket     TI	- Abel Babini			- 04/12/2020 - Validação de posicionamento nas empresas e filiais corretas para geração do XML.
	For _i := 1 to Len(_aX5nNF)
		If Alltrim(_aX5nNF[_i,3]) == Alltrim(cSerie)
			_cUltNFe := Alltrim(_aX5nNF[_i,4])
			EXIT
		Endif
	Next _i

	IF Alltrim(cNota) > Alltrim(_cUltNFe)
		u_GrLogZBE(Date(),TIME(),cUserName,"01-ERRO NO FATURAMENTO. EMPRESA/FILIAL DIVERGENTE","FATURAMENTO","NFESEFAZ",;
	...
	//FIM Ticket     TI	- Abel Babini			- 04/12/2020 - Validação de posicionamento nas empresas e filiais corretas para geração do XML.
		...
		For nCountNF := 1 To Len(aCupRefLoj)
			...
			If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
				...
				ElseIf FunName() == "SPEDNFSE"
					...
					If (Alltrim(SA1->A1_BCO1)=="KOB")
						aadd(aDest,ConvType(SA1->A1_EMAIL)+";"+cEmlKob) //fernando Sigoli 16/05/2018
					Else
						aadd(aDest,ConvType(SA1->A1_EMAIL)) //ADICIONADO CONVTYPE POR WILLIAM COSTA 05/07/2017 CHAMADO 036081 
					Endif	
					//
				...
				Else
					...
					If !SF2->F2_TIPO $ "DB" 
						...
						*/
						If !lUsaCliEnt
							...
							EndIf
							// Especifico Adoro
							_cA1_MENSAGE := iif(!Empty(SA1->A1_MENSAGE),SA1->(Formula(A1_MENSAGE)),"") //	Incluido em 01/06/17 por Adriana para considerar mensagem indicada no campo A1_MENSAGE - chamado 035300					
							//
						Else
						...
						//Especifico Adoro
						//aadd(aDest,SA1->A1_EMAIL)
						//aadd(aDest,SA1->A1_SUFRAMA) // RICARDO LIMA - 27/07/2018 | ITEM EM DUPLICIDADE
						If (Alltrim(SA1->A1_BCO1)=="KOB")
							//aadd(aDest,ConvType(SA1->A1_EMAIL)+";documentos@kobold.com.br;sistemas@adoro.com.br;reginaldo@adoro.com.br") //ADICIONADO CONVTYPE POR WILLIAM COSTA 05/07/2017 CHAMADO 036081
							aadd(aDest,ConvType(SA1->A1_EMAIL)+";"+cEmlKob) //fernando Sigoli 16/05/2018 // 16
						Else   
							aadd(aDest,ConvType(SA1->A1_EMAIL)) //ADICIONADO CONVTYPE POR WILLIAM COSTA 05/07/2017 CHAMADO 036081
						Endif
						//
					...
					//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
					//Especifico Adoro 
					// Trecho inclusao noturno - Alex Borges
							
					If !Empty( SF2->F2_PLACA ) //.And. cEmpAnt $ _cEmpAt1  Desabilitada condicao para executar bloco para Ceres igual Adoro em 21/02/2017 por Adriana chamado 033526// &&Mauricio - 29/03/10 Tratamento para NFE Ceres.         
					
						dbSelectArea( "ZV4" )
						dbSetOrder( 01 )
					
						If ZV4->( dbSeek( xFilial( "ZV4" ) + SF2->F2_PLACA ) )

							aAdd( aVeiculo, AllTrim( ZV4->ZV4_PLACA ) )
							aAdd( aVeiculo, AllTrim( ZV4->ZV4_EST ) )
							aAdd( aVeiculo, "" ) //RNTC
							...
							If SA2->( dbSeek( xFilial( "SA2" ) + ZV4->ZV4_FORNEC + ZV4->ZV4_LOJFOR ) )

								aAdd( aTransp, AllTrim( SA2->A2_CGC ) )
								aAdd( aTransp, AllTrim( SA2->A2_NOME ) )
								aAdd( aTransp, VldIE( SA2->A2_INSCR ) )
								aAdd( aTransp, AllTrim( SA2->A2_END ) + "," + AllTrim( SA2->A2_NR_END ) )
								aAdd( aTransp, AllTrim( SA2->A2_MUN ) )
								aAdd( aTransp, AllTrim( SA2->A2_EST ) )
							...
							*/
							...
							Else
								dbSelectArea("SA4")
								dbSetOrder(1)
								MsSeek(xFilial("SA4")+ZV4->ZV4_FORNEC)
								if found()
									aadd(aTransp,AllTrim(SA4->A4_CGC))
									aadd(aTransp,SA4->A4_NOME)
									aadd(aTransp,VldIE(SA4->A4_INSEST))
							...
							Endif   					
						EndIf
	/*				Elseif cEmpAnt == "02" .And. !Empty( SF2->F2_PLACA ) .And. !Empty(SF2->F2_ESTPLAC) && Mauricio 05/04/10 - Dados transportadora Ceres.
					aAdd( aVeiculo, AllTrim( SF2->F2_PLACA ) )
					aAdd( aVeiculo, AllTrim( SF2->F2_ESTPLAC ) )  //Est
					aAdd( aVeiculo, "" )  //RNTC
	...
	*/				
					ELSE // PEGA A TRANSPORTADORA NOS CASOS DAS NOTAS DE SAIDA DE SC 			
				
						dbSelectArea( "SA2" )
						dbSetOrder( 01 )
						
						If SA2->( dbSeek( xFilial( "SA2" ) + SF2->F2_TRANSP,.T.) )
					...
					//Parte do fonte padrão para tratar veiculo relacionado a nota fiscal através dos campos na tabela SF2 e DA3 desabilitado pois a 
					//Adoro não utiliza estes campos
					// Fim trecho noturno - Alex Borges */
					//
					...
					If MsSeek(cChave)
						...
						EndIf
						//Especifico Adoro      
						//Verifica se é Venda Ordem     
						If AllTrim(SF3->F3_CFO) == "5118" .Or. AllTrim(SF3->F3_CFO) == "6118"
							_lVendaOrd  := .T.
						else
							_lVendaOrd  := .F.
						Endif
						...
						EndIf
						// Busca valor da redução do total da nota, referênte ao 
					...
					#IFDEF TOP
						...
						IF cVerAmb >= "4.00" .And. (ExistTemplate("PROCMSG")) //Tratativa para Grupo de Repasse de Combustiveis
							BeginSql Alias cAliasSD2
									...
									,D2_SEGUM,D2_QTSEGUM %Exp:cField% //Especifico Adoro
									FROM %Table:SD2% SD2
									WHERE
					...
					While !Eof() .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
						...
						If lContinua
							...
							//Especifico Adoro desabilita linhas abaixo
							...
							dbSelectArea("SX5")
							dbSetOrder(1)
							dbSeek(xFilial("SX5")+"S2"+"00")
							If SF4->F4_SITTRIB == "00"
								cDesc:=Upper(AllTrim(SubStr(SX5->X5_DESCRI,1,55)))
							Endif 
							
							dbSelectArea("SX5")
							dbSetOrder(1)
							dbSeek(xFilial("SX5")+"S2"+"10")
							...
							dbSelectArea("SX5")
							dbSetOrder(1)
							dbSeek(xFilial("SX5")+"S2"+"20")
							If SF4->F4_SITTRIB == "20"
								cDesc:=Upper(AllTrim(SubStr(SX5->X5_DESCRI,1,55)))
							Endif  
					
							dbSelectArea("SX5")
							dbSetOrder(1)
							dbSeek(xFilial("SX5")+"S2"+"30")
							...
							dbSelectArea("SX5")
							dbSetOrder(1)
							dbSeek(xFilial("SX5")+"S2"+"40")
							If SF4->F4_SITTRIB == "40"
								cDesc:=Upper(AllTrim(SubStr(SX5->X5_DESCRI,1,55)))
							Endif
						
							dbSelectArea("SX5")
							dbSetOrder(1)
							dbSeek(xFilial("SX5")+"S2"+"41")
							If SF4->F4_SITTRIB == "41"
								cDesc:=Upper(AllTrim(SubStr(SX5->X5_DESCRI,1,55)))
							Endif   
					
							dbSelectArea("SX5")
							dbSetOrder(1)
							dbSeek(xFilial("SX5")+"S2"+"50")
							If SF4->F4_SITTRIB == "50"
								cDesc:=Upper(AllTrim(SubStr(SX5->X5_DESCRI,1,55)))
							Endif
					
							dbSelectArea("SX5")
							dbSetOrder(1)
							dbSeek(xFilial("SX5")+"S2"+"51")
							If SF4->F4_SITTRIB == "51"
								cDesc:=Upper(AllTrim(SubStr(SX5->X5_DESCRI,1,55)))
							Endif                    
					
							dbSelectArea("SX5")
							dbSetOrder(1)
							dbSeek(xFilial("SX5")+"S2"+"60")
							If SF4->F4_SITTRIB == "60"
								cDesc:=Upper(AllTrim(SubStr(SX5->X5_DESCRI,1,55)))
							Endif      
					
							dbSelectArea("SX5")
							dbSetOrder(1)
							dbSeek(xFilial("SX5")+"S2"+"70")
							If SF4->F4_SITTRIB == "70"
								cDesc:=Upper(AllTrim(SubStr(SX5->X5_DESCRI,1,55)))
							Endif  
					
							dbSelectArea("SX5")
							dbSetOrder(1)
							dbSeek(xFilial("SX5")+"S2"+"90")
							If SF4->F4_SITTRIB == "90"
								cDesc:=Upper(AllTrim(SubStr(SX5->X5_DESCRI,1,55)))
							Endif
							...
							If !Empty((SB1->B1_ORIGEM))
								cDesc1:=Upper(AllTrim(SubStr(SX5->X5_DESCRI,1,55)))
							Endif                  
					
							cComplet:= "PRODUTO "+ cDesc1+" "+cDesc
							
							If !Empty(SF4->F4_SITTRIB) .and. !ALLTRIM(cComplet) $ ALLTRIM(cMsgTes)
								If cMsgTes <> ""
									cMsgTes+= cComplet
									cMsgNf1+= cComplet+" / "
								Else
							...
							If !Empty(SF4->F4_ADFORM)
				
								dbSelectArea("SM4")
								dbSetOrder(1)
								dbSeek(xFilial("SM4")+SF4->F4_ADFORM)
								

								//If !ALLTRIM((SM4->M4_FORMULA)) $ ALLTRIM(cFormul)
								//INCLUIDO "UPPER" 28/03/11 - ANA
								//Ticket 69334  - Abel Babini             - 13/09/2022 - Ajusta criação de mensagens para NFE baseada nas formulas.
								If !UPPER(ALLTRIM((SM4->M4_FORMULA))) $ UPPER(ALLTRIM(cFormul))				    
								// If !UPPER(ALLTRIM((FORMULA(SM4->M4_CODIGO)))) $ UPPER(ALLTRIM(cFormul))				    
									If cFormul <> ""
										cFormul+= Upper(AllTrim(SM4->M4_FORMULA))
										cMsgNf1+= Upper(AllTrim(SM4->M4_FORMULA)) 
										// cFormul+= Upper(AllTrim(FORMULA(SM4->M4_CODIGO)))
										// cMsgNf1+= Upper(AllTrim(FORMULA(SM4->M4_CODIGO))) 
									Else
										cFormul+= " / "+ Upper(AllTrim(SM4->M4_FORMULA))
										cMsgNf1+= " / "+ Upper(AllTrim(SM4->M4_FORMULA))  
										// cFormul+= " / "+ Upper(AllTrim(FORMULA(SM4->M4_CODIGO)))
										// cMsgNf1+= " / "+ Upper(AllTrim(FORMULA(SM4->M4_CODIGO)))  
									Endif	 
									//Inicio Ticket 692 por Adriana em 04/09/2020
									if SF4->F4_ADFORM = _cM57608
										cFormul+= ", Processo Nr. "+U_TrazN57608(SD2->D2_CLIENTE, SD2->D2_LOJA, SD2->D2_EMISSAO)
										cMsgNf1+= ", Processo Nr. "+U_TrazN57608(SD2->D2_CLIENTE, SD2->D2_LOJA, SD2->D2_EMISSAO) +" / "
									Else
										cMsgNf1+= " / "
									Endif
									//Fim Ticket 692 por Adriana em 04/09/2020
								Endif
							Endif
							
							If !Empty(SF4->F4_ADFORM2)
				
								dbSelectArea("SM4")
								dbSetOrder(1)
								dbSeek(xFilial("SM4")+SF4->F4_ADFORM2)
									...
									If ( MsSeek(xFilial("SD1")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+PADL(alltrim((cAliasSD2)->D2_ITEMORI),TamSx3("D2_ITEMORI")[1],"0")) ) .OR. ;
										...
										if AllTrim(SF1->F1_ESPECIE)<>"NFP"
											...
											If ( ( cAliasSD2 )->D2_COD == SD1->D1_COD .AND. SF4->F4_AJUSTE == "S" )	
												...
												//FONTE PADRAO  aAdd( aNfVinc, { SD1->D1_EMISSAO, SD1->D1_SERIE, SD1->D1_DOC, iIf( SD1->D1_TIPO $ "DB", iIf( SD1->D1_FORMUL == "S", SM0->M0_CGC, SA1->A1_CGC ), iIf( SD1->D1_FORMUL == "S", SM0->M0_CGC, SA2->A2_CGC ) ), SM0->M0_ESTCOB, SF1->F1_ESPECIE, SF1->F1_CHVNFE,SD1->D1_TOTAL-SD1->D1_DESC,"",SF1->F1_TIPO} )
												
												//Adoro
												aAdd( aNfVinc, { SD1->D1_EMISSAO, SD1->D1_SERIE, SD1->D1_DOC, iIf( SD1->D1_TIPO $ "DB", iIf( SD1->D1_FORMUL == "S", SM0->M0_CGC, SA1->A1_CGC ), iIf( SD1->D1_FORMUL == "S", SM0->M0_CGC, SA2->A2_CGC ) ), SM0->M0_ESTCOB, SF1->F1_ESPECIE, SF1->F1_CHVNFE,/*INICIO ESPECIFICO ADORO CHAMADO 027912 GRINGOLETTO*/ Posicione("SD1",1,xFilial("SD2")+( cAliasSD2 )->D2_NFORI+( cAliasSD2 )->D2_SERIORI+SA2->A2_COD+SA2->A2_LOJA+( cAliasSD2 )->D2_COD,"D1_TOTAL") - Posicione("SD1",1,xFilial("SD2")+( cAliasSD2 )->D2_NFORI+( cAliasSD2 )->D2_SERIORI+SA2->A2_COD+SA2->A2_LOJA+( cAliasSD2 )->D2_COD,"D1_DESC ") /*FINAL ESPECIFICO ADORO CHAMADO 027912 GRINGOLETTO*/,"",SF1->F1_TIPO} )
												//
							...
							//Especifico Adoro
							//Ch. 049014 || ABEL BABINI || DEJAIME || FISCAL || ACRESCENTAR MENSAGEM EM TODAS AS NOTAS
							cMensCli := "CONFERIR A MERCADORIA NO ATO DA ENTREGA - PRODUTO, QUANTIDADE E VALOR. NÃO ACEITAMOS RECLAMAÇÕES POSTERIORES.                                          "
							If !Empty(SC6->C6_NUM)
								If Alltrim(SC5->C5_TIPO) $ "D/B"
									cMensCli += "Telefone Cobranca: 11 4596-8450" + "   Pedido: " + AllTrim(SC6->C6_NUM) + " Fornecedor: " + AllTrim(SC5->C5_CLIENTE) +"-"+ AllTrim(SC5->C5_LOJACLI) //chamado: 044910 - Fernando Sigoli 01/11/2018 
								Else
							//		Alterado em 01/06/17 por Adriana para considerar mensagem indicada no campo A1_MENSAGE - chamado 035300					
							//		cMensCli := "Telefone Cobranca: 11 4596-8450" + "   Pedido: " + AllTrim(SC6->C6_NUM) + " Cliente: " + AllTrim(SC5->C5_CLIENTE)
									cMensCli += _cA1_MENSAGE+"Telefone Cobranca: 11 4596-8450" + "   Pedido: " + AllTrim(SC6->C6_NUM) + " Cliente: " + AllTrim(SC5->C5_CLIENTE) +"-"+ AllTrim(SC5->C5_LOJACLI) //chamado: 044910 - Fernando sigoli 01/11/2018 
								Endif	
							Endif
					
							If !Empty(SF2->F2_ROTEIRO)
								cMensCli += " Roteiro: "+SF2->F2_ROTEIRO	
							Endif
						
							If !Empty(SF2->F2_SEQUENC)	
								cMensCli += " Seq.: "+SF2->F2_SEQUENC+" "					
							EndIf        
							...
							_cSaiEnt := iif(cFilAnt $ _cFilDtS .and. TIME() >= _HrDtS, ConvType(SF2->F2_EMISSAO+1)+"T00:01:00", "") // Incluido em 09/09/2019 por Adriana para alterar data saida na TAG dhSaiEnt chamado 044314
							...
							//Especifico Adoro 
							//Incluido 29/03/11 por ANA - Adoro
							If cEmpAnt $ _cEmpAt1   &&Mauricio 29/03/10 - Tratamento para NF Ceres. //ticket TI - Antonio Domingos - 13/05/2023
								// TRATAMENTO DE DADOS DE GRANJA DA ADORO SC 
								If AllTrim( xFilial( "SF2" ) ) == "03" 
									if!Empty(Alltrim(SC5->C5_GRANJA))
										cMensCli+= PADR("Granja "+Alltrim(SC5->C5_GRANJA ),70)   
									endif
									if!Empty(Alltrim(SC5->C5_GRANJA))
										cMensCli+= PADR("Granjada "+Alltrim(SC5->C5_GRANJDA),70)   
									endif
									if!Empty(Alltrim(SC5->C5_GRANJA))
										cMensCli+= PADR("Fase "+Alltrim(SC5->C5_FASE) ,70)   
									endif
								EndIf					  
								// FIM TRATAMENTO DE DADOS DE GRANJA DA ADORO SC				 
							Endif						   	              
					
							If !empty(SC5->C5_MENNOT2) .or. !empty(SC5->C5_MENNOT3) .or. !empty(SC5->C5_MENNOT4) .or. !empty(SC5->C5_MENNOT5)  
								cMensCli += " "+AllTrim(SC5->C5_MENNOT2)+" "+AllTrim(SC5->C5_MENNOT3)+" "+AllTrim(SC5->C5_MENNOT4) +" "+AllTrim(SC5->C5_MENNOT5) 									
							Endif
									
							If SC5->C5_TIPOCLI == 'X'  		
								//Ticket 78894  - Abel Babini             - 29/08/2022 - Valida se for exportação para não transmitir Notas sem a informação de lacre
								IF Empty(ALLTRIM(SF2->F2_NLACRE2)) .AND. Empty(ALLTRIM(SF2->F2_TEMPERA)) .AND. Empty(ALLTRIM(SF2->F2_DTPROD))
									u_GrLogZBE(Date(),TIME(),cUserName,"04-ERRO NA EXPORTAÇÃO. LACRE, TEMPERATURA E DATA DE PRODUÇÃO EM BRANCO","FATURAMENTO","NFESEFAZ",;
												"NF: "+cNota+" Serie: "+cSerie+" Ultima NF: "+_cUltNFe+" M0_CODIGO: "+SM0->M0_CODIGO+" M0_CODFIL: "+SM0->M0_CODFIL+" cEmpAnt/cFilAnt: "+cEmpAnt+cFilAnt,ComputerName(),LogUserName())
									MsgAlert("(03) Erro nos campos de Lacre, Temperatura e data de produção e a NF NÃO será transmitida. Reinicie o Protheus e tente novamente, e se o problema persistir fale com a TI.","Erro na Transmissão")
									Return({cNfe,EncodeUTF8(cString),cNotaOri,cSerieOri})
								ENDIF
								//ENDIF Ticket 78894  - Abel Babini             - 29/08/2022 - Valida se for exportação para não transmitir Notas sem a informação de lacre
								...
								DbSelectArea("SM2")       	
								DbSetOrder(1)                 
								DbSeek(DtoS(SF2->F2_EMISSAO))       
								//Alterado por Adriana em 30/01/2018 para tratar outras moedas	
								//cMensCli += ",           PTAX: R$ "+STR(SM2->M2_MOEDA2,6,4) 
								cMensCli += ",  PTAX: R$ "+STR((&("M2_MOEDA"+STR(SC5->C5_MOEDA,1))),6,4) 
								DbSelectArea("EE7")													
								//DbSetOrder(10)                                             
								DBORDERNICKNAME("EE7NFE")   //alterado para Adoro por Adriana em 17/01/2018 ajuste pós migraçao v12                                          
								//DbSeek(xFilial("EE7")+SF2->F2_CLIENTE+SC5->C5_NUM)       
								DbSeek(xFilial("EE7")+SC5->C5_NUM)       
								cMensCli += ", Proc. Export.: "+ALLTRIM(EE7_PEDIDO)		
							
								lTipoCli	:= .F.
												
							ENDIF 
					
							/* alteração  chamado Adoro N° 019258 referente ao endereço de entrega quando NF de Beneficiamento */

							If !SF2->F2_TIPO $ "DB"
											
								If !_lVendaOrd //Se não for Venda Ordem
									If Alltrim(SA1->A1_SATIV1)="50" .and. Alltrim(SA1->A1_SATIV2)$"51,52,53"
										cMenscli += PADR(" LOCAL DE ENTREGA CONF. PORT. CAT 37/02-COZINHA INDUSTRIAL ",70)
										cMenscli += Alltrim(SA1->A1_RAZENT)+ " "+Alltrim(SA1->A1_ENDENT)+" - "+Alltrim(SA1->A1_BAIRROE)+" "+Alltrim(SA1->A1_MUNE)+" - "+Alltrim(SA1->A1_ESTE)+" "+PADR(Transform(SA1->A1_CEPE,"@R 99999-999")+" CNPJ: " +SA1->A1_CGCENT,70) 
									ElseIf SA1->A1_IMPENT = "S" .and. SA1->A1_EST <> 'EX' //Adoro  para endereço de entrega diferente e cozinha industrial //Alterado por Adriana em 07/03/2019 chamado 047725 incluido condicao EX   
										cMenscli += " LOCAL DE ENTREGA: "
										cMenscli += Alltrim(SA1->A1_RAZENT)+ " "+Alltrim(SA1->A1_ENDENT)+" - "+Alltrim(SA1->A1_BAIRROE)+" "+Alltrim(SA1->A1_MUNE)+" - "+Alltrim(SA1->A1_ESTE)+" "+PADR(Transform(SA1->A1_CEPE,"@R 99999-999")+" CNPJ: " +SA1->A1_CGCENT,70) 
									ElseIf SA1->A1_EST = 'EX' //Adoro  para exportacao - incluido por Adriana em 07/03/2019 chamado 047725 - para atender SIF     
										cMenscli += " CONSIGNEE: "
										cMenscli += Alltrim(SA1->A1_NOME)+" - "+Alltrim(SA1->A1_END)+" "+Alltrim(SA1->A1_COMPLEM)+" - "+Alltrim(SA1->A1_BAIRRO)+" - "+Alltrim(SA1->A1_MUN)+" - "+Alltrim(SA1->A1_EST)				
									Else
										cMenscli += " LOCAL DE ENTREGA: "
										cMenscli += Alltrim(SA1->A1_END)+" - "+Alltrim(SA1->A1_BAIRRO)+" - "+Alltrim(SA1->A1_MUN)+" - "+Alltrim(SA1->A1_EST)				
									Endif				
								Endif
								//Ticket 69334  - Abel Babini             - 21/09/2022 - Ajusta criação de mensagens para NFE baseada nas formulas.
								IF Alltrim(cFilAnt)  == "0B" .AND. !" MERCADORA SERA RETIRADA DO ARMAZEM JAMPAC IND COM ALIM LTDA, RUA MAURO SIMOES, 440 ITAPIRA, SP" $ cMenscli
										cMenscli += " MERCADORA SERA RETIRADA DO ARMAZEM JAMPAC IND COM ALIM LTDA, RUA MAURO SIMOES, 440 ITAPIRA, SP"
										cMenscli += " CNPJ 20.844.980/0001-16 E INSC. ESTADUAL 374.068.543.113-CONFORME ANEXO VII, ARTIGO 8 DO RICMS-SP"									
								Endif
								...
								MsSeek(xFilial("SA2")+SF2->F2_CLIENTE+SF2->F2_LOJA)
								cMenscli += " LOCAL DE ENTREGA: "
								cMenscli += Alltrim(SA2->A2_END)+" - "+Alltrim(SA2->A2_BAIRRO)+" - "+Alltrim(SA2->A2_MUN)+" - "+Alltrim(SA2->A2_EST)
							
							endif

							//Everson - 13/03/2023.
							//Everson - 25/10/2022.
							If FieldPos("C5_XPAGRUP") > 0 .And. ! Empty(SC5->C5_XPAGRUP)
								cEndereço := Alltrim(cValToChar(Posicione("ZHZ", 1, FWxFilial("ZHZ") + SC5->C5_XRAGRUP, "ZHZ_ENDERE")))
								cMensCli  += " Local de movimentação da mercadoria: " + cEndereço + " "
								cMensCli  += " Placa: " + SC5->C5_PLACA + " "

							EndIf

							//Fim mensagens customizadas Adoro
							...
							//Inicio: Especifico Adoro - Alterado por Adriana em 04/08/2020 - chamado 060170 - incluido Upper para nao duplicar mensagem apos habilitar MV_NFEMSF4=C
							If SF4->(ColumnPos("F4_FORINFC") ) > 0  .And. !Empty(SF4->F4_FORINFC) .And. Formula(SF4->F4_FORINFC) <> NIL .And. ( ( cMVNFEMSF4=="C" .And. !AllTrim(UPPER(Formula(SF4->F4_FORINFC))) $ cMensCli ) .Or. (cMVNFEMSF4=="F" .And. !AllTrim(UPPER(Formula(SF4->F4_FORINFC)))$cMensFis) )
								If cMVNFEMSF4=="C"
									If Len(cMensCli) > 0 .And. SubStr(cMensCli, Len(cMensCli), 1) <> " "
										cMensCli += " "
									EndIf
									cMensCli	+=	UPPER(SF4->(Formula(F4_FORINFC)))
								ElseIf cMVNFEMSF4=="F"
									If Len(cMensFis) > 0 .And. SubStr(cMensFis, Len(cMensFis), 1) <> " "
										cMensFis += " "
									EndIf
									cMensFis	+=	UPPER(SF4->(Formula(F4_FORINFC)))
								EndIf
							ElseIf !Empty(SF4->F4_FORMULA) .And. Formula(SF4->F4_FORMULA) <> NIL .And. ( ( cMVNFEMSF4=="C" .And. !AllTrim(UPPER(Formula(SF4->F4_FORMULA))) $ cMensCli ) .Or. (cMVNFEMSF4=="F" .And. !AllTrim(UPPER(Formula(SF4->F4_FORMULA)))$cMensFis) )
								If cMVNFEMSF4=="C"
									If Len(cMensCli) > 0 .And. SubStr(cMensCli, Len(cMensCli), 1) <> " "
										cMensCli += " "
									EndIf
									cMensCli	+=	UPPER(SF4->(Formula(F4_FORMULA)))
								ElseIf cMVNFEMSF4=="F"
									If Len(cMensFis) > 0 .And. SubStr(cMensFis, Len(cMensFis), 1) <> " "
										cMensFis += " "
									EndIf
									cMensFis	+=	UPPER(SF4->(Formula(F4_FORMULA)))
								EndIf
							EndIf
							//Fim: Especifico Adoro - por Adriana em 04/08/2020 - chamado 060170
							...
							aadd(aProd,	{Len(aProd)+1,;
								...
								Alltrim(IIF(Empty(SB1->B1_DESCOMP),SC6->C6_DESCRI,SB1->B1_DESCOMP))+; // Adoro - Alterado por Leonardo em 25/03 para atender necessidade Exportacao 
								Iif(Alltrim((cAliasSD2)->D2_COD)$'886641 886640',"", ;                // Adoro
								iIf((cAliasSD2)->D2_QTSEGUM>0," ("+Alltrim(Str((cAliasSD2)->D2_QTSEGUM))+Alltrim((cAliasSD2)->D2_SEGUM)+")","")),; //Adoro
							...
							// Especifico Adoro - Campo para gravar o numero do pedido do cliente   
							If !Empty(SC6->C6_PEDCLI)
								//aadd(aPedCli,SC6->C6_PEDCLI)    
								aadd(aPedCli,ConvType(SC6->C6_PEDCLI))   //por Adriana em 21/09/2018 devido a erro no DANFE - chamado 044042
							Else                              
								aadd(aPedCli,"")
							Endif
							//	
					...
					// Msg Zona Franca de Manaus / ALC
						...
						If !SF2->F2_DESCZFR == 0 .or. ( lInfAdZF .and. nValBse > 0 )//Desnecessario seek redundante na SF3 pois o campo F2_DESCZFR ja possui os valores de ZFR de toda a venda
							...
							Else 
	//					        Especifico Adoro
	//					        Desabilitado por Adriana para impressao da mensagem conforme boletim IOB  - chamado 031280 em novembro/2016
	//					    	cMensFis += "Total do desconto Ref. a Zona Franca de Manaus / ALC. R$ "+str(nValBse-SF2->F2_DESCONT,13,2)
								cMensFis += "Valor do ICMS desonerado R$ "+str(nValBse-SF2->F2_DESCONT,13,2)
							EndIF
						EndIf 	
					EndIF
			...
			EndIf
		Next
		//Especifico ADORO - Ch. 049014 || ABEL BABINI || DEJAIME || FISCAL || ACRESCENTAR MENSAGEM EM TODAS AS NOTAS
		//cMensCli += " / CONFERIR A MERCADORIA NO ATO DA ENTREGA - PRODUTO, QUANTIDADE E VALOR. NÃO ACEITAMOS RECLAMAÇÕES POSTERIORES. "
	Else
		dbSelectArea("SF1")
		dbSetOrder(1)
		If MsSeek(xFilial("SF1")+cNota+cSerie+cClieFor+cLoja)
			...
			Else				
						...
						*/
						...
						Else
							...
							*/
							MsSeek(xFilial("SA1")+SF1->F1_FORNECE+SF1->F1_LOJA)
						EndIf
						aadd(aDest,AllTrim(SA1->A1_CGC)) //1
						aadd(aDest,SA1->A1_NOME) //2
						aadd(aDest,MyGetEnd(SA1->A1_END,"SA1")[1]) //3
		
						If MyGetEnd(SA1->A1_END,"SA1")[2]<>0
							aadd(aDest,MyGetEnd(SA1->A1_END,"SA1")[3]) //4
						Else 
							aadd(aDest,"SN") 
						EndIf
		
						aadd(aDest,IIF(SA1->(FieldPos("A1_COMPLEM")) > 0 .And. !Empty(SA1->A1_COMPLEM),SA1->A1_COMPLEM,MyGetEnd(SA1->A1_END,"SA1")[4])) //5
		
						aadd(aDest,SA1->A1_BAIRRO) //6
						If !Upper(SA1->A1_EST) == "EX"
							aadd(aDest,SA1->A1_COD_MUN) //7
							aadd(aDest,SA1->A1_MUN)			//8	
						Else
							aadd(aDest,"99999")			
							aadd(aDest,"EXTERIOR")
						EndIf
						aadd(aDest,Upper(SA1->A1_EST)) //9
						aadd(aDest,SA1->A1_CEP) //10
						aadd(aDest,IIF(Empty(SA1->A1_PAIS),"1058"  ,Posicione("SYA",1,xFilial("SYA")+SA1->A1_PAIS,"YA_SISEXP"))) //11
						aadd(aDest,IIF(Empty(SA1->A1_PAIS),"BRASIL",Posicione("SYA",1,xFilial("SYA")+SA1->A1_PAIS,"YA_DESCR" ))) //12
						aadd(aDest,SA1->A1_DDD+SA1->A1_TEL) //13
						If !Upper(SA1->A1_EST) == "EX"
							aadd(aDest,VldIE(SA1->A1_INSCR)) //14
						Else
							aadd(aDest,"")							
						EndIf
						aadd(aDest,SA1->A1_SUFRAMA) //15
						...
						//
						aAdd(aDest, SA1->A1_CONTRIB) // Posição 17
						aadd(aDest,Iif(SA1->(FieldPos("A1_IENCONT")) > 0 ,SA1->A1_IENCONT,"")) //18
						aadd(aDest,SA1->A1_INSCRM) //19
						aadd(aDest,SA1->A1_TIPO) //20
						aadd(aDest,SA1->A1_PFISICA)//21-Identificação estrangeiro
					EndIf
				...
				While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
					...
					//Especidfico Adoro - Alex 03/02 - transmitir a caixa de mensagem MENNOTB
					If SF1->(FieldPos("F1_MENNOTB"))>0
						If !AllTrim(SF1->F1_MENNOTB) $ cMensCli
							If Len(cMensCli) > 0 .And. SubStr(cMensCli, Len(cMensCli), 1) <> " "
								cMensCli += " "
							EndIf
							cMensCli += AllTrim(SF1->F1_MENNOTB)
						EndIf
					EndIf     
					//
					...
					SFT->( dbSetOrder( 1 ) )
					//utiliza a funcao SpedNatOper ( SPEDXFUN ) que possui o tratamento para a natureza da operacao/prestacao
					//Especifico Adoro - desabilita linhas padrao
	...
	//				cDescProd := SB1->B1_DESC 
					cDescProd := iif((cAliasSD1)->D1_TIPO == "D" .and. !Empty(SB1->B1_DESCOMP), Alltrim(SB1->B1_DESCOMP)+ Iif(Alltrim((cAliasSD2)->D2_COD)$'886641 886640',"", ;        
										iIf((cAliasSD2)->D2_QTSEGUM>0," ("+Alltrim(Str((cAliasSD2)->D2_QTSEGUM))+Alltrim((cAliasSD2)->D2_SEGUM)+")","")),SB1->B1_DESC) //Por Adriana em 24/09/2018 - chamado 043873          
				...
				//Especifico ADORO
				If SF1->F1_EST <> 'EX'   // <> Nota de Importacao 	
					If SF1->F1_CONTSOC > 0 .or. SF1->F1_VLSENAR > 0 	//Alterado por Adriana Chamado 048333 em 25/04/2019
						cMensTr += PadR("Desconto de IPR    : "+Transform(SF1->F1_CONTSOC+SF1->F1_VLSENAR,"@E 99,999,999.99"),70)  //Acrescentado Valor Senar em 12/07/2018 por Adriana
					Endif
				
					If cEmpAnt $ _cEmpAt1     &&Mauricio 29/03/10 - Tratamento para empresa Ceres. //ticket TI - Antonio Domingos - 13/05/2023
						If!Empty(Alltrim(SF1->F1_GRANJA))
							cMensTr += PADR("Granja "+Alltrim(SF1->F1_GRANJA ),70)   &&Mauricio 26/04/10 - corrigido(TROCADO) := POR +=.
						EndIf
					Endif		    
					cMenscli += cMensTr 
					cMensTr := ""
	...
	If !Empty(aNota)
		...
		_aX5nNF				:= FWGetSX5("01")
		For _i := 1 to Len(_aX5nNF)
			If _aX5nNF[_i,3] == cSerie
				_cUltNFe := _aX5nNF[_i,4]
				EXIT
			Endif
		Next _i

		IF cNota > _cUltNFe
			u_GrLogZBE(Date(),TIME(),cUserName,"03-ERRO NO FATURAMENTO. EMPRESA/FILIAL DIVERGENTE","FATURAMENTO","NFESEFAZ",;
						"NF: "+cNota+" Serie: "+cSerie+" Ultima NF: "+_cUltNFe+" M0_CODIGO: "+SM0->M0_CODIGO+" M0_CODFIL: "+SM0->M0_CODFIL+" cEmpAnt/cFilAnt: "+cEmpAnt+cFilAnt,ComputerName(),LogUserName())
			MsgAlert("(03) Erro no posicionamento de tabelas e a NF não será transmitida. Reinicie o Protheus e tente novamente, e se o problema persistir fale com a TI.","Erro na Transmissão")
			Return({cNfe,EncodeUTF8(cString),cNotaOri,cSerieOri})
		ENDIF
		//FIM Ticket     TI	- Abel Babini			- 04/12/2020 - Validação de posicionamento nas empresas e filiais corretas para geração do XML.
		...
		If lTagProduc .and. FindFunction("NfeRespTec") // chamado 050729 - FWNM - 15/07/2020 - || OS 052035 || TECNOLOGIA || LUIZ || 8451 || REDUCAO DE BASE
			cString += NfeRespTec("")
		EndIf
...
Static Function NfeIde(cChave,aNota,cNatOper,aDupl,aNfVinc,cVerAmb,aNfVincRur,aRefECF,cIndPres,aDest,aProd,aExp,aComb)
	...
	//  Especifico Adoro - desabilitada tag padrao e Incluido tratamento Data Saida - em 09/09/2019 por Adriana para alterar data saida na TAG dhSaiEnt chamado 044314
	//	cString += NfeTag('<dhSaiEnt>',Iif(lDSaiEnt,"",ConvType(aNota[03])+"T"+Iif(Len(AllTrim(aNota[06])) > 5,ConvType(aNota[06]),ConvType(aNota[06])+":00")))
		...
		// *** INICIO WILLIAM COSTA 09/03/2020 - CHAMADO 056380 - Ajuste data de Entrega RNX2 para levar para o XML
		IF cEmpAnt $ SuperGetMv("MV_#DNF001" , .F. ,'',)

			BEGINSQL ALIAS "DNF001"
				...
				INNER JOIN %Table:SC5% C5 
				     ...
				     WHERE D2_FILIAL  = %Exp:FWxFilial("SD2")%
				       AND D2_DOC     = %Exp:aNota[02]%
					   AND D2_SERIE   = %Exp:aNota[01]%
					   AND D2.%notdel% 

			ENDSQL

			IF Empty(DNF001->C5_DTENTR)

				cString += NfeTag('<dhSaiEnt>',Iif(lDSaiEnt,"",Iif(_cSaiEnt <> "",_cSaiEnt,ConvType(aNota[03])+"T"+Iif(Len(AllTrim(aNota[06])) > 5,ConvType(aNota[06]),ConvType(aNota[06])+":00"))))
                           
			ELSE

				cString += NfeTag('<dhSaiEnt>',CVALTOCHAR(YEAR(STOD(DNF001->C5_DTENTR))) + '-' + STRZERO(MONTH(STOD(DNF001->C5_DTENTR)),2) + '-' + STRZERO(DAY(STOD(DNF001->C5_DTENTR)),2)+"T"+Iif(Len(AllTrim(aNota[06])) > 5,ConvType(aNota[06]),ConvType(aNota[06])+":00"))
		...
		// *** FINAL WILLIAM COSTA 09/03/2020 - CHAMADO 056380 - Ajuste data de Entrega RNX2 para levar para o XML
	...
	*/
	...
	Static Function NfeEmit(aIEST, cVerAmb, aDest)
		...
		_aX5nNF	:= FWGetSX5("01")
		For _i := 1 to Len(_aX5nNF)
			If Alltrim(_aX5nNF[_i,3]) == Alltrim(_cNFSer)
				_cUltNFe := Alltrim(_aX5nNF[_i,4])
				EXIT
			Endif
		Next _i

		IF Alltrim(_cNFNum) > Alltrim(_cUltNFe)
			u_GrLogZBE(Date(),TIME(),cUserName,"02-ERRO NO FATURAMENTO. EMPRESA/FILIAL DIVERGENTE","FATURAMENTO","NFESEFAZ",;
						"NF: "+_cNFNum+" Serie: "+_cNFSer+" Ultima NF: "+_cUltNFe+" M0_CODIGO: "+SM0->M0_CODIGO+" M0_CODFIL: "+SM0->M0_CODFIL+" cEmpAnt/cFilAnt: "+cEmpAnt+cFilAnt,ComputerName(),LogUserName())
			MsgAlert("(02) Erro no posicionamento de tabelas e a NF não será transmitida. Reinicie o Protheus e tente novamente, e se o problema persistir fale com a TI.","Erro na Transmissão")
			cString := '<emit>'
			Return(cString)
		ENDIF
		//FIM Ticket     TI	- Abel Babini			- 04/12/2020 - Validação de posicionamento nas empresas e filiais corretas para geração do XML.
		...
		*/
		...
		cString += '<enderEmit>'
		//cString += '<Lgr>'+IIF(!lEndFis,ConvType(FisGetEnd(SM0->M0_ENDCOB,SM0->M0_ESTCOB)[1]),ConvType(FisGetEnd(SM0->M0_ENDENT,SM0->M0_ESTENT)[1]))+'</Lgr>' //padrao desabilitado por Adriana em 06/08/2019 chamado 044314
		//Adoro - para retirar o SN do endereco - por Adriana em 06/08/2019 chamado 044314
		_cEnderEmit := IIF(!lEndFis,ConvType(FisGetEnd(SM0->M0_ENDCOB,SM0->M0_ESTCOB)[1]),ConvType(FisGetEnd(SM0->M0_ENDENT,SM0->M0_ESTENT)[1]))
		_cEnderEmit := Alltrim(_cEnderEmit)
		_cEnderEmit := IIF(Right(_cEnderEmit,2)= "SN",Left(_cEnderEmit,Len(_cEnderEmit)-3 ),_cEnderEmit)
		cString += '<Lgr>'+_cEnderEmit+'</Lgr>'   
		//Fim chamado 044314 - por Adriana em 06/08/2019
...
Static Function NfeItem(aProd,aICMS,aICMSST,aIPI,aPIS,aPISST,aCOFINS,aCOFINSST,aISSQN,aCST,aMed,aArma,aveicProd,aDI,aAdi,aExp,aPisAlqZ,aCofAlqZ,aAnfI,cTipo,cVerAmb,aComb,cMensFis,cCsosn,aPedCom,aNota,aICMSZFM,aDest,cIpiCst,aFCI,lIcmDevol,nVicmsDeson,nVIcmDif,cMunPres,aAgrPis,aAgrCofins,nIcmsDif,aICMUFDest,nvFCPUFDest,nvICMSUFDest,nvICMSUFRemet,cAmbiente,aIPIDevol,nvBCUFDest, aItemVinc,npFCPUFDest,npICMSUFDest,npICMSInter,npICMSIntP,aLote)
	...
	cEan		:= IIF(!Empty(aProd[46]),iif( aProd[46] == "000000000000000","",iif(_lDUN14, aProd[46], aProd[03])), aProd[03]) // Incluido em 30/08/19 por Adriana para verificar se cliente utiliza DUN 14 no XML da NF - chamado 051408
	...
	//Padrao 
	cString += '<vUnTrib>'+ IIf(cF2Tipo == "C",ComplPreco(cTipo,cF2Tipo,aProd),ConvType(aProd[10]/aProd[12],21,8))+'</vUnTrib>'  // Inibido por Wagner Neves em 01/2016 - Adoro
	//Especifico Adoro
	//cString += '<vUnTrib>'+ IIf(cF2Tipo == "C",ComplPreco(cTipo,cF2Tipo,aProd),ConvType(aProd[16],21,8))+'</vUnTrib>'    // Linha acrescentada por Wagner Neves substituindo a linha de cima para alteração dos 6 digitos - Adoro
	//
	...
	// em 02/10/2020 por Adriana Ticket 34 - habilitado trecho padrao
	If Len(aDI)>0 .And. ConvType(aDI[04][1]) == "I19"
		...
		If lDInoDanfe
			...
			// Especifico Adoro - em 02/10/2020 por Adriana Ticket 34
			cMensFis += " Total PIS: "+Alltrim(Transform(SF1->F1_VALIMP5,"@E 99,999,999.99"))
			cMensFis += " Total COFINS: "+Alltrim(Transform(SF1->F1_VALIMP6,"@E 99,999,999.99"))
			cMensFis += Iif(Val(ConvType(aDI[37][3],15,2))>0," Total AFRMM: "+Alltrim(ConvType(aDI[37][3],15,2)),"")
			//
	...
	//Especifico Adoro para clientes vinculados as redes constantes no parametro MV_#REDEOP (SAPORE/MASSIMA) - Incluido por Adriana conforme chamado 020449
	if Len(aPedCli) > 0 .and. !Empty(aPedCli[01]) 
		cString += '<xPed>'+Alltrim(aPedCli[01])+'</xPed>'    
	endif
	//
			...
			If (aCST[01] = "60" .or. cCsosn$ "500") .AND. cTipo=="1"		
				...
				If nBaseIcm > 0 .and. nValICM > 0	.and. nAlqICM > 0
					...
					If cArt274 == "1"  .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT)) == "SP"  									
						cMensFis += "IMPOSTO RECOLHIDO POR SUBSTITUICAO - ARTIGO 274 DO RICMS (LEI 6.374/89, ART.67, PARAGRAFO 1o., E AJUSTE SINIEF-4/93', CLAUSULA TERCEIRA, NA REDACAO DO AJUSTE SINIEF-1/94) 'COD.PRODUTO:  " +ConvType(aProd[02])+"' VALOR DA BASE DE ST: R$ " +Alltrim(Str(nBaseIcm,15,2))+" VALOR DE ICMS ST: R$ "+Alltrim(str(nValICM,15,2))+" / "
			...
			*/
			If (valType(aPPDifal)== "A" .and. Len(aPPDifal)>0 .and. Year(aNota[03]) >= aPPDifal[1][1])
				...
				If Alltrim(Str(aProd[33])) == "4" .Or. Alltrim(Str(aProd[33])) == "7" .Or. Alltrim(Str(aProd[33])) == "12"
					...
					cString += '<AliquotaInter>'+ConvType(aProd[33],6,2)+'</AliquotaInter>' //pICMSInter //compatibilizado com padrão - 23/05/2019-Adriana 049388
					cString += '<pICMSInter>'+ConvType(aPICMSInter,8,4)+'</pICMSInter>'//pICMSInterPart //compatibilizado com padrão - 23/05/2019-Adriana 049388
	...
	If lSuframa .And. !empty(aDest[15])
	//	cMensFis += "Código Suframa: "+alltrim(aDest[15]+".") 
		cMensFis += space(40)+"Inscrição do destinatário no Suframa Nr.: "+alltrim(aDest[15]+".") 
	Endif
...
Static Function NfeInfAd(cMsgCli,cMsgFis,aPedido,aExp,cAnfavea,aMotivoCont,aNfSa,aNfVinc,aProd,aDI,aNfVincRur,aRet,cNfRefcup,cSerRefcup,cTipo,nIPIConsig,nSTConsig,lBrinde,cVerAmb,aRefECF,nVicmsDeson,nvFCPUFDest,nvICMSUFDest,nvICMSUFRemet,nvBCUFDest,aICMUFDest,nValIpiBene,npFCPUFDest,npICMSUFDest,npICMSInter,npICMSIntP,aObsCont,cValTotOpe)
	...
	//Everson - 14/06/2019. Chamado 044314.
	//Everson - 11/06/2019. Chamado 044314.
	//Everson - 03/06/2019. Chamado 044314.
	If cEmpAnt $cEmpFrt .And. cFilAnt $cFilGFrt
		cString += freteCTE(SF2->F2_DOC,SF2->F2_SERIE)
	...
	*/
	...
	//Especifico Adoro - desabilitadas linhas padroes
	/*
	...
	*/
	...
	//Especifico Adoro para cliente constantes no parametro MV_#REDEOP - Incluido por Adriana conforme chamado 020449
	if cTipo = '1' 
		if Alltrim(SA1->A1_REDE) $ cRedeOP .and. !Empty(SC5->C5_OPLOG) //Incluida verificacao do campo SC5->C5_OPLOG, para não gerar erro no XML - chamado 021430  //Alterado por Adriana chamado 031238 em novembro/2016
			cString += '<ObsCont xCampo="Filial">'
			cString += '<xTexto>'+SC5->C5_OPLOG+'</xTexto>'
			cString += '</ObsCont>'
		endif
	endif	
	//
		...
		*/
			...
			If ( cTipo == "1") //Somente se nota de saída ou devolução.
				If !Empty(aExp[01][04][03])
					cString += '<exporta>'                                                      
	// 				Especifico Adoro 				
	//				cString += '<UFEmbarq>'+ConvType(aExp[01][04][03][01][03])+ '</UFEmbarq>'// Desabilitado e substituido pela linha abaixo para não gerar erro quando transp. rodoviario
					cString += '<UFEmbarq>'+iif(Empty(ConvType(aExp[01][04][03][01][03])),"SP",ConvType(aExp[01][04][03][01][03]))+ '</UFEmbarq>' //Incluido Adoro
	//				
...
static FUNCTION NoAcento(cString)
	...
	cString := StrTran(cString,"°","o")  //Por Adriana em 21/09/2018 - chamado 044042
	//
...
Static Function RetPrvUnit(cAliasSD2,nDesconto)
	...
	//Adoro 
	//inicio chamado 027912 Gringoletto - William Costa
	IF ALLTRIM((cAliasSD2)->D2_CF) == '5902' 

		nRetorno := (cAliasSD2)->D2_PRCVEN
		
	ELSE
	// Gringoletto chamado 027912 - William Costa  
		...
		Else
			...
			Else
				...
				Else
		//			nRetorno := ( ( (cAliasSD2)->D2_TOTAL + nDesconto + (cAliasSD2)->D2_DESCZFR - ((cAliasSD2)->D2_DESCZFP + (cAliasSD2)->D2_DESCZFC)  ) / (cAliasSD2)->D2_QUANT ) // Desabilitada por Wagner Neves 07/12/2015 - 6 digitos - Adoro
					nRetorno := (cAliasSD2)->D2_PRCVEN // Linha Incluida por Wagner Neves EM 07/12/2015.
				Endif
			Endif
		
		Endif
	ENDIF // Gringoletto chamado 027912 - William Costa  
...
Static Function retQtd2UM( lNoImp2UM, cCFOPExp, cCFOP, nCONVDIP, nQUANT)
	...
	Endif
	Else
		If ( Left(cCFOP,1) $ "3-7" ) .Or. ( cCFOP $ cCFOPExp ) // Adoro - Ricardo Lima - 24/08/18
			If nCONVDIP > 0
				// Adoro
				// Ricardo Lima - 24/08/18
				If SB1->B1_TIPCONV = 'M'
...
Return aRet
/*/{Protheus.doc} freteCTE
	Cria as tags para preenchimento do valor de frete para 
	geração do CT-e. Chamado 044314.
	@type  Static Function
	@author Everson
	@since 03/06/2019
	@version 01
	/*/
Static Function freteCTE(cDocFrt,cSrFrt)
	...
	If Empty(cDocFrt) .Or. Empty(cSrFrt)
		RestArea(aArea)
		Return cRret
		
	EndIf

	//Everson 16/01/2020. Chamado 054941.
	cTpVa := getTpCrg(cDocFrt,cSrFrt)
	If ! Empty(cTpVa)
		cAverba	:= getApolic(cTpVa) 
	...
	//
	If  Val(cValToChar(nVlfFrete)) > 0
		cRret += '<obsCont xCampo="CT1|VALOR">'
		cRret += '<xTexto>' + Alltrim(cValToChar(nVlfFrete)) + '</xTexto>'
		cRret += '</obsCont>'
	
	EndIf

	//Everson 16/01/2020. Chamado 054941.
	//26/06/2019³Everson 044314.
	If ! Empty(cAverba)
		cRret += '<obsCont xCampo="CT4|AVERBAR">'
		cRret += '<xTexto>' + cAverba + '</xTexto>'
		cRret += '</obsCont>'
...
/*/{Protheus.doc} getTpCrg
	...
	@version 01
	/*/
Static Function getTpCrg(cDocFrt,cSrFrt)
	...
	//
	Conout( DToC(Date()) + " " + Time() + " NFESEFAZ - getTpCrg - cDocFrt/cSrFrt " + cValToChar(cDocFrt) + "/" + cValToChar(cSrFrt) )

	//
	DbSelectArea("ZFD")
	ZFD->(DbSetOrder(6))
	ZFD->(DbGoTop())
	If ZFD->( DbSeek( FWxFilial("ZFD") + cDocFrt + cSrFrt ) )
	...
	//
	Conout( DToC(Date()) + " " + Time() + " NFESEFAZ - getTpCrg - cTpVa " + cValToChar(cTpVa) )
...
/*/{Protheus.doc} getPorEm 
	...
	@version 01
	/*/
Static Function getApolic(cTpVa)
	...
	Local aApol := Separa(cValToChar(GetMv("MV_#CTEAVB",,"////")),"/",.T.)
	Local cApol := 0

	//
	If nTpVa > 0 .And. Len(aApol) >= nTpVa
		cApol := Alltrim(cValToChar(aApol[nTpVa]))

	EndIf

	//
	Conout( DToC(Date()) + " " + Time() + " NFESEFAZ - getApolic - cApol " + cValToChar(cApol) )

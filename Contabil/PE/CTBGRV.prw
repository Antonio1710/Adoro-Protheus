...
/*/{Protheus.doc} User Function CTBGRV
	...
	@history ticket 77295   - Fernando Macieira - 26/08/2022 - A INTEGRAÇÃO DE CONTABILIZAÇÃO DA CERES NÃO ESTA FUNCIONANDO
	@history ticket 83115   - Antonio Domingos  - 26/08/2022 - Diferença nos indicadores nos lançamentos de compensações de ciente e ncc de filiais diferentes..
	@history ticket 83115   - Antonio Domingos  - 26/08/2022 - Diferença nos indicadores nos lançamentos de compensações de ciente e ncc de filiais diferentes.
	@history ticket 84225   - 12/01/2023 - Fernando Macieira - Alteração da Regra de lançamento de depreciação para inclusão de lote X CC e Descri Lote
	@history ticket 89227 - 03/04/2023 - Fernando Macieira - Diferença nos indicadores nos lançamentos de compensações de ciente e ncc de filiais diferentes
    ...
    @history ticket 93253 - 04/05/2023 - Fernando Macieira - PROBLEMA NA INTEGRAÇÃO - FATURAMENTO CERES
	...
	/////////////////////////////////////////////////////////////
		...
		If nCV3_RECORI > 0
			...
			If AllTrim(SE5->E5_TIPO)=="NCC" .AND. !Empty(CT2->CT2_DEBITO)
				...
				RecLock("CT2", .f.)
					CT2->CT2_FILKEY := SE5->E5_FILORIG //SE5->E5_FILIAL // @history ticket 89227 - 03/04/2023 - Fernando Macieira - Diferença nos indicadores nos lançamentos de compensações de ciente e ncc de filiais diferentes
					CT2->CT2_PREFIX := SE5->E5_PREFIXO
					CT2->CT2_NUMDOC := SE5->E5_NUMERO
					CT2->CT2_PARCEL := SE5->E5_PARCELA
					CT2->CT2_TIPODC := SE5->E5_TIPO
			...
			Else
				...
				If AllTrim(SE5->E5_TIPO)<>"RA"
					...
					If SE1->( dbSeek(FWxFilial("SE1")+cPrefixo+cNum+cParcela+cTipo) )
						...
						RecLock("CT2", .f.)
							CT2->CT2_FILKEY := SE1->E1_FILORIG //SE1->E1_FILIAL // @history ticket 89227 - 03/04/2023 - Fernando Macieira - Diferença nos indicadores nos lançamentos de compensações de ciente e ncc de filiais diferentes
							CT2->CT2_PREFIX := SE1->E1_PREFIXO
							CT2->CT2_NUMDOC := SE1->E1_NUM
							CT2->CT2_PARCEL := SE1->E1_PARCELA
							CT2->CT2_TIPODC := SE1->E1_TIPO
			...
			If AllTrim(SE5->E5_TIPO)=="NCC" 
				...
				Else
					...
					If SE1->( dbSeek(FWxFilial("SE1")+cPrefixo+cNum+cParcela+cTipo) )
						...
						RecLock("CT2", .f.)
							CT2->CT2_FILKEY := SE1->E1_FILORIG //SE5->E5_FILIAL // @history ticket 89227 - 03/04/2023 - Fernando Macieira - Diferença nos indicadores nos lançamentos de compensações de ciente e ncc de filiais diferentes
							CT2->CT2_PREFIX := SE1->E1_PREFIXO
							CT2->CT2_NUMDOC := SE1->E1_NUM
							CT2->CT2_PARCEL := SE1->E1_PARCELA
							CT2->CT2_TIPODC := SE1->E1_TIPO
...
/*/{Protheus.doc} newLoteZCN()
	Preenche o lote recria na contabilização da depreciação
	Regra: De acordo com o centro de custo utilizado no lançamento da depreciação, buscar no cadastro de Lote x CC o lote correspondente que estiver com status “Aberto”.
	@type  Static Function
	@author FWNM
	@since 12/01/2023
	@version version
	...
	(examples)
	@see (links_or_references)
	@ticket 84225   - 12/01/2023 - Fernando Macieira - Alteração da Regra de lançamento de depreciação para inclusão de lote X CC e Descri Lote
/*/
	...
	If CV3->CV3_TABORI == "SD1" .And. alltrim(cEmpAnt)  $ _cMVEUSALT
	   If CV3->CV3_TABORI == "SD1" 
			...
			If CT2->(DBSEEK(CV3->CV3_FILIAL+CV3->CV3_KEY))
				//Aviso("Atenção", " CV3_KEY "+CV3->CV3_KEY+" CT2_KEY "+CT2->CT2_KEY, {"OK"}, 3)
				If CT2->CT2_XLTXCC <> SD1->D1_XLOTECC .And. !Empty(SD1->D1_XLOTECC)
					...
					CT2->( msUnLock() )
					//Aviso("Atenção", " CT2_XLTXCC "+CT2->CT2_XLTXCC+" D1_XLOTECC "+SD1->D1_XLOTECC, {"OK"}, 3)
				EndIf
	   		EndIf
	   EndIf
	EndIf

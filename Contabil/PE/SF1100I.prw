...
/*/{Protheus.doc} User Function SF1100I
	...
	@history Ticket 82123 - Antonio Domingos  - 20/10/2022 - Correção do erro Alias already in use: WORK  on U_GETSBE(SF1100I.PRW) 07/10/2022 11:59:15 line : 741
...
User Function SF1100I()
	...
	ELSE
		...
		If Alltrim(cEmpAnt) $ _cEmpAt1 //ticket TI - Antonio Domingos - 22/05/2023 
	...
	//-------------------------------------------------------------------------------------------------------------
	IF cEmpAnt $ _cEmpAt1 .and. cFormul == "N" .And. SF1->F1_TIPO=="N" .And. Alltrim(SF1->(F1_FORNECE+F1_LOJA)) $GetMv("MV_XFORPCC") // Verifica se a nota é tipo normal e o fornecedor da nota está contido no parâmetro
		...
		IF DbSeek(_ChaveSe2)
			...
			IF (Se2->E2_Pis<>0 .And. Se2->E2_Cofins<>0 .And. Se2->E2_Csll<>0 .And. Se2->E2_PretPis=="1" .And. Se2->E2_PretCof=="1" .And. Se2->E2_PretCsl=="1")
				...
				FOR _wGeraPcc := 1 to 3				
					...
					Se2->E2_VENCREA  	:= DataValida( F050VImp(_wImposto,_wEmisTitPai,_wContTitPai,_wVencTitPai), .T. )  // Calcula o vencimento do imposto

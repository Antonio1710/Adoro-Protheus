...
/*{Protheus.doc} User Function ADCON008P
	...
	@history ticket TI - 28/02/2023 - Rodrigo Mello - Ajuste estabilização pós migração dicionário dados
*/	
...
Static Function FileTRC()
	...
	iif( select("TRC") > 0,  TRC->(dbCloseArea()), Nil)

	oTemp := FWTemporaryTable():New("TRC", aStrut)
	oTemp:AddIndex("01", {"TMP_CODUSU"} )
	oTemp:AddIndex("02", {"TMP_NOME"} )
	oTemp:Create()	

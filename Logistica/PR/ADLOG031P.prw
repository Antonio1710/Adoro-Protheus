...
/*/{Protheus.doc} User Function ADLOG031P
	...
	@history Ticket 77523   - Everson - 18/08/2022 - Adiconadas as informações de pedidos faturados e não faturados.
	@history Ticket 77523   - Everson - 19/08/2022 - Detalhes das informações de pedidos faturados e não faturados.
	@history Ticket 78656	- Everson - 22/08/2022 - Adicionados os campos roteiro, sequência e placa.
	@history Ticket 83363	- Antonio Domingos - 14/11/2022 - Correção do erro: type mismatch on compare  on LOADARQ(ADLOG031P.PRW) 23/08/2022 05:48:38 line : 272
	@history Ticket 83518	- Antonio Domingos - 16/11/2022 - Não estamos visualizando o volume de vendas no monitor de entregas
	@history Error Log      - Antonio Domingos - 27/02/2023 - Ajuste do array _aDet qdo Vazio - array out of bounds ( 10 of 9 )  on {|| {_aDet[oDet:nAt,1],_aDet[oDet:nAt][2],_aDet[oDet:nAt][3],_aDet[oDet:nAt][4],_aDet[oDet:nAt][5],_aDet[oDet:nAt][6],_aDet[oDet:nAt][7],_aDet[oDet:nAt][8],_aDet[oDet:nAt][9],_aDet[oDet:nAt][10]}}(ADLOG031P.PRW) 16/11/2022 15:06:39 line : 500
/*/
User Function ADLOG031P() // U_ADLOG031P()
	...
	//Everson - 18/08/2022. Chamado 77523.
	Private nFatPed := 0 
	Private nFatCx  := 0
	Private nFatKg  := 0
	Private nFatVl  := 0
	...
	******/
	DEFINE MSDIALOG oDlg TITLE cTitulo FROM 0,0 TO 340,720 PIXEL

	@ 10,10 LISTBOX oLbx FIELDS HEADER " ","Situacao","Pedido(s)","Caixa(s)", "  KG  ","  R$  "," % ", SIZE 340, 110 OF oDlg PIXEL 
	...
	@ 130,20 SAY "Entrega De : "+Dtoc(Mv_Par01) SIZE 100,007 OF oDlg PIXEL
	@ 140,20 SAY "Entrega Ate: "+Dtoc(Mv_Par02) SIZE 100,007 OF oDlg PIXEL 

	DEFINE SBUTTON FROM 130,260 TYPE 15 ACTION Processa({|lEnd| LoadArq(2) },OemToAnsi("Atualiza"),OemToAnsi("Processando..."),.F.) ENABLE OF oDlg //Removido atualização automatica e adicionado para atualizar no botao
	DEFINE SBUTTON FROM 130,300 TYPE 2  ACTION oDlg:End() ENABLE OF oDlg
...
Static Function LoadArq(nTp) 
	...
	If nTp==2 
		...
		Query += " SUM(CASE WHEN SFAT = 'FATURADO' THEN 1 ELSE 0 END) AS FPEDIDOS,"
		Query += " SUM(CASE WHEN SFAT = 'FATURADO' THEN FONTES2.QTDCX ELSE 0 END) AS FFQTDCX, "
		Query += " SUM(CASE WHEN SFAT = 'FATURADO' THEN FONTES2.QTDKG ELSE 0 END) AS FFQTDKG, "
		Query += " SUM(CASE WHEN SFAT = 'FATURADO' THEN FONTES2.VALOR ELSE 0 END) AS FFVALOR, "

		Query += " SUM(CASE WHEN SFAT = 'NFATURADO' THEN 1 ELSE 0 END) AS NFPEDIDOS,"
		Query += " SUM(CASE WHEN SFAT = 'NFATURADO' THEN FONTES2.QTDCX ELSE 0 END) AS NFFQTDCX, "
		Query += " SUM(CASE WHEN SFAT = 'NFATURADO' THEN FONTES2.QTDKG ELSE 0 END) AS NFFQTDKG, "
		Query += " SUM(CASE WHEN SFAT = 'NFATURADO' THEN FONTES2.VALOR ELSE 0 END) AS NFFVALOR, "
		...
		Query += " ( "
			...
			Query += " FROM "
				...
				Query += " ( "
					...
					Query += " FROM "
						...
						Query += " ( "
							...
							Query += " SELECT "  
								...
								Query += " CASE WHEN C5_NOTA = '' THEN 'NFATURADO' ELSE 'FATURADO' END AS SFAT " 
							...
							Query += " WHERE SC6.C6_SEGUM NOT IN ('KG','') "
								...
								Query += " AND SC5.C5_NOTA <> 'XXXXXXXXX' " 
								...
								// Ticket 83363	- Antonio Domingos - 14/11/2022
								cMV_PAR07 := If(Valtype(MV_PAR07)=="N",Str(MV_PAR07,1,0),alltrim(MV_PAR07)) //Ticket 83518 - Antonio Domingos - 16/11/2022
								If cMV_PAR07 == "1" // Desconsiderar FOB? = 1 = SIM 
					...
					Query += " GROUP BY PED, VENDEDOR,C9PEDIDO,PREAPROV, C5BLQ, SFAT "
		...
		While !EOF()  
			...
			nFatPed += TMP1->FPEDIDOS 
			nFatCx  += TMP1->FFQTDCX 
			nFatKg  += TMP1->FFQTDKG 
			nFatVl  += TMP1->FFVALOR 

			nNFatPed += TMP1->NFPEDIDOS 
			nNFatCx  += TMP1->NFFQTDCX 
			nNFatKg  += TMP1->NFFQTDKG 
			nNFatVl  += TMP1->NFFVALOR 
		...
		//Faturados
		aAdd(aVetor,{.F.,"FATURADO ->",nFatPed,Transform(nFatCx, "@E 999,999,999") ,Transform(nFatKg,"@E 999,999,999.99"),Transform(nFatVl,"@E 999,999,999.99"),,})

		//Faturados
		aAdd(aVetor,{.F.,"NÃO FATURADO ->",nNFatPed,Transform(nNFatCx, "@E 999,999,999") ,Transform(nNFatKg,"@E 999,999,999.99"),Transform(nNFatVl,"@E 999,999,999.99"),,})
	...
	******/
	...
	If nTp==2 
		...
		oLbx:bLine := {||{ Iif(Empty(aVetor[oLbx:nAt,02]),"",If(aVetor[oLbx:nAt,01],oOK,oNO)) ,aVetor[oLbx:nAt,02],;                      
...
Static function fTela()
  Local aSize := MsAdvSize(.F.)
	...
	if aVetor[oLbx:nAt,2]                      == "CORTE.EXPED ->" .Or. ;
		...
		Alltrim(cValToChar(aVetor[oLbx:nAt,2])) == "BLOQ.REGRA COMERCIAL" .Or.;	 //Chamado 060113 - Abel Babini, 04/08/2020, Redefinição da situação do pedido
		Alltrim(cValToChar(aVetor[oLbx:nAt,2])) == "FATURADO ->" .Or.;  //Everson - 19/08/2022 - Chamado 77523.
		Alltrim(cValToChar(aVetor[oLbx:nAt,2])) == "NÃO FATURADO ->"	//Everson - 19/08/2022 - Chamado 77523.
		...
		//TICKET 77523 - Everson - 19/08/2022
		ElseIf Alltrim(cValToChar(aVetor[oLbx:nAt,2])) == "FATURADO ->"

			For j:=2 To 13 //Ticket 78656	- Everson - 22/08/2022
				_cLin += ",_aDet[oDet:nAt]["+AllTrim(Str(j))+"]"
			Next		
			nOpc := 7	

		//TICKET 77523 - Everson - 19/08/2022
		ElseIf Alltrim(cValToChar(aVetor[oLbx:nAt,2])) == "NÃO FATURADO ->"
		...
		//TICKET 10851 - ADRIANO SAVOINE - 19/03/2021
		ElseIf nOpc == 7 .Or. nOpc == 8 //Ticket 78656	- Everson - 22/08/2022
			@ 10,10 LISTBOX oDet FIELDS HEADER "Pedido", "Cliente", "Loja", "Nome","Carga","CX(s)","KG(s)","Endereco","Bairro","Cidade","Roteiro","Sequência","Placa", SIZE 660,260 OF oDlg1 PIXEL	
...
Static Function LeDetalhe(nOpc) 
	...
	ElseIf nOpc == 2 //Everson - 09/11/2017. Chamado 037937.
		...
		If Len(_aDet) == 0
			aAdd(_aDet,{"","","","","","","","","","" }) //Error Log - Antonio Domingos - 27/02/2023
		Endif
	...
	ElseIf nOpc == 5
		...
		If Len(_aDet) == 0
			...
			aAdd(_aDet,{;
				...
				"",; 
				"",; 
				"",; //Error Log - Antonio Domingos - 27/02/2023
				})
		Endif
	...
	//TICKET 77523 - ADRIANO SAVOINE - 19/08/2022
	ElseIf nOpc == 7 .Or. nOpc == 8
		...
		While DETPED->(!Eof())
			...
			//Ticket 78656	- Everson - 22/08/2022
			cRoteiro := Posicione("SC5",1,xFilial("SC5") + Alltrim(DETPED->PEDIDO), "C5_ROTEIRO" )
			cSequenc := Posicione("SC5",1,xFilial("SC5") + Alltrim(DETPED->PEDIDO), "C5_SEQUENC" )
			cPlaca   := Posicione("SC5",1,xFilial("SC5") + Alltrim(DETPED->PEDIDO), "C5_PLACA" )
			...
			cBairro,; 
			cCidade,;
			cRoteiro,; //Ticket 78656	- Everson - 22/08/2022
			cSequenc,; 
			cPlaca,; 
			})
...
Static Function AD031EXC(nOpc)
	...
	//Ticket 10851 - ADRIANO SAVOINE - 19/03/2021
	ElseIf nOpc == 7
		cArquivo    := 'REL_PEDIDOS_FATURADOS.XML'
		cPlanilha   := "Pedidos Faturados"
		cTitulo     := "Rel.Pedidos Faturados"	

	//Ticket 10851 - ADRIANO SAVOINE - 19/03/2021
	ElseIf nOpc == 8
		cArquivo    := 'REL_PEDIDOS_NAO_FATURADOS.XML'
		cPlanilha   := "Pedidos Não Faturados"
		cTitulo     := "Rel.Pedidos Não Faturados"	
...
Static Function GeraExcel(nOpc)
	...
	WHILE DETPED->(!EOF())
		...
		ElseIf nOpc == 7
			IncProc("Processando Ped. Faturados. "+cNumPV)
		
		ElseIf nOpc == 8
			IncProc("Processando Ped. Não Faturados. "+cNumPV)
		...
		ElseIf nOpc == 7 .Or. nOpc == 8
			...
			AADD(aLinhas,{ "", ; // 01 A  
				...
				"", ; // 11 K  
				"", ; // 12 L  
				""  ; // 13 M  //Ticket 78656	- Everson - 22/08/2022
				})
		...
		ElseIf nOpc == 7 .Or. nOpc == 8
			...
			aLinhas[nLinha][09] := Posicione("SA1",1,xFilial("SA1")+cCliente+cLoja ,"A1_BAIRROE")            //I
			aLinhas[nLinha][10] := Posicione("SA1",1,xFilial("SA1")+cCliente+cLoja ,"A1_MUNE")               //J
			aLinhas[nLinha][11] := Posicione("SC5",1,xFilial("SC5") + Alltrim(DETPED->PEDIDO), "C5_ROTEIRO" ) //K
			aLinhas[nLinha][12] := Posicione("SC5",1,xFilial("SC5") + Alltrim(DETPED->PEDIDO), "C5_SEQUENC" ) //L
			aLinhas[nLinha][13] := Posicione("SC5",1,xFilial("SC5") + Alltrim(DETPED->PEDIDO), "C5_PLACA" )   //M //Ticket 78656	- Everson - 22/08/2022
	...
	FOR nExcel := 1 TO nLinha
		...
		ElseIf nOpc == 7 .Or. nOpc == 8 // TICKET 77523 - Everson - 19/08/2022
			...
			aLinhas[nExcel][13] ; // 13 M //Ticket 78656	- Everson - 22/08/2022
			})
...
Static Function SalvaXml(nOpc)
	...
	//TICKET 77523 - Everson - 19/08/2022
	ElseIf nOpc == 7
		oExcel:GetXMLFile("C:\temp\REL_PED_FAT.XML")
	//TICKET 77523 - Everson - 19/08/2022
	ElseIf nOpc == 8
		oExcel:GetXMLFile("C:\temp\REL_PED_N_FAT.XML")
	EndIf
...
Static Function CriaExcel(nOpc)              
	...
	ElseIf nOpc == 7
		oMsExcel:WorkBooks:Open("C:\temp\REL_PED_FAT.XML")	

	ElseIf nOpc == 8
		oMsExcel:WorkBooks:Open("C:\temp\REL_PED_N_FAT.XML")	
...
Static Function Cabec(nOpc)
	...
	ElseIf nOpc == 7 .Or. nOpc == 8
		...
		oExcel:AddColumn(cPlanilha,cTitulo,"Bairro"                 ,1,1) // 09 I
		oExcel:AddColumn(cPlanilha,cTitulo,"Cidade"                 ,1,1) // 10 J 
		oExcel:AddColumn(cPlanilha,cTitulo,"Roteiro"                ,1,1) // 11 K 
		oExcel:AddColumn(cPlanilha,cTitulo,"Sequência"              ,1,1) // 12 L 
		oExcel:AddColumn(cPlanilha,cTitulo,"Placa"                  ,1,1) // 13 M //Ticket 78656	- Everson - 22/08/2022
	EndIf

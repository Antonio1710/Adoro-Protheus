...
/*/{Protheus.doc} ADLOG084R - Relatorio de envio de e-mail de BUONNY a Vencer
    @type  Function
...
/*/
User Function ADLOG084R(aParam)

    Local bProcess 		:= {|oSelf| Executa(oSelf) }
    Local cPerg 		:= "ADLOG084R"
    Local aInfoCustom 	:= {}
    Local cTxtIntro	    := "Rotina respons�vel envio de e-mail de BUONNY a Vencer"
    ...
    Private nAdDias1   := 7 
    Private nAdDias2   := 1
    Private cAssunto   := "Aviso de BUONNY a Vencer"
    Private cCorpo     := " "
    Private aAnexos    := {}
    ...
    If lJob
        ...
        cMVPAR02 := czFIL
        cMVPAR03 := czFIL
        nAdDias1   := SuperGetMV("AD_LOG084A",.F.,7) //Numero de dias de vencto para o primeiro e-mail de aviso para data de BUONNY a Vencer
        nAdDias2   := SuperGetMV("AD_LOG084B",.F.,1) //Numero de dias de vencto para o segundo e-mail de aviso  para data de BUONNY a Vencer
        //    
        Qout(" JOB ADLOG084R-Protheus - 01 - Parametros dMVPAR01="+ Dtoc(dMVPAR01) + ", dMVPAR02=" + cMVPAR02 +", cMVPAR03="+ cMVPAR03 )
        //   
        logZBN("1") 
        oProcess := Executa()
        logZBN("2") 
    ...
    Else
        nAdDias1   := SuperGetMV("AD_LOG084A",.F.,7) //Numero de dias de vencto para o primeiro e-mail de aviso para data de BUONNY a Vencer
        nAdDias2   := SuperGetMV("AD_LOG084B",.F.,1) //Numero de dias de vencto para o segundo e-mail de aviso  para data de BUONNY a Vencer
        oProcess := tNewProcess():New("ADLOG084R","Aviso de BUONNY a Vencer",bProcess,cTxtIntro,cPerg,aInfoCustom, .T.,5, "BUONNY a Vencer", .T. )
    Endif

    U_ADINF009P(SUBSTRING(ALLTRIM(PROCNAME()),3,LEN(ALLTRIM(PROCNAME()))) + '.PRW',SUBSTRING(ALLTRIM(PROCNAME()),3,LEN(ALLTRIM(PROCNAME()))),'Envio de e-mail de BUONNY a Vencer')
...
Static Function Executa(oProcess)
    ...
    Private cNomArq := "REL_LOG\BUONNY_a Vencer_"+DTOS(dMVPAR01)+"_"+TIME()
    Private cDIRARQ := "C:\TEMP\"
    Private cDIRREDE := "\REL_LOG\"
    Private cNomMotorista := ""
    Private cCPFMotorista  := ""
    Private cCelMotorista  := ""
    Private cBUONNY  := ""
    Private cDtValid := ""
    aAnexos:={}
    ...
    BeginSQL Alias _cAlias1
		%NoPARSER%  
        SELECT LTRIM(STR((DATEDIFF(day, %Exp:DTOS(dMVPAR01)%,ZVC_DTVALI)),3)) AS DIAS_VENC,ZVC.ZVC_DTVALI,ZVC.ZVC_EMLMOT,ZVC.ZVC_BUONNY,ZVC.ZVC_DTVALI,ZVC.*,SA4.A4_EMAIL,SA4.A4_XEMAILN 
        ...
        AND 
        (LTRIM(STR((DATEDIFF(day, %Exp:DTOS(dMVPAR01)%,ZVC_DTVALI)),3)) = %Exp:cAdDias1% OR 
        LTRIM(STR((DATEDIFF(day, %Exp:DTOS(dMVPAR01)%,ZVC_DTVALI)),3)) = %Exp:cAdDias2%) 
        AND ZVC.%notDel%
	EndSQl             
    ...
    If (_cAlias1)->(!EOF())
        While (_cAlias1)->(!EOF())
            ...
            cBUONNY        := (_cAlias1)->ZVC_BUONNY   
            cDtValid       := DTOC(STOD((_cAlias1)->ZVC_DTVALI))
                ...
                cCorpo += '				<tr>'
                cCorpo += '					<th scope="col">'
                cCorpo += '						<span style="background-color:#d3d3d3;">BUONNY&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;&nbsp;</span></th>'
                cCorpo += '					<th scope="col">'
                cCorpo += '						<span style="background-color:#d3d3d3;">Data Valid.&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;&nbsp;</span></th>'
                cCorpo += '				</tr>'
                cCorpo += '			</thead>'
                cCorpo += '			<tbody>'
                cCorpo += '				<tr>'

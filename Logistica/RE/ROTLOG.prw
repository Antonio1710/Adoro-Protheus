...
/*/{Protheus.doc} User Function ROTLOG
	...
	@history Ticket 90191 - Everson  - 16/03/2023 - Correção errorlog.
/*/
...
User Function ROTLOG() // U_ROTLOG()
	...
	aAdd(_aStru,{"TP_ICAIXAS"  , "N",06,0}) // INVERSO PARA INDICE Flek - Rodrigo Mello
	...
	oTemp:AddIndex("IDX001", {"TP_ROTEIRO","TP_PRODUTO"} )
	oTemp:AddIndex("IDX002", {"TP_ROTEIRO","TP_XCATEG","TP_CAIXAS"} )
	oTemp:Create()
	...
	Do While !Eof()
		...
		IF Alltrim(cValToChar(MV_PAR14)) == "2" // 1== NAO 2==SIM  INCLUIDO POR WESLEY CHAMADO 029389
				...
				If (SC6->C6_NOTA  <> ' ' .AND. mv_par11 == 1) .OR.( mv_par11 <> 1 ) .OR. (XC5->C5_TIPO $ 'DB')
					...
					if _lLib .And. mv_par11 == 2//.or. mv_par12 == 2 .or. C6_QTDENT>0  &&Mauricio - 26/03/10 - Solicitado por Caio trazer apenas itens liberados no relatorio.
						...
						If  !(SUBSTR(C6_CF,2,3) $ ALLTRIM(GETMV("MV_NOSC6C1")))     
							...
							Replace TP_ICAIXAS   With ( TP_CAIXAS + SC6->C6_UNSVEN ) * -1

...
User Function RQRYQUEBRA()
	...
	AADD(_aStr,{'ZX_FILIAL'   ,"C",02,00})
	AADD(_aStr,{'ZX_QUEBRA'   ,"N",15,02})
	AADD(_aStr,{'QBRXTOT'     ,"N",15,02})
	AADD(_aStr,{'ZD_OBSR1'    ,"C",200,00})
	AADD(_aStr,{'ZD_OBSR2'    ,"C",200,00})
	AADD(_aStr,{'ZD_OBSR3'    ,"C",200,00})
	AADD(_aStr,{'ZD_NUMNF'    ,"C",09,00})
	AADD(_aStr,{'ZD_SERIE'    ,"C",03,00})
	AADD(_aStr,{'ZX_ITEMNF'   ,"C",02,00})
	AADD(_aStr,{'ZD_CODCLI'   ,"C",06,00})    
	AADD(_aStr,{'ZD_LOJA'     ,"C",06,00})
	AADD(_aStr,{'ZD_NOMECLI'  ,"C",20,00})
	AADD(_aStr,{'ZX_CODPROD'  ,"C",09,00})
	AADD(_aStr,{'ZX_DESCRIC'  ,"C",30,00})
	AADD(_aStr,{'ZX_QTDEV2U'  ,"N",15,02})
	AADD(_aStr,{'ZX_QTDE'     ,"N",15,02})
	AADD(_aStr,{'ZX_UNIDADE'  ,"C",02,00})
	AADD(_aStr,{'ZX_QTSEGUM'  ,"N",15,02})
	AADD(_aStr,{'ZX_SEGUM'    ,"C",02,00})
	AADD(_aStr,{'ZX_QTDEV1U'  ,"N",15,02})
	AADD(_aStr,{'ZX_TOTAL'    ,"N",15,02})
	AADD(_aStr,{'ZX_MOTIVO'   ,"C",02,00})
	AADD(_aStr,{'ZD_DEVTOT'   ,"C",01,00})
	AADD(_aStr,{'ZD_DTDEV'    ,"C",08,00})
	AADD(_aStr,{'ZD_AUTNOME'  ,"C",15,00})
	AADD(_aStr,{'ZD_RESPONS'  ,"C",06,00})
	AADD(_aStr,{'ZD_RESPNOM'  ,"C",20,00})
	AADD(_aStr,{'ZD_MOTIVO'   ,"C",06,00})
	AADD(_aStr,{'ZD_DESCMOT'  ,"C",20,00})
	AADD(_aStr,{'ZD_PEDIDO'   ,"C",06,00})
	AADD(_aStr,{'ZD_PLACA'    ,"C",07,00})
	AADD(_aStr,{'ZD_ROTEIRO'  ,"C",03,00})
	AADD(_aStr,{'ZD_SEQUENC'  ,"C",02,00})
	AADD(_aStr,{'ZD_OBS1'     ,"C",40,00})
	AADD(_aStr,{'ZD_VEND'     ,"C",06,00})
	AADD(_aStr,{'ZD_PERNOIT'  ,"C",01,00})
	AADD(_aStr,{'ZD_RESP1'    ,"C",03,00})
	AADD(_aStr,{'ZD_CODMOTP'  ,"C",03,00})
	AADD(_aStr,{'ZD_MTVPERN'  ,"C",20,00})
	AADD(_aStr,{'ZD_PEDPERN'  ,"C",20,00})
	AADD(_aStr,{'ZD_MOTORI'   ,"C",50,00})
	AADD(_aStr,{'ZD_TELEFON'  ,"C",13,00})
	AADD(_aStr,{'ZD_EMLMOT'   ,"C",50,00})

	/*_cArqTmp :=CriaTrab(_aStr,.T.)
	DbUseArea(.T.,,_cArqTmp,"TMPIMP",.F.,.F.)
	_cIndex:="ZD_NUMNF"
	indRegua("TMPIMP",_cArqTmp,_cIndex,,,"Criando Indices...")         */
	oTMPIMP := FWTemporaryTable():New("TMPIMP", _aStr)
	oTMPIMP:AddIndex("IDX001", {"ZD_NUMNF"} )
	oTMPIMP:Create()

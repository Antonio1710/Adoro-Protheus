...
/*/{Protheus.doc} User Function ADFIS001
	...
	@history Ticket 91025   - Jonathan Carvalho - 30/03/2023 - Inlcluido os IDent da SAFEGG
	@history Ticket 91093   - Jonathan Carvalho - 30/03/2023 - Inclusão do alerta de Filial não cadastrada para o Relatorio
	@history Ticket 91081   - Jonahtan Carvalho - 31/03/2023 - INclusão das Filiais Novas de Matrizes 
	@history Ticket 91081   - Jonathan Carvalho - 04/04/2023 - Utilização do cadastro 000022 para a SAFEGG
	/*/
...
USER FUNCTION ADFIS001()
	...
	Private _cEntidade  := ""
	Private _cEntidad2  := ""
	Private cEmpAt1     := SuperGetMv("MV#EMPAT1",.F.,"01/13") //Codigo de Empresas Ativas Grupo 1 //ticket TI - William Costa - 23/05/2023
	Private cEmpAt4     := SuperGetMv("MV#EMPAT4",.F.,"01/02/07/09/13") //Codigo de Empresas Ativas Grupo 4 //ticket TI - William Costa - 23/05/2023
	...
	//If cEmpAnt =  "01" 
	If cEmpAnt $ cEmpAt1 //ticket TI - William Costa - 23/05/2023 
		//Ticket 69574   - Abel Babini          - 25/04/2022 - Projeto FAI
		//Ticket 91081   - Jonathan Carvalho    - 31/03/2023
		_aEntidade  :={{"01","000011"},{"02","000001"},{"03","000002"},{"04","000005"},{"05","000007"},{"06","000004"},{"07","000013"},{"08","000014"},{"09","000015"},{"0A","000021"},{"0B","000023"},{"0C","000024"},{"0D","000025"},{"0E","000026"},{"0F","000027"},{"0G","000028"},{"0H","000029"},{"0I","000030"},{"0J","000031"},{"0K","000032"},{"0L","000033"},{"0M","000034"},{"0N","000035"},{"0O","000036"},{"0P","000037"},{"0Q","000038"},{"0R","000039"},{"0S","000040"}}
	...
	ElseIf cEmpAnt =  "09" //SAFEGG
		...
		_aEntidade  :={{"01","000022"}} //Ticket 91025 - Jonathan Carvalho 04/04/2023

		//_aEntidade  :={{"01","000020"},{"01","000022"}} //Ticket 69574   - Abel Babini          - 25/04/2022 - Projeto FAI
		//_cEntidad2 := alltrim(_aEntidade[1][2]) + '", "' + alltrim(_aEntidade[2][2]) //Ticket 91025 - Jonathan Carvalho 30/03/2023
	...
	//Ticket 91093 - Jonathan Carvalho 30/03/2023
	If Ascan(_aEntidade, {|x| Alltrim(x[1]) = cFilAnt }) == 0
		Alert("Filial nao configurada para este relatorio !!!")
		Return
	EndIf

	///Ticket 91025 - Jonathan Carvalho 30/03/2023
	//If len(_cEntidad2) > 0
	//	_cEntidade := replace(_cEntidad2, '"',"'")
	//Else
	//	_cEntidade := _aEntidade[Ascan(_aEntidade, {|x| Alltrim(x[1]) = cFilAnt } ), 2]
	//EndIf

	_cEntidade := _aEntidade[Ascan(_aEntidade, {|x| Alltrim(x[1]) = cFilAnt } ), 2] //Ticket 91025 - Jonathan Carvalho 04/04/2023
...
Static Function RunReport(Cabec1,Cabec2,Titulo,nLin)
	...
	For _i := Val(MV_PAR01) to Val(MV_PAR02)
		...
		cQuery+="SP50.ID_ENT IN('"+_cEntidade+"') AND "

...
/*{Protheus.doc} User Function ADLOG003P
	...
	@history Ticket 80369   - Everson - 21/09/2022 - Tratamento para filial de Itapira.
	@history Ticket 78062   - Everson - 29/11/2022 - Tratamento para envio de carga de pedido de venda de fornecedor.
	@history TI             - Adriano Savoine - 11/04/2023 - Retirado o UnLockByName.
*/
...
User Function ADLOG003P(aXEmpFil)
	...
	//Ticket 80369   - Everson - 21/09/2022 - Tratamento para filial de Itapira.
	cFilini       := FWxFilial("SC5")
    cFilfin       := FWxFilial("SC5")
	...
	While TRB->(!EOF())  
		...
		ENDIF //fecha if cRotCab <> cRotCabOld
			...
			TRG->(dbCloseArea()) 
	      	...
	      	SqlEntregas(TRB->C5_TIPO)  //Everson - 29/11/2022 - ticket 78062.        
	      	cFil    := ''
	      	cDtEntr := ''
			...
			/////// ************FINAL CRIA VETOR E VARIAVEIS DE ENTREGA****************************************** //
      		...
      		While TRC->(!EOF())
				...
				If TRB->C5_TIPO == "B"
					...
					cCodCli    	:= TRC->A2_CGC
					cReferenc  	:= NoAcento2(ALLTRIM(TRC->A2_NOME))
					cLatitude	:= TRC->A2_XLATITU
					cLongitude	:= TRC->A2_XLONGIT
					cTipoCli	:= Posicione("SX5",1,TRC->C5_FILIAL+"_S"+TRC->A2_SATIV1,"X5_DESCRI")
					cHrIniM	    := TRC->A2_HRINIM
					cHrFimM	    := TRC->A2_HRFINM
					cHrIniT	    := TRC->A2_HRINIT
					cHrFimT	    := TRC->A2_HRFINT

					cEndereco  := TRC->A2_END
					cBairro    := NOACENTO2(TRC->A2_BAIRRO)
					cCidade    := TRC->A2_MUN
					cCep       := TRC->A2_CEP

				Else

					cCodCli 	:= ALLTRIM(TRC->C5_CLIENTE+TRC->C5_LOJACLI)
					cReferenc	:= NoAcento2(ALLTRIM(POSICIONE('SA1',1,xFilial("SA1")+TRC->C5_CLIENTE+TRC->C5_LOJACLI,'A1_NOME')))
					cLatitude	:= TRC->A1_XLATITU
					cLongitude	:= TRC->A1_XLONGIT
					cTipoCli    := Posicione("SX5",1,TRC->C5_FILIAL+"_S"+TRC->A1_SATIV1,"X5_DESCRI")
					cHrIniM	    := TRC->A1_HRINIM
					cHrFimM	    := TRC->A1_HRFINM
					cHrIniT	    := TRC->A1_HRINIT
					cHrFimT	    := TRC->A1_HRFINT
	    		...
	    		OWSENTREGA:CCODIGO           := cCodCli //Everson - 29/11/2022 - ticket 78062.
	    		...
	    		OWSENTREGA:CESTIMATIVAFIM    := ALLTRIM(cHoraRet)
		      	...
		      	OWSENTREGA:CREFERENCIA       := cReferenc //Everson - 29/11/2022 - ticket 78062.
		      	OWSENTREGA:NCUBAGEM          := 0
		      	OWSENTREGA:NLATITUDE         := VAL(STRTRAN(cLatitude,',','.')) //Everson - 29/11/2022 - ticket 78062.
		      	OWSENTREGA:NLONGITUDE        := VAL(STRTRAN(cLongitude,',','.')) //Everson - 29/11/2022 - ticket 78062.
		      	OWSENTREGA:NPESO             := TRC->C5_PBRUTO
		      	OWSENTREGA:NSEQUENCIA        := nLTRC
		      	OWSENTREGA:NVALOR            := TRC->C6_VALOR  
		      	OWSENTREGA:CTIPOCLIENTE      := cTipoCli
		      	OWSENTREGA:NQTDCAIXAS        := 0 
		      	OWSENTREGA:NTEMPOENTREGA     := 0
		      	OWSENTREGA:cIniRecebManha    := cHrIniM //Everson - 29/11/2022 - ticket 78062.
				OWSENTREGA:cFimRecebManha    := cHrFimM //Everson - 29/11/2022 - ticket 78062.
				OWSENTREGA:cIniRecebTarde    := cHrIniT //Everson - 29/11/2022 - ticket 78062.
				OWSENTREGA:cFimRecebTarde    := cHrFimT //Everson - 29/11/2022 - ticket 78062.
				...
				/////// ************FINAL CRIA VETOR E VARIAVEIS DE Notas Fiscais******************************************
	      		...
	      		While TRD->(!EOF())
					...
					OWSNOTAFISCAL:CREFERENCIA          := cReferenc
	...
	BeginSQL Alias "TRB"
			%NoPARSER% 
				SELECT SC5.C5_FILIAL, 
					   ...
					   ZFA.ZFA_TABFRT,
					   ZFA.ZFA_VEICPG
					   ,C5_TIPO //Everson - 29/11/2022 - ticket 78062.
...
STATIC FUNCTION SqlEntregas(cTipo)
		...
		BeginSQL Alias "TRC"
				%NoPARSER%
					SELECT 
						...
						SUM(SC5.C5_PBRUTO) AS C5_PBRUTO,
						SUM(SC6.C6_VALOR) AS C6_VALOR	      	
					FROM %Table:SC5% SC5 WITH(NOLOCK), %Table:SC6% SC6 WITH(NOLOCK), %Table:SA2% SA2 WITH(NOLOCK)
					WHERE SC5.C5_FILIAL   = %exp:cFil%
						AND SC5.C5_DTENTR   = %exp:cDtEntr%
						AND SC5.C5_PLACA   >= %exp:cPlacaIni%
						AND SC5.C5_PLACA   <= %exp:cPlacaFin%
...
STATIC FUNCTION SqlHrCarga(cNumCarga)
	...
	//Ticket 80369   - Everson - 21/09/2022 - Tratamento para filial de Itapira.
	Local cQuery		:= ""
	Local cLnkSrv		:= Alltrim(SuperGetMV("MV_#UEPSRV",,"LNKMIMS"))
	...
	cQuery += " SELECT TOP(1) "
		...
		cQuery += " DT_FECHCARG, "
		cQuery += " HR_FECHCARG "
    cQuery += " FROM [" + cLnkSrv + "].[SMART].[dbo].[VW_EXPECARG_01]  "
    cQuery += " WHERE ID_CARGEXPE = '" + cNumCarga + "' "

...
/*{Protheus.doc} User Function ADLOG017P
	...
	@history Ticket TI    - Adriano Savoine - 11/04/2023 - Retirado o Lockbyname.
*/	
...
Static Function ExportaXML()
    ...
    Local nLTRC           	:= 0
	Local cFCNPJ			:= "" //Everson - 29/11/2022 - ticket 78062.
	...
	While TRB->(!EOF())  
		...
		/////// ************FINAL CRIA VETOR E VARIAVEIS DE ENTREGA****************************************** //
      	...
      	While TRC->(!EOF())
			...
			cFCNPJ := ""
			If TRC->C5_TIPO == "B"
				Aadd(aClientes, U_ADLOG087P(TRC->C5_CLIENTE,TRC->C5_LOJACLI)) //Enviar o fornecedor para o Ravex.
				cFCNPJ := Posicione("SA2", 1, FWxFilial("SA2") + TRC->C5_CLIENTE + TRC->C5_LOJACLI, "A2_CGC")
		    ...
		    OWSENTREGAPLANEJADA:CCODIGOCLIENTE     := Iif(!Empty(cFCNPJ), cFCNPJ, Alltrim(TRC->C5_CLIENTE+TRC->C5_LOJACLI)) //Everson - 29/11/2022 - ticket 78062.
...
STATIC FUNCTION SqIntPedido()
	...
	//Everson - 26/10/2022 - ticket 78062.
	BeginSQL Alias "TRB"
			%NoPARSER% 
				 SELECT SC5.C5_FILIAL, 
					...
					AND ( (SC5.C5_ROTEIRO  >= '300' AND SC5.C5_ROTEIRO  <= %EXP:cRotFin% AND C5_TIPO = 'N') OR (SC5.C5_ROTEIRO  >= '230' AND SC5.C5_ROTEIRO  <= %EXP:cRotFin% AND C5_TIPO = 'B') ) //Everson - 29/11/2022 - ticket 78062.

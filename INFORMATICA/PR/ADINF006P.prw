...
Static Function FileTRB()
	...
	oTrb := FWTemporaryTable():New("TRB", aStruct)
	oTrb:AddIndex("01", {"PAA_TPENCE","PAA_SPRINT"} )//Ticket 85319  - Antonio Domingos - 21/02/2023
	oTrb:AddIndex("02", {"PAA_CHAMAD"} )//Ticket 85319  - Antonio Domingos - 21/02/2023
	oTrb:Create()
...
Static Function SqlGeral()
	...
	iif( Select("TMP0") != 0, TRB->(dbCloseArea()) , Nil)
...
STATIC FUNCTION SqlHoras(cCodChamado)
	...
	iif( Select("TRC") != 0, TRB->(dbCloseArea()) , Nil)

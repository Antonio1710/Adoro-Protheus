...
User Function AlteOS()
	...
	If ! ("ADCH004" $ AllTrim( Upper( FunName() ) ))
		...
		AEVal( FWSX3Util():GetAllFields( "PAA" ), {|u| iif( alltrim(u) $ 'PAA_FIM|PAA_HRFIM|PAA_ACEITE|PAA_DTACT|PAA_AVLACT|PAA_USRACT|PAA_HRACT|PAA_MAILUS', AAdd( _aAlter, u), Nil ) } )
		/*
		dbSelectArea( "SX3" )
		dbSetOrder( 01 )
		dbSeek( "PAA" )
...
Static Function MntArraySX3( _cArquivo )
    ...
    //@history Ticket 70142  - Rodrigo / Flek Solution - 27/07/2022 - Ajustes Dic. de dados no Banco
	AEVal ( FWSX3Util():GetListFieldsStruct( _cArquivo ), {|u| aAdd( _aArray, { u[1], Iif( u[2] == "C", "", Iif( u[2] == "N", 00, Iif( u[2] == "D", StoD( "" ), "" ) ) ) } ) } ) 
	/*
	dbSelectArea( "SX3" )
	dbSetOrder( 01 )
...
/*/{Protheus.doc} ADCH005_A
	Quando campo preeenchido, mostra mensagem e retorna verdadeiro, não bloqueia se estiver vazio.
	@type  Function
	@author Antonio Domingos
	@since 26/01/2023
	@version version
	@param _cCampo,_cMensagem
	@return _lRet
	@example
	U_ADCH005_A(M->A1_TEL,"Informe o Telefone fixo.")
	@see (links_or_references)
	@history Ticket 85319  	- Antonio Domingos - 26/01/2023 - Validação Fontes V33 - Dic. Banco de Dados.
	/*/
User Function ADCH005_A(_cCampo,_cMensagem)
_lRet := .T.
If !Empty(_cCampo)
	MSGINFO(_cMensagem)
EndIf

...
/*/{Protheus.doc} User Function ADINFLIB
	...
	@history Everson, 13/02/2023, ticket 88099 - Adicionado campo memo.
	@history Everson, 21/03/2023, ticket 90522 - Tratamento errorlog.
/*/    

#Define STR_PULA        Chr(13)+ Chr(10)

User Function GrLogZBE(dDate,cTime,cUser,cLog,cModulo,cRotina,cParamer,cEquipam,cUserRed,cMemo) //Everson - 13/02/2023 - ticket 88099.
    ...
    If RecLock("ZBE",.T.)//incluido if para certificar que o registro foi travado em 23/08/2017 por Adriana 
        ...
        //Everson, 21/03/2023, ticket 90522 - Tratamento errorlog.
        If FieldPos("ZBE_LOGMEM") > 0
            Replace ZBE_LOGMEM  WITH cMemo //Everson - 13/02/2023 - ticket 88099.
...
Static Function fCriaMsNew()
    ...
    For nAtual := 1 To Len(aStruAux)
        ...
        If !empty(GetSX3Cache( cCampoAtu, "X3_CAMPO"))  // SX3->(DbSeek(cCampoAtu))
        
            //Cabeçalho ...    Titulo        Campo        Mask                                    Tamanho                Dec                    Valid    Usado    Tip                F3    CBOX
            aAdd(aHeadAux,{    GetSX3Cache( cCampoAtu, "X3_TITULO"), cCampoAtu, GetSX3Cache( cCampoAtu, "X3_PICTURE"), TamSX3(cCampoAtu)[1], TamSX3(cCampoAtu)[2],    ".F.",    ".F.",    GetSX3Cache( cCampoAtu, "X3_TIPO"),    "",    ""})
             
            //Se o campo atual for retornar, aumenta o tamanho do retorno
            If cCampoAtu $ cCampoRet
            
                nTamanRet += TamSX3(cCampoAtu)[1]

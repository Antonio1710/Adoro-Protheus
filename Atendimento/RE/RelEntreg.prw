#Include "Rwmake.ch"
#Include "Topconn.ch"
#Define CRLF Chr(13) + Chr(10) //Ticket 85319  - Antonio Domingos - 01/02/2023
...
/*/{Protheus.doc} User Function RelEntrg
	...
	@history Ticket 85319  - Antonio Domingos - 10/01/2023 - Validação Fontes V33 - Dic. Banco de Dados.
	@history Ticket 85319  - Antonio Domingos - 01/02/2023 - Validação Fontes V33 - Dic. Banco de Dados.
	/*/
...
Static function DadosRel()
	...
	oTrb := FWTemporaryTable():New("TRB", aStru)
	oTrb:AddIndex("IDX001", {"ORDEM"} )
	oTrb:Create()
...
Static Function obtemDetalhe(cRotIni, cRotFim, lInverte) //Éverson - Chamado 029359
	...
	Local cQuery	:= ""
	//Ticket 85319  - Antonio Domingos - 01/02/2023 - INCLUINDO CRLF
    cQuery += " SELECT * "+CRLF
	cQuery += " FROM "+CRLF 
		cQuery += " ( "+CRLF 
		cQuery += " SELECT SC5.C5_ROTEIRO, " +CRLF
			cQuery += " SC5.C5_PLACA, " +CRLF
			cQuery += " SC5.C5_DTENTR, " +CRLF
			cQuery += " COUNT(SC5.C5_SEQUENC) AS QTD_DE_ENTREGA, " +CRLF
			cQuery += " SUM(CASE WHEN SC5.C5_MOK <> '02' then 1  else 0  END) AS A_ENTREGAR, " +CRLF
			cQuery += " SUM(CASE WHEN SC5.C5_MOK = '02' then 1  else 0  END) AS ENTREGUES, " +CRLF
			cQuery += " (SUM(CASE WHEN SC5.C5_MOK = '02' then 1  else 0  END) * 100) / COUNT(SC5.C5_SEQUENC) AS PORCENTAGEM_REALIZADO, " +CRLF
			cQuery += " CASE CAST(C5_ROTEIRO AS INT)%2 WHEN 0 THEN 'PAR' ELSE 'IMPAR' END AS PAR_OU_IMPAR " +CRLF //
		cQuery += " FROM " + RetSqlName("SC5") + " SC5 " +CRLF
		cQuery += " WHERE SC5.C5_FILIAL  IN ("+ALLTRIM(MV_PAR05)+") "+CRLF
			cQuery += " AND SC5.C5_DTENTR  >= '" +dtos(mv_par01)+ "' " +CRLF
			cQuery += " AND SC5.C5_DTENTR  <= '" +dtos(mv_par02)+ "' " +CRLF
			
			If ! lInverte
				cQuery += " AND SC5.C5_ROTEIRO >= '" + cRotIni + "' " +CRLF
				cQuery += " AND SC5.C5_ROTEIRO <='"  + cRotFim + "' " +CRLF
			
			Else
				cQuery += " AND (SC5.C5_ROTEIRO <= '" + cRotIni + "' OR SC5.C5_ROTEIRO >= '"  + cRotFim + "') " +CRLF

			EndIf
			
			cQuery += " AND SC5.D_E_L_E_T_ = '' " +CRLF
			cQuery += Iif(MV_PAR06 = 1, " AND (SC5.C5_NOTA <> '' OR SC5.C5_LIBEROK = 'E' AND SC5.C5_BLQ = '' ) ","") +CRLF
		cQuery += " GROUP BY SC5.C5_DTENTR,SC5.C5_ROTEIRO,SC5.C5_PLACA " +CRLF
		cQuery += " ) AS FONTE "+CRLF
		
	If ! Empty(cTipoRoteiro)
	cQuery += " WHERE " +CRLF
		cQuery += " FONTE.PAR_OU_IMPAR = '" + cTipoRoteiro + "' " +CRLF
	
	EndIf
	
	cQuery += " ORDER BY FONTE.PORCENTAGEM_REALIZADO  " +CRLF
	//Ticket 85319  - Antonio Domingos - 01/02/2023 - INCLUINDO CRLF E CAST(C5_ROTEIRO AS INT)
	
	If Select("DETALHES") > 0
		DETALHES->(DbCloseArea())
	EndIf
	
	//Memowrite("c:\temprelentreg.sql",cQuery) //Ticket 85319  - Antonio Domingos - 01/02/2023

...
/*/{Protheus.doc} User Function ADIMPDA1
	...
	@history Everson, 16/09/2022. Chamado TI - Tratamento para filial de Itapira.
	@history Everson, 24/02/2023. Chamado 89026 - Tratamento para filial de Itapira.
...
User Function ADIMPDA1()
	...
	//Everson, 24/02/2023. Chamado 89026.
	//Everson - 04/03/2018. Chamado 037261. SalesForce.
	//If ! (cEmpAnt + cFilAnt $"0102/010B")
	If ! (alltrim(cEmpAnt) + alltrim(cFilAnt) $ _cEmpFL3)
		Return 
		...
		If MsgBox(_cMesg,"ATENCAO","YESNO")
			...
			For _nInd = 1 to Len(_aDado)
				...
				If DA1->(DBSeek( Alltrim(cValToChar(CFILANT)) + Alltrim(cValToChar(_cTabela))+ Alltrim(cValToChar(_cProduto)) )) //Everson - 28/07/2020. Chamado 059906.
					...
					//Everson, 16/09/2022. Chamado TI - Tratamento para filial de Itapira.
					DbSelectArea("SB1")
					SB1->(DbSetOrder(1))
					SB1->(DbGoTop())
					...
					If SB1->( DbSeek( FWxFilial("SB1") + _cProduto ) )
						...
						cQuery := " SELECT MAX(DA1_ITEM) AS DA1_ITEM FROM " + RetSqlName("DA1") + " (NOLOCK) DA1 WHERE DA1_FILIAL = '" + FWxFilial("DA1") + "' AND DA1_CODTAB = '" + _cTabela + "'  AND DA1.D_E_L_E_T_ = '' " 

						cNewAlias := GetNextAlias()
						DbUseArea(.T., "TOPCONN", TCGenQry(,, cQuery), cNewAlias, .F., .T.)

						If ! Empty((cNewAlias)->DA1_ITEM) .And. DA1->(Reclock("DA1",.T.))
							DA1->DA1_FILIAL	:= FWxFilial("DA1")
							DA1->DA1_CODTAB	:= _cTabela
							DA1->DA1_ITEM	:= Soma1((cNewAlias)->DA1_ITEM)

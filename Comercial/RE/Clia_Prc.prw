#include "rwmake.ch"
#include "topconn.ch"
#Define CRLF Chr(13) + Chr(10) //Ticket TI  - Antonio Domingos - 11/05/2023
...
//@history ticket TI - Antonio  Domingos - 09/05/2023 - FV9010: SetOrder - Index not found. on RUNCON1(CLIA_PRC.PRW) 14/02/2023 11:34:28 line : 379												
//@history ticket TI - Antonio  Domingos - 11/05/2023 - FV9010: SetOrder - Ajuste de Query
...
If Select ("NSD2") > 0
   ...
   NSD2->(DbCloseArea())
Endif   
//INIICIO ticket TI - Antonio  Domingos - 11/05/2023					
_cQueryTRB := " SELECT * FROM "+RetSqlName("SD2")+ " SD2 (NOLOCK) "+CRLF
_cQueryTRB += " INNER JOIN "+RetSqlName("SF4")+ " SF4 "+CRLF
_cQueryTRB += " ON SF4.F4_FILIAL = '' "+CRLF
_cQueryTRB += " AND SF4.F4_CODIGO = SD2.D2_TES "+CRLF
_cQueryTRB += " AND SF4.F4_DUPLIC = 'S' "+CRLF
_cQueryTRB += " AND SF4.D_E_L_E_T_ = ' ' "+CRLF
_cQueryTRB += " WHERE D2_FILIAL IN ("+ALLTRIM(MV_PAR04)+")  "+CRLF
_cQueryTRB += " AND D2_EMISSAO BETWEEN '"+dtos(_dDataIni)+"' AND '"+dtos(_dDataFim)+"' "+CRLF
_cQueryTRB += " AND D2_TIPO = 'N' "+CRLF
_cQueryTRB += " AND SD2.D_E_L_E_T_= ''  "+CRLF
_cQueryTRB += " ORDER BY D2_EMISSAO, D2_NUMSEQ "+CRLF

MemoWrite("C:\TEMP\.Clia_Prc.sql",_cQueryTRB)
//FIM ticket TI - Antonio  Domingos - 11/05/2023
...
Do While !Eof() //.and. SD2->D2_FILIAL == xFilial("SD2") .and.;
	...
	If _cTip_Frt =  'C'  // frete CIF
		...
		If _nValFrete > 1000000
			...
			SZK->(DbGoTop())
			FV9->(DbSetOrder(2)) // Indexa por filial + regial + data //ticket TI - Antonio  Domingos - 09/05/2023												
			FV9->(DbGoTop())

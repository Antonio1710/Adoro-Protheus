...
/*/{Protheus.doc} User Function MT140LOK
	...
	@history Everson, 16/02/2023, ticket 88584 - Adicionada tolerância no preço unitário.
	@history Everson, 28/02/2023, ticket TI - Alterada variável de tolerância no preço unitário.
	@history Jonahtan, 03/038/2023, ticket 89490 - Incluido Round para o valor nVlrUni na validação de preço unitario. 
/*/
		...
		If SB1->( dbSeek(FWxFilial("SB1")+SC7->C7_PRODUTO) )
			...
			If _lOk
				...
				If SC7->C7_MOEDA == 1 
					...
					If Round( (SC7->C7_PRECO + GETMV("MV_#VLTUNT",,0.01)) ,2) < Round(nVlrUni,2) //Everson - 16/02/2023 - ticket 88584. //Everson - 28/02/2023 - ticket 88584. // Jonathan - 03/03/2023 - Ticket 89490.
					
						cErroVunit := 	"Valor unitario da NF está maior que o Valor Unitario "+ STR_LF +"do item do Pedido " + Alltrim(cPC) + "/ " + ;
										cPCItem + STR_LF +;
										"Valor Unitario NF: R$" + TRANSFORM(nVlrUni, "@E 999,999.999") + STR_LF + ;
										"Valor Unitario PC: R$" + TRANSFORM(SC7->C7_PRECO, "@E 999,999.999")

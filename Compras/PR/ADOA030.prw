...
FillGetDados(nOpcx,"PAH",1,''/*cSeek*/,{||'*'}/*bWhile*/,,{"PAH_FILIAL","PAH_CODGRP","PAH_NOMGRP","PAH_USERGI","PAH_USERGA"},,,,,,@aHeader,/*@aCols*/,,,,,,.T.)
IIF( AScan(aHeader,{|u| alltrim(u[2])=="PAH_ALI_WT"}) > 0, Eval({|| ADel( aHeader, AScan(aHeader,{|u| alltrim(u[2])=="PAH_ALI_WT"})), ASize(aHeader, Len(aHeader)-1)}), Nil)
IIF( AScan(aHeader,{|u| alltrim(u[2])=="PAH_REC_WT"}) > 0, Eval({|| ADel( aHeader, AScan(aHeader,{|u| alltrim(u[2])=="PAH_REC_WT"})), ASize(aHeader, Len(aHeader)-1)}), Nil)

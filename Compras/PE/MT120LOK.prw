...
Static lProdPF := .f. // @history ticket 86055 - 18/01/2023 - Fernando Macieira - Compra de produtos químicos controlados	
...
/*/{Protheus.doc} User Function MT120LOK
	...
	@history ticket 85790 - 16/01/2023 - Fernando Macieira - OPERAÇÃO NOVA MATRIZES - ITEM CONTÁBIL
	@history ticket 86055 - 18/01/2023 - Fernando Macieira - Compra de produtos químicos controlados
	@history ticket TI - 26/01/2023 - Fernando Macieira - Error log - Called from {|| OSELF:EXECUTE("A131GERA",              4,             0,"GERA COTACAO",              3,"","GERA COTACAO","",               3) }(FWFORMBROWSE.PRW) 08/06/2022 16:53:24 line : 579
...
User Function MT120LOK()
	...
	*/
	...
	lRetorno := lRetorno .And. U_ADMNT022P(gdFieldGet("C7_OP", n), "")

	PutCTD() // @history ticket 85790 - 16/01/2023 - Fernando Macieira - OPERAÇÃO NOVA MATRIZES - ITEM CONTÁBIL
...
User Function GetAprov( cCusto, cItemCta, nTotPed, cProduto )
	...
	If Type("aCols") <> "U" // @history ticket TI - 26/01/2023 - Fernando Macieira - Error log - Called from {|| OSELF:EXECUTE("A131GERA",              4,             0,"GERA COTACAO",              3,"","GERA COTACAO","",               3) }(FWFORMBROWSE.PRW) 08/06/2022 16:53:24 line : 579
		...
		For i := 1 To Len(aCols)
			...
			If !gdDeleted(i)
				If !lProdPF
					lProdPF := GetSB5(gdFieldGet("C7_PRODUTO", i)) 
				EndIf
			EndIf
	...
	Else
		...
		// Análise Cotação 
		lProdPF := .f.
		If AllTrim(FunName()) == "MATA161" 

			If Select("WorkMAPA") > 0
				WorkMAPA->( dbCloseArea() )
			EndIf

			cQuery := " SELECT DISTINCT C8_PRODUTO 
			cQuery += " FROM " + RetSqlName("SC8") + " (NOLOCK)
			cQuery += " WHERE C8_FILIAL='"+FWxFilial("SC8")+"' 
			cQuery += " AND C8_NUM='"+SC8->C8_NUM+"'
			cQuery += " AND D_E_L_E_T_=''
			
			tcQuery cQuery New Alias "WorkMAPA"

			WorkMAPA->( dbGoTop() )
			Do While WorkMAPA->( !EOF() )
			
				If !lProdPF
					lProdPF := GetSB5(WorkMAPA->C8_PRODUTO) 
				EndIf
	...
	If !Eof()
		...
		While !Eof() .And. PAG->PAG_FILIAL == xFilial( "PAG" ) .And. PAG->PAG_CODGRP == PAF->PAF_CODGRP
			...
			// @history ticket 86055 - 18/01/2023 - Fernando Macieira - Compra de produtos químicos controlados
			//lProdPF := GetSB5(cProduto)
			If lProdPF
				cAprovB5 := GetMV("MV_#SB5APV",,"002100") // LUIS_ROCHA == 002100 ou PRISCILA_CARVALHO
				// Crio um nível entre o último vistador e o aprovador
				If cTipLib == "A"
			...
			// @history ticket 86055 - 18/01/2023 - Fernando Macieira - Compra de produtos químicos controlados
			cNivel := PAG->PAG_NIVEL
			If lProdPF .and. cTipLib == "A"
				cNivel := Soma1(PAG->PAG_NIVEL)
			EndIf
			//
...
/*/{Protheus.doc} nomeStaticFunction PutCTD
	ticket 85790 - 16/01/2023 - Fernando Macieira - OPERAÇÃO NOVA MATRIZES - ITEM CONTÁBIL
...
Static Function PutCTD()
	...
	If FWCodEmp() == "01" .and. (FWCodFil() >= "0C" .AND. FWCodFil() <= "0R")
		gdFieldPut("C7_ITEMCTA", cItemCTD, n)
	EndIf
	
	// @history ticket 94010 - 25/05/2023 - Fernando Macieira - EMPRESAS NOVAS (EMPRESAS 11, 12 E 13) - ITEM CONTÁBIL 221
	If AllTrim(FWCodEmp()) == "11" .or. AllTrim(FWCodEmp()) == "12" .or. AllTrim(FWCodEmp()) == "13"
		gdFieldPut("C7_ITEMCTA", cItemNEW, n)
	EndIf
	// 

Return

/*/{Protheus.doc} Static Function GetSB5
	Função para checar se o produto é controlado pela Polícia Federal, campos B5_PRODPF e B5_CODMAPA
	@type  Static Function
	@author user
	@since 18/01/2023
	@version version
	...
	(examples)
	@see (links_or_references)
	ticket 86055 - 18/01/2023 - Fernando Macieira - Compra de produtos químicos controlados
/*/
Static Function GetSB5(cProdC7)
	...
	If !Empty(cProdC7)
		If SB5->( FieldPos("B5_PRODPF")) > 0
			lRet := AllTrim(Upper(Posicione("SB5",1,FWxFilial("SB5")+cProdC7,"B5_PRODPF"))) == "S"
		EndIf
	EndIf

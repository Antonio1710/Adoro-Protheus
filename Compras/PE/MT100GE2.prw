...
/*/{Protheus.doc} User Function MT100GE2
	...
	@history ticket 74270  -  Fernando Macieira - 26/07/2022 - campo E2_VENCREA (Vencimento real) D+1, somente se este conteúdo for vencido. Calcular D+1. O resultado do cálculo D+1 poderá ser mudado pela função padrão que checa se é feriado/fds e joga para o próximo dia útil. No caso em que existir impostos, somente será mudado o título PAI. Os impostos gerados pelo sistema permanecerão íntegros
	@history ticket 81308  -  Antonio Domingos - 01/11/2022 - ALTERAR VENCIMENTO DE TITULOS A PAGAR NO DIA 12/10
...
User Function MT100GE2()
	...
	Local dNewVencrea := SE2->E2_VENCREA
	Local cTipoVencto := GetMV("MV_#SE2VEN",,"NF")
	Local nX
	Private _cEmpAt5 := SuperGetMv("MV_#EMPAT5",.F.,"01/02/09/13") //Codigo de Empresas Ativas Grupo 4 //ticket TI - Antonio Domingos - 17/05/2023
	...
	IF (cEmpAnt $ _cEmpAt5) .and. lChkXml //ticket TI - Antonio Domingos - 18/05/2023 
							...
							IF Chr( nX + 64 ) = aCols[nPos]
								IF STOD(sDtVenc) > SE2->E2_VENCTO
									...
									RecLock("SE2",.F.)
										...
										SE2->E2_VENCREA := DataValida( STOD(sDtVenc) , .T. ) //ticket 81308 01/11/2022 - Antonio Domingos
									MsUnlock()
	...
	Regras:
	Decisões da reunião:
	(Participaram: Valéria, Erica, Antônio e Macieira)
	- Não mudar campo E2_VENCTO (Vencimento)
	- Mudar apenas o campo E2_VENCREA (Vencimento real)
	=> Regras de quando tiver que mudar o conteúdo deste campo E2_VENCREA:
	. somente se este conteúdo for vencido
	. Calcular D+1
	. O resultado do cálculo D+1 poderá ser mudado pela função padrão que checa se é feriado/fds e joga para o próximo dia útil
	. No caso em que existir impostos, somente será mudado o título PAI. Os impostos gerados pelo sistema permanecerão íntegros
	*/
	If Upper(Left(AllTrim(SE2->E2_ORIGEM),3)) <> "FIN" // Se vier de outra origem que não seja a própria manunteção do contas a pagar
		If SE2->E2_VENCTO < msDate() // Se vencido
			dNewVencrea := msDate() + 1
			dNewVencrea := DataValida(dNewVencrea, .T.) // - Se .T. posterga a data recebida para o próximo dia últil - Se .F. retrocede a data recebida para o dia últil anterior.
			If AllTrim(SE2->E2_TIPO) $ AllTrim(cTipoVencto)
				u_GrLogZBE( msDate(), TIME(), cUserName, "TITULO " + SE2->E2_FILIAL + "/" + SE2->E2_PREFIXO + "/" + SE2->E2_NUM + "/" + SE2->E2_PARCELA + "/" + SE2->E2_TIPO + "/" + SE2->E2_FORNECE + "/" + SE2->E2_LOJA, "FINA050", "D+1", "ANTES ALTERACAO CAMPO E2_VENCREA " + DtoC(SE2->E2_VENCREA), ComputerName(), LogUserName() )
				RecLock("SE2", .F.)
					SE2->E2_VENCREA := DataValida(dNewVencrea,.T.)
				SE2->( msUnLock() )
				u_GrLogZBE( msDate(), TIME(), cUserName, "TITULO " + SE2->E2_FILIAL + "/" + SE2->E2_PREFIXO + "/" + SE2->E2_NUM + "/" + SE2->E2_PARCELA + "/" + SE2->E2_TIPO + "/" + SE2->E2_FORNECE + "/" + SE2->E2_LOJA, "FINA050", "D+1", "DEPOIS ALTERACAO CAMPO E2_VENCREA " + DtoC(SE2->E2_VENCREA), ComputerName(), LogUserName() )
			EndIf
		EndIf
	EndIf	

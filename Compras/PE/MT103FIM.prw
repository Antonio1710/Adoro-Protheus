...
/*/{Protheus.doc} User Function MT103FIM
  ...
  @history Ticket 78387   - Fernando Macieira - 17/08/2022 - Endereçamento itens rotina de Gera ajuste (DOC RATEIO - TABELA SD3)
  @history Ticket 78550   - Abel Babini       - 22/08/2022 - Correção de error.log no lançamento de NF da Ceres
  @history Ticket 78550   - Everson           - 25/08/2022 - Adicionado o parâmetro "BENEF". 
  @history Ticket 69574   - Abel Babini       - 05/09/2022 - Projeto FAI
  @history Ticket 81697   - Abel Babini       - 18/10/2022 - NF Complemento de IPI
  @history Ticket 18465   - Everson           - 22/11/2022 - Removido fitro para envio ao barramento.
  @history ticket TI      - Everson - 30/11/2022 - Inclusão de CT-e pela Ceres.
  ...
  @history Ticket 91374 - Antonio Domingos - 17/04/2022 - Corrigir regra centro de custo 5908 ItCta 112
...
User Function MT103FIM()
  ...
  Private _cEmpAt2 := SuperGetMv("MV_#EMPAT2",.F.,"01/02/13") //Codigo de Empresas Ativas Grupo 2 //ticket TI - Antonio Domingos - 17/05/2023
  Private _cEmpFL3 := SuperGetMv("MV_#EMPFL3",.F.,"0102/010B/1301") //Codigo de Empresas+Filiais Ativas Grupo 3 //ticket TI - Antonio Domingos - 18/05/2023
  Private _cEmpFL1 := SuperGetMv("MV_#EMPFL1",.F.,"0102/1301") //Codigo de Empresas+Filiais Ativas Grupo 1 //ticket TI - Antonio Domingos - 18/05/2023
  ...
  If Alltrim(cEmpAnt) $ _cEmpAt1 .and. nConfirma == 1 //ticket TI - Antonio Domingos - 18/05/2023 
    ...
    If (Alltrim(FunName()) == "MATA103") .and. SF1->F1_TIPO == "D" .and. alltrim(cEmpAnt)+Alltrim(cFilAnt) $ _cEmpFL3  .and. (nOpcao == 3 .or. nOpcao == 4) //Incluir e Classificar
      ...
      If !Empty(SF1->F1_X_SQED)
        ...
        IF alltrim(cEmpAnt)+Alltrim(cFilAnt) $ _cEmpFL1 //ticket TI - Antonio Domingos - 18/05/2023 
          TcSQLExec('EXEC ['+cLnkSrv+'].[SMART].[dbo].[FU_PEDIDEVOVEND_FATURA] ' +Str(SF1->(Recno()))+","+"'"+cEmpAnt+"'") //Ticket 69574   - Abel Babini       - 05/09/2022 - Projeto FAI
        ELSE
          TcSQLExec('EXEC ['+cLnkSrv+'].[SMART].[dbo].[FU_PEDIDEVOVEND_FATURA] ' +Str(SF1->(Recno()))+","+"'"+cEmpAnt+"','"+cFilAnt+"'" ) //Ticket 69574   - Abel Babini       - 05/09/2022 - Projeto FAI
        ENDIF
	      //EndTran()
	    EndIf
    ...
    If SF1->F1_TIPO == "D" .and. alltrim(cEmpAnt)+Alltrim(cFilAnt) $ _cEmpFL1  .and. (nOpcao == 3 .or. nOpcao == 4)  //ticket TI - Antonio Domingos - 18/05/2023 
	...
	if alltrim(cEmpAnt) $ _cEmpAt2  .and. lBlqICM //ticket TI - Antonio Domingos - 17/05/2023
	...
	if lBlqNvS .and. lGrBlqIcm .And. !IsInCallStack("U_ADFIS032P") .And. !IsInCallStack("U_ADFIS042P")//Everson - 02/10/2019. Chamado //Everson - 30/11/2022 - ticket TI.
	  //if cEmpAnt = '01' .or. cEmpAnt = '02'
	  if alltrim(cEmpAnt) $ _cEmpAt2  //ticket TI - Antonio Domingos - 17/05/2023
      If nConfirma == 1 .AND. nOpcao == 3
  ...
  IF nOpcao == 3 .OR. nOpcao == 4 //3: Incluir/ 4: Classificar
    ...
    IF nOpcao == 3 .OR. nOpcao == 4 .AND. nConfirma == 1
      ...
      //INICIO Ticket 81697   - Abel Babini       - 18/10/2022 - NF Complemento de IPI
      dbselectarea("SD1")
      dbsetorder(1)
      SD1->(dbseek(xfilial("SD1") + SF1->F1_DOC + SF1->F1_SERIE + SF1->F1_FORNECE + SF1->F1_LOJA,.T. ))
      ...
      While SD1->(!Eof())	.AND. SD1->D1_DOC == SF1->F1_DOC .AND. SD1->D1_SERIE == SF1->F1_SERIE .AND. ;
        ...
        IF Alltrim(SF1->F1_TIPO) == 'P' .AND. Alltrim(SF1->F1_ESPECIE) == 'SPED' .AND. !Empty(Alltrim(SD1->D1_PEDIDO)) .AND. !Empty(Alltrim(SD1->D1_ITEMPC)) 
          dbselectarea("SC7")
          dbsetorder(1)
          IF SC7->(dbSeek(SD1->D1_FILIAL+SD1->D1_PEDIDO+SD1->D1_ITEMPC))
            IF SC7->C7_QUANT == 1 .AND. SC7->C7_QUJE == 0
        ...
        SD1->(dbskip())
      EndDo              
      //FIM Ticket 81697   - Abel Babini       - 18/10/2022 - NF Complemento de IPI

    ENDIF
    // *** FINAL CHAMADO 053259 - WILLIAM COSTA - 23/01/2020

    //Everson - 03/03/2023 - ticket 18465.
  ...
  If nConfirma == 1 .and. (nOpcao == 3 .or. nOpcao == 4) .and. AllTrim(SF1->F1_TIPO) == "N"
    ...
    If AllTrim(Posicione("SA2",1,FWxFilial("SA2")+SF1->F1_FORNECE+SF1->F1_LOJA,"A2_XTIPO")) == "4" // // A2_FILIAL, A2_COD, A2_LOJA, R_E_C_N_O_, D_E_L_E_T_
      ...
      If LockByName("TERCEIRO", .T., .F.)
        u_UpSDASDB()
        //Everson - 25/08/2022. Chamado 78550. Adicionado o parâmetro "BENEF".
        u_ChkSDA("BENEF") // @history Ticket 62276   - Fer Macieira    - 01/12/2021 - Endereçamento automático - Armazéns de terceiros 70 a 74 - Projeto Industrialização - Alguns casos o EXECAUTO retorna ERRO
      EndIf
  ...
  //Everson - 22/11/2022 - ticket 18465.
  ...
  // If FieldPos("F1_XPOSORD") > 0 .And. nConfirma == 1 .And. SF1->F1_XPOSORD == "1"
    ...
    U_M103F02(SF1->F1_DOC+SF1->F1_SERIE+SF1->F1_FORNECE+SF1->F1_LOJA) //Everson - 03/03/2023 - ticket 18465.

  // EndIf
  //

  //INICIO Ticket 81697   - Abel Babini       - 18/10/2022 - NF Complemento de IPI
  IF Alltrim(SF1->F1_TIPO) == 'P' .AND. Alltrim(SF1->F1_ESPECIE) == 'SPED' .AND. !Empty(Alltrim(SD1->D1_PEDIDO)) .AND. !Empty(Alltrim(SD1->D1_ITEMPC)) .AND. nOpcao == 5 .AND. nConfirma == 1
      ...
      While SD1->(!Eof())	.AND. SD1->D1_DOC == SF1->F1_DOC .AND. SD1->D1_SERIE == SF1->F1_SERIE .AND. ;
        ...
        IF Alltrim(SF1->F1_TIPO) == 'P' .AND. Alltrim(SF1->F1_ESPECIE) == 'SPED' .AND. !Empty(Alltrim(SD1->D1_PEDIDO)) .AND. !Empty(Alltrim(SD1->D1_ITEMPC)) 
          ...
          dbsetorder(1)
          IF SC7->(dbSeek(SD1->D1_FILIAL+SD1->D1_PEDIDO+SD1->D1_ITEMPC))
            IF SC7->C7_QUANT == 1 .AND. SC7->C7_QUJE == 1
...
User Function UpSDASDB()
  ...
  Local nX
  Local dDtSDB     := CtoD("//")
  Local _cEmpFL1 := SuperGetMv("MV_#EMPFL1",.F.,"0102/1301") //Codigos de Empresas+Filiais Ativas Grupo 1 //ticket TI - Antonio Domingos - 23/05/2023
        ...
        If SD1->( dbSeek(FWxFilial("SD1")+SF1->F1_DOC+SF1->F1_SERIE+SF1->F1_FORNECE+SF1->F1_LOJA) )
          ...
          Do While SD1->(!EOF()) .and. SF1->(F1_FILIAL+F1_DOC+F1_SERIE+F1_FORNECE+F1_LOJA) == SD1->(D1_FILIAL+D1_DOC+D1_SERIE+D1_FORNECE+D1_LOJA)
            ...
            If Localiza(SD1->D1_COD)
              ...
              If SDA->( dbSeek(FWxFilial("SDA")+SD1->(D1_COD+D1_LOCAL+D1_NUMSEQ+D1_DOC+D1_SERIE+D1_FORNECE+D1_LOJA)) ) //Everson - 02/02/2022. Chamado 67570.
                dDtSDB := SDA->DA_DATA
              Else
...
User Function ChkSDA(cTipo)
  ...
  Local nX
  Local aTamSX3    := {} //Ticket 78550   - Abel Babini       - 22/08/2022 - Correção de error.log no lançamento de NF da Ceres

  Default cTipo := "" // @history Ticket 78387 - Fernando Macieira - 17/08/2022 - Endereçamento itens rotina de Gera ajuste (DOC RATEIO - TABELA SD3)
  ...
  If cTipo == "BENEF" .OR. cTipo == "RATEIO"
    tcQuery cQuery New Alias "Work3"

    aTamSX3	:= FWTamSX3("DA_SALDO") //Ticket 78550   - Abel Babini       - 22/08/2022 - Correção de error.log no lançamento de NF da Ceres
    tcSetField("Work3", "DA_SALDO", aTamSX3[3], aTamSX3[1], aTamSX3[2])

    // @history Ticket 67479   - Fer Macieira    - 01/02/2022 - Endereçamento produto 383104 - Retorno Coopeval
    aTamSX3	:= FWTamSX3("DA_DATA") //Ticket 78550   - Abel Babini       - 22/08/2022 - Correção de error.log no lançamento de NF da Ceres
...
/*/{Protheus.doc} UpItemCta
  ...
  @see (links_or_references)
  @history Ticket 69712 - Fernan Macieira - 14/03/2022 - Integração Notas Centro de Custo 5134 - Item 113
  @history Ticket 91374 - Antonio Domingos - 10/04/2022 - Corrigir regra centro de custo 5908
/*/
Static Function UpItemCta()
  ...
  Local _cItCta112 := SuperGetMV("MV_#ITM112",.F.,"112")  //Ticket 91374 - Antonio Domingos - 10/04/2022
  Local _cCC5908   := SuperGetMV("MV_#CC5908",.f.,"5908") //Ticket 91374 - Antonio Domingos - 10/04/2022
  ...
  If SD1->( dbseek(SF1->F1_FILIAL + SF1->F1_DOC + SF1->F1_SERIE + SF1->F1_FORNECE + SF1->F1_LOJA) )
    ...
    Do While SD1->( !EOF() ) .and. SF1->F1_FILIAL+SF1->F1_DOC+SF1->F1_SERIE+SF1->F1_FORNECE+SF1->F1_LOJA == SD1->D1_FILIAL+SD1->D1_DOC+SD1->D1_SERIE+SD1->D1_FORNECE+SD1->D1_LOJA
      ...
      If SD1->D1_FILIAL == '03' .And. alltrim(SD1->D1_CC) $ _cCC5908 //Ticket 91374 - Antonio Domingos - 10/04/2022
      
        Reclock("SD1", .F.)
          SD1->D1_ITEMCTA := _cItCta112
        SD1->( msUnlock() )

      ElseIf AllTrim(SD1->D1_CC) $ AllTrim(cCC5134) .and. AllTrim(SD1->D1_ITEMCTA) <> AllTrim(cMVItemCta)
...
Return Nil
/*/{Protheus.doc} User Function M103F01
  Envia dados para o barramento.
  @type  Function
  @author Everson
  @since 28/02/2023
  @version 01
/*/
User Function M103F01()

    //Variáveis.
    Local aArea := GetArea()

    If ! MsgYesNo("Deseja enviar o registro para o barramento?", "Função M103F01(MT103FIM)")
      RestArea(aArea)
      Return Nil

    EndIf

    If Alltrim(cValToChar(SF1->F1_ESPECIE)) == "CTE"
      MsAguarde({|| U_MT116A1(4, SF1->F1_DOC+SF1->F1_SERIE+SF1->F1_FORNECE+SF1->F1_LOJA) }, "Aguarde", "Enviando registro(CT-e)...")
    
    Else
      MsAguarde({|| grvBarr(4, SF1->F1_DOC+SF1->F1_SERIE+SF1->F1_FORNECE+SF1->F1_LOJA) }, "Aguarde", "Enviando registro(NF)...")

    EndIf
  
    MsgInfo("Processo finalizado.", "Função M103F01(MT103FIM)")

    RestArea(aArea)

Return Nil

/*/{Protheus.doc} User Function M103F02
  Encerra contrato de compra de matéria-prima.
  @type  Function
  @author Everson
  @since 03/03/2023
  @version 01
 /*/
User Function M103F02(nNumero)
  ...
  If ! SD1->(DbSeek(FWxFilial("SD1") + nNumero))
    RestArea(aArea)
    Return Nil

  EndIf

  While ! SD1->(Eof()) .And. FWxFilial("SD1") == SD1->D1_FILIAL .And. SD1->(D1_DOC+D1_SERIE+D1_FORNECE+D1_LOJA) == nNumero
    ...
    If Empty(SC7->C7_XCTRCOM) .Or. Empty(SC7->C7_XCTRVER)
      SD1->(DbSkip())
      Loop

    EndIf

    U_ADCOM42I(SC7->C7_XCTRCOM, SC7->C7_XCTRVER)

...
/*/{Protheus.doc} User Function MT100TOK
	...
	@history ticket 83280 - Abel Babini    		- 14/11/2022 - RETORNOS DE INDUSTRIALIZAÇÃO JAMPAC X AlMOXARIFADO
	@history ticket TI    - Everson - 30/11/2022 - Inclusão de CT-e pela Ceres.
	@history ticket 84321 - Antonio Domingos - 02/12/2022 - Correção do erro "variable does not exist CA100FOR on U_MT100TOK(MT100TOK.PRW) 01/12/2022 06:32:04 line : 19"
	@history Ticket 18465 - Everson - 19/01/2023 - Valida pedido na entrada de MP - Processo de compra de MP com entrega futura.
	@history Ticket 93361 - Rafael Moraes Rosa - 09/05/2023 - Regra Complementada para Industrializacao sobre produtos do tipo PA
	@history ticket TI - Antonio Domingos    - 13/05/2023 - Ajuste Nova Empresa
	@history ticket TI - Antonio Domingos - 22/05/2023 - RevisÃ£o Ajuste Nova Empresa
...
User Function MT100TOK()
	...
	Private _cEmpAt2 := SuperGetMv("MV_#EMPAT2",.F.,"01/02/13") //Codigo de Empresas Ativas Grupo 2 //ticket TI - Antonio Domingos - 22/05/2023
	...
	*/
	...
	If _lRet
		...
		For nCont :=1 To Len(aCols)
			...
			If !gdDeleted(nCont)
				...
				//Everson - 19/01/2023 - Ticket 18465.
				If ! U_ADCOM42F(gdFieldGet("D1_PEDIDO", nCont), gdFieldGet("D1_COD", nCont), gdFieldGet("D1_TES", nCont), .F.)
					RestArea(_aArea)
					Return .F.
				...
				If cEmpAnt $ _cEmpAt1 // Adoro //ticket TI - Antonio Domingos - 22/05/2023 
					...
					// ticket 83280 - Abel Babini    		- 14/11/2022 - RETORNOS DE INDUSTRIALIZAÇÃO JAMPAC X AlMOXARIFADO
					//If cEmpAnt $ GetMV("MV_#BENEMP",,"01") .and. cFilAnt $ GetMV("MV_#BENFIL",,"02/0B")
					If Alltrim(cEmpAnt)+Alltrim(cFilAnt) $ _cEmpFL3  //ticket TI - Antonio Domingos - 22/05/2023 
						...
						If ALLTRIM(FUNNAME()) == "MATA920"  //ticket  84321 - Antonio Domingos - 02/12/2022
							cArm3 := Posicione("SA2",1,FWxFilial("SA2")+C920CLIENT+C920LOJA,"A2_LOCAL") // Armazém de retorno para consumo da rotina de controladoria
						else
							cArm3 := Posicione("SA2",1,FWxFilial("SA2")+CA100FOR+CLOJA,"A2_LOCAL") // Armazém de retorno para consumo da rotina de controladoria
						EndIf		
						//*** INICIO ticket 83280 - Abel Babini    		- 14/11/2022 - RETORNOS DE INDUSTRIALIZAÇÃO JAMPAC X AlMOXARIFADO
						If AllTrim(SA2->A2_XTIPO) == '4' .and. Alltrim(cEmpAnt)=='01' .and. Alltrim(cFilAnt) == '0B' // Terceiro
							...
							If cCFOP $ GetMV("MV_#BENCFO",,"1902#2902#1903#2903#1925#2925#")
								...
								If !Empty(cArm3)
									...
									//ticket 93361 - Rafael Moraes Rosa - 09/05/2023 - Regra Complementada para Industrializacao sobre produtos do tipo PA - INICIO
									IF Posicione("SB1",1,FWxFilial("SB1")+cProd,"B1_TIPO") = "PA"
										cArm3	:= GetNewPar("MV_#ALINPA","52")
									ENDIF
									//ticket 93361 - Rafael Moraes Rosa - 09/05/2023 - Regra Complementada para Industrializacao sobre produtos do tipo PA - FIM
								...
								EndIf
							EndIf
						//*** FIM ticket 83280 - Abel Babini    		- 14/11/2022 - RETORNOS DE INDUSTRIALIZAÇÃO JAMPAC X AlMOXARIFADO
						ElseIf AllTrim(SA2->A2_XTIPO) == '4' // Terceiro
				...
				If AllTrim(FWCodEmp()) == "11" .or. AllTrim(FWCodEmp()) == "12" .or. AllTrim(FWCodEmp()) == "13"
					gdFieldPut("D1_ITEMCTA", cItemNEW, nCont)
				EndIf
				// 
				...
				For i:=1 to Len(aCols)
					...
					If !gdDeleted(i)
						...
						If lTESFin
							...
							Else
								//Ticket 81697   - Abel Babini       - 18/10/2022 - NF Complemento de IPI
								If (cTipo $ 'P')	
									nTtNF += nTt
								Else
	...
	For _i:=1 To Len(aCols)
		...
		if !acols[_i][len(aheader)+1] //verifica se esta deletado
			...
			IF found()
				...
				If SF4->F4_ESTOQUE == "S" .And. Alltrim(cEmpAnt) $ _cEmpAt2 //ticket TI - Antonio Domingos - 22/05/2023 
	...
	If !IsInCallStack("U_CENTNFEXM") .And. !IsInCallStack("U_ADFIS032P") .And. !IsInCallStack("U_ADFIS042P")//Everson - 19/07/2019. Chamado 044314. //Everson - 30/11/2022 - ticket TI.

...
/*/{Protheus.doc} User Function MT121BRW
	...
	@history Ticket 76976   - Antonio Domingos     - 08/08/2022 - ALTERAR DATA DE ENTREGA POR PEDIDO Por Dt Entrega, Por Pedido, Por Item
	@history Ticket 18465   - Everson			   - 20/10/2022 - Enviar registro para o barramento.
	@history Ticket 18465   - Everson			   - 31/10/2022 - Enviar manualmente o registro para o barramento.
/*/
...
User Function MT121BRW()
	...
	//Everson - 31/10/2022 - ticket 18465.
	If cEmpAnt == "01" .And. cFilAnt == "03"
		Aadd( aRotina, {'Env Barramento', "MsAGuarde({|| U_grvSC7Barr(SC7->C7_NUM, .T., .F., 4) }, 'Aguarde', 'Processando...')", 0, 14, 0, Nil} )
...
User Function PCFRETE()    
	...
	For ni := 1 to Len(aLstC01)
		If !Empty(aLstC01[ni])
			aAdd(aHeader01,{ aLstC01[ni]							,;
							GetSX3Cache(aLstC01[ni], 'X3_PICTURE')	,;
							GetSX3Cache(aLstC01[ni], 'X3_TITULO')	,;
							GetSX3Cache(aLstC01[ni], 'X3_TAMANHO')	,;
							GetSX3Cache(aLstC01[ni], 'X3_TIPO')	})
		endif
	Next ni
	...
	If !(cAliasT)->(Eof())
		...
		For ni := 1 to Len(aHeader01)
			If !Empty(aHeader01[ni][TCB_POS_CMP]) .AND. aHeader01[ni][TCB_POS_TIP] # "C"
				...
				//If SX3->(Found())
					TcSetField(cAliasT,	GetSX3Cache(aHeader01[ni][TCB_POS_CMP], "X3_CAMPO")	 ,;
										GetSX3Cache(aHeader01[ni][TCB_POS_CMP], "X3_TIPO")	 ,;
										GetSX3Cache(aHeader01[ni][TCB_POS_CMP], "X3_TAMANHO"),;
										GetSX3Cache(aHeader01[ni][TCB_POS_CMP], "X3_DECIMAL") )
				//Endif
			Endif
		Next ni
				...
				For r := 1 to Len(aLstPED)
					...
					If DbSeek(xFilial("SC7") + Alltrim(aLstPED[r][1]))
						While SC7->(!EOF()) .and. SC7->C7_FILIAL == xFilial("SC7") .and. SC7->C7_NUM == Alltrim(aLstPED[r][1])
							...
							U_grvSC7Barr(SC7->C7_NUM, .T., .F., 4) //Everson - 20/10/2022 - ticket 18465.
...
//DT.ENTREGA
	...
	Local nRadio := 1
	Local aItems := {'Por Pedido','Por Item' }
	Private dNewDT     := SC7->C7_DATPRF
	Private  dAtuDT     := SC7->C7_DATPRF
	...
	If lUsrAut
		...
		EndIf
	EndIf
	//fVldItPc(_cC7_FILIAL,_cPedido,_cPRODUTO,_cC7_ITEM,_cC7_USER,_nC7_QUJE,_nC7_QTDACLA,_cC7_ENCER,_cC7_RESIDUO)
	lRet := U_fVldItPc(SC7->C7_FILIAL,SC7->C7_NUM,SC7->C7_PRODUTO,SC7->C7_ITEM,SC7->C7_USER,SC7->C7_QUJE,SC7->C7_QTDACLA,SC7->C7_ENCER,SC7->C7_RESIDUO)
	...
	If lRet
		...
		Do While .T.
			...
			DEFINE MSDIALOG oDlgAltDtEnt TITLE "Nova DT.Entrega" FROM 0,0 TO 168,385  OF oMainWnd PIXEL
				
				@ 003, 003 TO 068,190 PIXEL OF oDlgAltDtEnt
				
				@ 015,018 Say "Dt.Entrega Atual:" of oDlgAltDtEnt PIXEL
				@ 010,065 MsGet oCmpAtuDT Var dAtuDT WHEN .F. SIZE 100,12 of oDlgAltDtEnt PIXEL 
				@ 025,018 Say "Nova Dt.Entrega:" of oDlgAltDtEnt PIXEL
				@ 020,065 MsGet oCmpNewDT Var dNewDT SIZE 100,12 of oDlgAltDtEnt PIXEL Valid dNewDT >= Date() .And. dNewDT <> dAtuDT
				oRadio := TRadMenu():New (35,018,aItems,,oDlgAltDtEnt,,,,,,,,100,12,,,,.T.)
				oRadio:bSetGet := {|u|Iif (PCount()==0,nRadio,nRadio:=u)}

				@ 070,015 BUTTON OBTNALTDT[01] PROMPT "Confirma" of oDlgAltDtEnt SIZE 68,12 PIXEL ACTION (lOKAltDT := .t. , oDlgAltDtEnt:End())
				@ 070,105 BUTTON OBTNALTDT[02] PROMPT "Cancela"  of oDlgAltDtEnt SIZE 68,12 PIXEL ACTION oDlgAltDtEnt:End()
			...
			If lOKAltDT
				...
				If lRet
					If msgNoYes("Tem certeza de que deseja alterar Data de Entrega do pedido de compra? " + CHR(13) + CHR(10) + "Atenção: A Data de Entrega será alterado a partir desta confirmação, caso não queira alterar clique no botão cancelar...")
						MsgRun( "Efetuando alteração da Dt.Entrega...","Específico - Altera Data Entrega", { || RunAltDTPC(dNewDT,cPedido,cItem,nRadio,dAtuDT) } )
						If _lfRet
...
/*/{Protheus.doc} User Function RunAltDTPC
	...
	@version 01
	@history Chamado 054127 - ADRIANO SAVOINE - 20/12/2019 - Alteração da Data de Entrega sem passar pela Alçada de aprovação.
	@history Ticket 76976 - Antonio Domingos - 08/08/2022 - ALTERAR DATA DE ENTREGA POR PEDIDO
	/*/


Static Function RunAltDTPC(dNewDT,cPedido,cItem,nRadio,dAtuDT)

	LOCAL aAreaAnt := GETAREA()
	Local lRet      := .F.
	Local _cNextAlias := GetNextAlias()
	Local _cC7_ENCER   := 'E'
	Local _cC7_RESIDUO := 'S'
	Begin Transaction

		//Ticket TI - 04/03/2021 - Adriano Savoine - Inicio
		//Ticket 76976 - Antonio Domingos - 08/08/2022 - ALTERAR DATA DE ENTREGA POR PEDIDO
		If nRadio = 1 //Por Pedido - //Ticket 76976 - Antonio Domingos - 08/08/2022 - ALTERAR DATA DE ENTREGA POR PEDIDO
			dbselectarea("SC7")
			DBSETORDER(1)
			DBSeek(xFilial("SC7")+cPedido)      // Posiciona o Pedido
			If SC7->(Eof())
				Alert("Não foram encontrados pedidos com o Numero "+cPedido+"!")
			Else
				WHILE !SC7->(EOF()) .AND. xFilial("SC7")==SC7->C7_FILIAL .AND. SC7->C7_Num == cPedido
					lRet := U_fVldItPc(SC7->C7_FILIAL,SC7->C7_NUM,SC7->C7_PRODUTO,SC7->C7_ITEM,SC7->C7_USER,SC7->C7_QUJE,SC7->C7_QTDACLA,SC7->C7_ENCER,SC7->C7_RESIDUO)
					If !lRet
						Alert("Data de Entrega não alterada!")
						Exit
					EndIf
					SC7->(dbSkip())
				EndDo
			...
			If lRet
				dbselectarea("SC7")
				DBSETORDER(1)
				DBSeek(xFilial("SC7")+cPedido,.T.)      // Posiciona o Pedido
				WHILE !SC7->(EOF()) .AND. xFilial("SC7")==SC7->C7_FILIAL .AND. SC7->C7_Num == cPedido
		...
		Else //Por Item //Ticket 76976 - Antonio Domingos - 08/08/2022 - ALTERAR DATA DE ENTREGA POR PEDIDO
			dbselectarea("SC7")
			DBSETORDER(1)
			DBSeek(xFilial("SC7")+cPedido+cItem)  // Posiciona o Pedido
			//TICKET 76976 ALTERAR DATA DE ENTREGA POR PEDIDO
			WHILE !SC7->(EOF()) .and. SC7->C7_Num == cPedido .and. C7_ITEM == cItem
...
//-------------------------------------------------------------------
/*/{Protheus.doc} AltPcGrv()
Funcao de Validação Geral do Pedido de Compra que está sendo alterado
@author Antonio Domingos
@since 15/07/2022
@version 1.0
@history Ticket 76976   - Antonio Domingos     - 08/08/2022 - ALTERAR DATA DE ENTREGA POR PEDIDO Por Dt Entrega, Por Pedido, Por Item
/*/
//-------------------------------------------------------------------
User Function fVldItPc(_cC7_FILIAL,_cPedido,_cPRODUTO,_cC7_ITEM,_cC7_USER,_nC7_QUJE,_nC7_QTDACLA,_cC7_ENCER,_cC7_RESIDUO)

Local lRet := .T.

	// Consisto comprador com login
	If lRet
		If AllTrim(_cC7_USER) <> AllTrim(RetCodUsr()) .And.;
		   ! checkGrp(AllTrim(_cC7_USER), AllTrim(RetCodUsr()), "Alteração data de entrega " + _cPedido) //Everson. 04/10/2021. Chamado 43012.
			lRet := .f.
			Aviso(	"MT121BRW-fVldItPc-01",;
	...
	// C7_ENCER Consisto se Pedido foi encerrado
	If lRet
		If AllTrim(_cC7_ENCER) = "E"
			lRet := .f.
			Aviso(	"MT121BRW-fVldItPc-02",;
					"Este PC foi encerrado... Alteração da Data de Entrega não permitida!",;
					{ "&Retorna" },,;
					"Altera Data de Entrega" )
		EndIf
	EndIf
	//C7_RESIDUO Consisto se o Pedido foi encerrado por residuo
	If lRet
		If AllTrim(_cC7_RESIDUO) = "S"
			lRet := .f.
			Aviso(	"MT121BRW-fVldItPc-03",;
					"Este PC foi encerrado por residuo... Alteração da Data de Entrega não permitida!",;
					{ "&Retorna" },,;
					"Altera Data de Entrega" )
		EndIf
	EndIf
	//Consisto se o Pedido foi atendido ou parcialmente atendido
	If lRet
		If _nC7_QUJE <> 0
			lRet := .f.
			Aviso(	"MT121BRW-fVldItPc04",;
					"Pedido de compra atendido ou parcialmente atendido... Alteração da Data de Entrega não permitida!",;
					{ "&Retorna" },,;
					"Altera Data de Entrega" )
		EndIf
	EndIf
	//Consisto se o Pedido foi usado em pré-nota	
	If lRet
		If _nC7_QTDACLA > 0
			lRet := .f.
			Aviso(	"MT121BRW-fVldItPc-05",;
	...
	If lRet
		FIE->( dbSetOrder(1) ) // FIE_FILIAL+FIE_CART+FIE_PEDIDO
		If FIE->( dbSeek( FWxFilial("FIE")+"P"+_cPedido ) )
			lRet := .f.
			Aviso(	"MT121BRW-fVldItPc-05",;

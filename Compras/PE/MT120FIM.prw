...
/*/{Protheus.doc} User Function MT120FIM
    ...
    @history Ticket 18465   - Eveson            - 18/10/2022 - Tratamento para envio do pedido para o barramento após aprovação automática vinculada ao contrato de compra.
    @history Ticket 18465   - Eveson            - 16/11/2022 - Correção da filial.
...
User Function MT120FIM()
    ...
    If INCLUI .Or. nOpc == 3 .And. ! Empty(SC7->C7_OP)
        U_ADMNT220(SC7->C7_OP, "", "INCLUSÃO DE PEDIDO DE COMPRA " + SC7->C7_NUM, "")

    EndIf

    //Everson - 16/02/2023 - ticket 88099.
    If ALTERA .Or. nOpc == 4 .And. ! Empty(SC7->C7_OP)
        U_ADMNT220(SC7->C7_OP, "", "ALTERAÇÃO DE PEDIDO DE COMPRA " + SC7->C7_NUM, "")
